...
def save_photo_result(image_bytes: bytes, uploaded_name: str, result) -> None:
    ...
    score = float(getattr(result, "total_score", 0.0) or 0.0)
    data = _safe_json_load(PHOTO_HISTORY_FILE)
    data = [item for item in data if not (item.get("date") == today and item.get("image_hash") == image_hash)]
    data.append({
        "date": today,
    ...
    _safe_json_save(PHOTO_HISTORY_FILE, data[-500:])


def get_today_photo_entries(selected_date: str | None = None) -> list:
    now = datetime.now(TURKEY_TZ)
    target_date = selected_date or now.strftime("%Y-%m-%d")
    data = _safe_json_load(PHOTO_HISTORY_FILE)
    todays = [item for item in data if item.get("date") == target_date]
    return sorted(todays, key=lambda x: (float(x.get("score", 0)), x.get("timestamp", "")), reverse=True)


def delete_photo_entry(image_hash: str, selected_date: str | None = None) -> None:
    target_date = selected_date or datetime.now(TURKEY_TZ).strftime("%Y-%m-%d")
    data = _safe_json_load(PHOTO_HISTORY_FILE)
    filtered = [
...
def select_photo_of_the_day(selected_date: str | None = None) -> dict:
    ...
    winner["ready"] = now.time() >= time(PHOTO_OF_DAY_HOUR, 0) if target_date == now.strftime("%Y-%m-%d") else True
...
def render_photo_of_day_candidates(context: str = "main") -> None:
    ...
    if context == "sidebar":
        ...
        st.markdown(
            f"<div class='mini-note'>Skor: {float(pod.get('score', 0)):.1f}/100 · Gün içi yükleme: {int(pod.get('entries', 0))}</div>",
            unsafe_allow_html=True,
        )
        if pod.get("ready"):
            st.success(f"Türkiye saatine göre {PHOTO_OF_DAY_HOUR:02d}:00 sonrası en yüksek puanlı fotoğraf seçildi.")
        else:
            st.info(f"Saat {PHOTO_OF_DAY_HOUR:02d}:00'a kadar adaylar burada görünür. Şu an Türkiye saati {now.strftime('%H:%M')}. {PHOTO_OF_DAY_HOUR:02d}:00 sonrası lider otomatik günün fotoğrafı olur.")
        ...
        for idx, entry in enumerate(entries[:6]):
            ...
            with cols[1]:
                ...
                st.caption(f"Skor: {float(entry.get('score', 0)):.1f}/100")
                ts = str(entry.get('timestamp', ''))
                if 'T' in ts:
                    st.caption(f"Saat: {ts[11:16]}")
                if st.button("Sil", key=f"sidebar_delete_{idx}_{entry.get('image_hash', '')}", use_container_width=True):
                    delete_photo_entry(str(entry.get("image_hash", "")))
                    st.rerun()
        if st.button("Bugünkü adayları temizle", key="sidebar_clear_today_candidates", use_container_width=True):
            clear_today_photo_entries()
            st.rerun()
    ...
    badge_label = "🏆 Günün Fotoğrafı" if pod.get("ready") else "⭐ Şu An Lider"
    leader_name = escape(str(pod.get('filename', 'fotoğraf')))
    leader_score = float(pod.get('score', 0))
    total_entries = int(pod.get('entries', 0))
    status_text = (
        f"Türkiye saatine göre {PHOTO_OF_DAY_HOUR:02d}:00 sonrası en yüksek puanlı fotoğraf otomatik olarak günün fotoğrafı oldu."
        if pod.get("ready")
        else f"Şu an lider fotoğraf burada öne çıkıyor. Saat {PHOTO_OF_DAY_HOUR:02d}:00 olduğunda seçim otomatik kesinleşecek."
    )
    ...
    with leader_cols[1]:
        ...
        st.markdown("<div class='pod-subtitle'>Günün fotoğrafı paneli Türkiye saatiyle çalışır ve lider fotoğrafı canlı olarak öne çıkarır.</div>", unsafe_allow_html=True)
        st.markdown(
            f"""
            <div class='pod-meta-grid'>
                <div class='pod-meta-card'>
                    <div class='pod-meta-label'>Skor</div>
                ...
                <div class='pod-meta-card'>
                    <div class='pod-meta-label'>Seçim saati</div>
                    <div class='pod-meta-value'>{PHOTO_OF_DAY_HOUR:02d}:00</div>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        fb_share_link = build_facebook_share_link(pod)
        share_url = build_photo_share_url(pod)
        share_caption = build_facebook_caption(pod)
        share_button_label = "Günün Fotoğrafını Paylaş"

        st.markdown("<div style='height:10px;'></div>", unsafe_allow_html=True)
        st.markdown("### Facebook")

        if pod.get("ready") and fb_share_link:
            st.markdown(
                f"""
                <a href="{fb_share_link}" target="_blank" style="
            ...
            st.caption("Paylaşım bağlantısı aktif. Facebook penceresi yeni sekmede açılır.")
        else:
            st.markdown(
                f"""
                <div style="
            ...
            st.caption(f"Buton Türkiye saatiyle {PHOTO_OF_DAY_HOUR:02d}:00'da aktif olur.")

        st.text_input(
            "Paylaşım bağlantısı",
            value=share_url,
            key=f"pod_share_url_{pod.get('image_hash', '')}"
        )

        with st.expander("Paylaşım metni"):
            st.code(share_caption, language="text")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    for idx, entry in enumerate(entries[:9]):
        ...
        with col:
            ...
            st.markdown(f"<div class='candidate-score-badge'>Skor: {float(entry.get('score', 0)):.1f}/100</div>", unsafe_allow_html=True)
            ts = str(entry.get('timestamp', ''))
            if 'T' in ts:
                st.caption(f"Saat: {ts[11:16]}")
            if st.button("🗑️ Sil", key=f"main_delete_{idx}_{entry.get('image_hash', '')}", use_container_width=True):
                delete_photo_entry(str(entry.get("image_hash", "")))
                st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
    if st.button("🧹 Bugünkü tüm adayları temizle", key="main_clear_today_candidates", use_container_width=True):
        clear_today_photo_entries()
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
...
def get_public_app_url() -> str:
    """
    Streamlit'te yayınlanan herkese açık uygulama adresi.
    Önce secrets/env'den okumayı dener, yoksa varsayılan örnek URL kullanır.
    """
    return _read_secret_or_env("PUBLIC_APP_URL", "https://cofsat.streamlit.app").rstrip("/")


def build_photo_share_url(photo_entry: dict) -> str:
    """
    Günün fotoğrafı için paylaşılabilir URL üretir.
    İleride ?photo=<hash> ile özel görünüm de açabilirsiniz.
    """
    base_url = get_public_app_url()
    image_hash = str(photo_entry.get("image_hash", "")).strip()
    if not image_hash:
        return base_url
    return f"{base_url}/?photo={image_hash}"


def build_facebook_share_link(photo_entry: dict) -> str:
    share_url = build_photo_share_url(photo_entry)
    if not share_url:
        return ""
    return f"https://www.facebook.com/sharer/sharer.php?u={quote_plus(share_url)}"


def build_facebook_caption(photo_entry: dict) -> str:
    filename = str(photo_entry.get("filename", "fotoğraf"))
    score = float(photo_entry.get("score", 0))
    return (
        f"ÇOFSAT Günün Fotoğrafı 📸\n\n"
        f"Bugünün öne çıkan karesi: {filename}\n"
        f"Skor: {score:.1f}/100\n\n"
        f"#cofsat #photography #streetphotography"
    )
...
def render_turkey_time_info(context: str = "sidebar") -> None:
    now = get_turkey_now()
    if context == "sidebar":
        st.markdown("### 🕒 Türkiye Saati")
        st.info(now.strftime("%d.%m.%Y %H:%M"))
        if now.time() >= time(PHOTO_OF_DAY_HOUR, 0):
            st.caption("Günün fotoğrafı seçimi aktif durumda.")
        else:
            st.caption(f"Günün fotoğrafı bugün saat {PHOTO_OF_DAY_HOUR:02d}:00'da seçilir.")
        return
...
GULER_STYLE_DNA = {
    "opening_moves": [
        "önce sahnenin havasını, ışığını ve dokusunu hissettir",
        "ardından iki ya da üç somut görsel öğeyi art arda diz",
        "kapanışta fotoğrafın etkisini yumuşak ama net bir cümleyle teslim et"
    ],
    "language_markers": [
        "gizem", "canlılık", "doku", "ton geçişi", "yansıma", "sade", "dengeli",
        "çerçeveleyici etki", "merak uyandıran", "beden dili", "yaşam öyküsü",
        "etkili", "güzel kompozisyon", "güzel kare"
    ],
    "tone": "yumuşak, zarif, duyusal ve kısa; teknikten çok atmosfer ve görsel hissi öne çıkaran",
    "avoid": [
        "sert yargı", "mekanik teknik rapor dili", "fazla uzun analiz", "soğuk editoryal hüküm"
    ],
    "signature": [
        "ışık, ton, doku ya da yansımadan en az ikisini mutlaka an",
        "kısa ve akıcı cümleler kur",
        "mümkünse küçük bir yaşam çağrışımı ya da duygu izi bırak",
        "yorumu yumuşak bir takdir cümlesiyle kapat"
    ]
}
...
MURSIDE_STYLE_DNA = {
    "opening_moves": [
        "önce insanın yüzünde, bakışında ya da beden dilinde saklı duyguyu yakala",
        "ardından sahnenin yalnızlık, aidiyet, masumiyet ya da sessiz acı gibi duygusal eksenini görünür kıl",
        "kapanışta fotoğrafın duygu aktarımını kısa ama etkili bir cümleyle teslim et"
    ],
    "language_markers": [
        "yalnızlık", "aidiyet", "masumiyet", "kaygı", "sessizlik", "sessiz çığlık",
        "duygu aktarımı", "güçlü an", "derin iz", "yaşamın yükü", "mistik duygu",
        "çok katmanlı anlatım", "belgesel fotoğraf", "çevresel portre", "güçlü portre"
    ],
    "tone": "empatik, insancıl, duygusal ama kısa; insan hikâyesine kalpten yaklaşan belgesel bir içtenlik taşır",
    "avoid": [
        "soğuk teknik rapor dili", "fazla uzun analiz", "sert yargı", "aşırı soyut teori"
    ],
    "signature": [
        "mutlaka insani bir bağ, kırılganlık ya da duygu izi an",
        "bakış, beden dili, yalnızlık ya da aidiyet gibi duygusal bir eksen kur",
        "gerekirse küçük bir toplumsal ya da yaşamsal çağrışım ekle",
        "yorumu güçlü duygu aktarımı ya da belgesel etkiyi teslim eden bir kapanışla bitir"
    ]
}
...
SEVGIN_STYLE_DNA = {
    "opening_moves": [
        "önce kadrajı, tonları ve ışığı net biçimde teslim et",
        "ardından modelin beden dili, ifade aktarımı ya da hareket etkisini görünür kıl",
        "kapanışta seyir keyfi, güzel kompozisyon ya da tebrik içeren kısa bir cümle kur"
    ],
    "language_markers": [
        "kadraj", "tonlar", "etkili ışık", "etkili kompozisyon", "güzel fotoğraf",
        "güzel portre", "seyir keyfi yüksek", "beden dili", "ifade aktarımı",
        "hareket netsizliği", "grafik soslar", "güzel zamanlama", "o an fotoğrafı", "emeğinize sağlık", "tebrikler"
    ],
    "tone": "kısa, net, olumlu ve destekleyici; teknik unsurla insan etkisini birlikte okuyan",
    "avoid": [
        "fazla şiirsellik", "uzun teorik çözümleme", "sert yargı", "kararsız ve muğlak dil"
    ],
    "signature": [
        "kadraj, ton ya da ışık gibi en az bir teknik öğeyi mutlaka an",
        "beden dili ya da ifade aktarımına değin",
        "yorumu kısa ve doğrudan tut",
        "kapanışta takdir eden bir ifade bırak"
    ]
}

GULCAN_STYLE_DNA = {
    "opening_moves": [
        "önce mimari, perspektif ya da mekânın kurduğu görsel cümleyi teslim et",
        "ardından modelin ya da yaşam öğesinin bu mekâna kattığı hikâyeyi aç",
        "kapanışta zamansız, şehre dair ya da film tadında bir estetik sonuçla bitir"
    ],
    "language_markers": [
        "mimari", "taş doku", "perspektif", "kadim şehir", "şehre dair",
        "çok katmanlı", "film tadında", "zamansız", "masalsı", "güzel kompozisyon",
        "tarihi mekan", "yaşam öğesi", "etkili", "tebrikler"
    ],
    "tone": "estetik, anlatı odaklı, betimleyici ve yumuşak; mekân ile hikâyeyi birlikte okuyan",
    "avoid": [
        "fazla teknik rapor dili", "sert hüküm", "kuru editoryal karar", "gereksiz uzatma"
    ],
    "signature": [
        "mekânsal bağlamı mutlaka vurgula",
        "ışık, perspektif ya da dokunun etkisini an",
        "fotoğrafın şehre, zamana ya da hayata dair çağrışımını aç",
        "kapanışı estetik ve olumlu tut"
    ]
}
...
- Önce sahnenin havasını, ışığını ve dokusunu hissettir.
- Yorumunu kısa tut; iki ya da üç cümlede görsel etkiyi görünür kıl.
- Işık, ton, doku, yansıma, beden dili ya da çerçeveleyici etki gibi somut öğeleri art arda an.
- Sert yargı kurma; eleştirin varsa bile bunu yumuşak bir dille söyle.
- Mümkünse küçük bir yaşam çağrışımı ya da atmosfer izi bırak.
- Şu söz alanlarını doğal biçimde kullanabilirsin: {', '.join(GULER_STYLE_DNA['language_markers'])}.
"""
    elif editor_name == "Sevgin Cingöz":
        extra_dna = f"""
Sevgin Çingöz yazılım DNA'sı:
- Önce kadrajı, tonları ve ışığı kısa ama net biçimde teslim et.
- Ardından modelin beden dili, ifade aktarımı ya da hareket etkisini görünür kıl.
- Yorumunu kısa tut; iki ya da üç cümlede doğrudan sonuca git.
- Teknik unsurla insan etkisini birlikte oku; seyir keyfi ve kompozisyon etkisini teslim et.
- Mümkünse güzel zamanlama, etkili kompozisyon ya da o an duygusunu an.
- Şu söz alanlarını doğal biçimde kullanabilirsin: {', '.join(SEVGIN_STYLE_DNA['language_markers'])}.
"""
    elif editor_name == "Mürşide Çilengir":
        extra_dna = f"""
Mürşide Çilengir yazılım DNA'sı:
- Önce insanın yüzünde, bakışında ya da beden dilinde saklı duyguyu yakala.
- Yorumu kısa tut; birkaç cümlede duygusal çekirdeği görünür kıl.
- Yalnızlık, aidiyet, masumiyet, kaygı ya da yaşamın yükü gibi bir eksen kurabiliyorsan bunu doğal biçimde işle.
- Teknik rapora dönme; insan hikâyesi, kırılganlık ve belgesel içtenlik önde olsun.
- Mümkünse güçlü bir an, sessiz bir çığlık ya da içte kalan bir iz hissi bırak.
- Şu söz alanlarını doğal biçimde kullanabilirsin: {', '.join(MURSIDE_STYLE_DNA['language_markers'])}.
"""
    elif editor_name == "Gülcan Ceylan Çağın":
        extra_dna = f"""
Gülcan Ceylan Çağın yazılım DNA'sı:
- Önce mekânın, mimarinin ya da perspektifin kurduğu görsel cümleyi teslim et.
- Ardından modelin ya da yaşam öğesinin bu sahneye kattığı hikâyeyi aç.
- Yorumu estetik ve akıcı tut; mekân ile anlatıyı birlikte oku.
- Zamansızlık, kadim şehir, çok katmanlı yapı ya da film tadında etki gibi çağrışımlar kullanabilirsin.
- Sert editoryal hüküm verme; daha çok estetik sonuç ve hikâye duygusunu görünür kıl.
- Şu söz alanlarını doğal biçimde kullanabilirsin: {', '.join(GULCAN_STYLE_DNA['language_markers'])}.
"""
...
EDITOR_PERSONAS = {
    ...
    "Güler Ataşer": {
        "focus": ["ışık", "atmosfer", "dokusal incelik", "ton geçişi", "yansıma", "beden dili", "çağrışım gücü"],
        "tone": "şiirsel, yumuşak, duyusal, zarif",
        "entry_style": "önce sahnenin havasını ve görsel etkisini teslim eder",
        "decision_style": "ışık, ton, doku ve çağrışım gücü üzerinden karar verir",
        "must_do": [
            "en az bir ışık veya ton vurgusu yap",
            "en az bir dokusal ayrıntı an",
            "mümkünse atmosfer ya da yaşam çağrışımı kur",
            "yorumu kısa ve akıcı tut"
        ],
        "avoid": ["sert hüküm", "mekanik analiz dili", "fazla teknik konuşmak", "soğuk editoryal ton"],
    },
    "Sevgin Cingöz": {
        "focus": ["kadraj", "ton", "ışık", "beden dili", "ifade aktarımı", "kompozisyon", "zamanlama"],
        "tone": "olumlu, destekleyici, net ve kısa",
        "entry_style": "kadraj ve tonlardan başlayarak fotoğrafın teknik gücünü teslim eder",
        "decision_style": "ışık, kadraj ve modelin beden dili üzerinden karar verir",
        "must_do": [
            "en az bir teknik unsur an",
            "beden dili veya ifade aktarımına değin",
            "yorumu kısa ve doğrudan tut",
            "takdir eden bir kapanış yap"
        ],
        "avoid": ["fazla romantizasyon", "uzun teori", "sert yargı", "muğlak cümle"],
    },
    "Mürşide Çilengir": {
        "focus": ["insan", "yakınlık", "duygu", "sessiz gerilim", "yalnızlık", "aidiyet", "masumiyet", "belgesel etki"],
        "tone": "empatik, insancıl, içten, duygusal ve belgesel duyarlılığı taşıyan",
        "entry_style": "önce yüz, bakış ya da beden dilindeki insani izi yakalar",
        "decision_style": "fotoğrafın kalbini insan ilişkilerinde, kırılganlıkta ve duygu aktarımında arar",
        "must_do": [
            "bakış, beden dili ya da mesafe hissini an",
            "yalnızlık, aidiyet, masumiyet ya da yaşam yükü gibi bir duygu ekseni kur",
            "yorumu kısa ama etkili tut",
            "duygu aktarımını ya da belgesel gücü teslim et"
        ],
        "avoid": ["soğuk teknik dil", "keskin yargı", "aşırı teknik raporlama", "fazla uzun çözümleme"],
    },
    "Gülcan Ceylan Çağın": {
        "focus": ["mimari", "perspektif", "mekânsal anlatı", "ışık", "zamansızlık", "şehir kültürü", "çok katmanlı kompozisyon"],
        "tone": "estetik, betimleyici, anlatı odaklı ve yumuşak",
        "entry_style": "mekânın ve mimarinin kurduğu görsel etkiyle açar",
        "decision_style": "mekân, perspektif ve yaşam öğesinin kurduğu hikâye üzerinden karar verir",
        "must_do": [
            "mekânsal bağlamı vurgula",
            "ışık, perspektif ya da dokudan söz et",
            "fotoğrafın hikâyesini veya çağrışımını aç",
            "estetik bir kapanış yap"
        ],
        "avoid": ["kuru editoryal hüküm", "fazla teknik dil", "sert yargı", "gereksiz uzatma"],
    },
}
...
EDITOR_VOICE_ENGINES = {
    ...
    "Güler Ataşer": {
        "lexicon": [
            "hava", "dokusal nefes", "ten", "süzülen ışık", "yumuşak yankı", "iç ısı",
            "gizem", "canlılık", "doku", "ton geçişi", "yansıma", "sade", "dengeli",
            "çerçeveleyici etki", "merak uyandıran", "beden dili", "yaşam öyküsü",
            "etkili", "güzel kompozisyon", "güzel kare"
        ],
        "openings": [
            "Bu karede ilk hissedilen şey atmosfer oluyor:",
            "Fotoğrafın etkisi önce ışık ve tonlardan geliyor:",
            "Burada sahneyi taşıyan şey, görüntünün havası:"
        ],
        "connectors": ["birlikte", "sayesinde", "katkısıyla", "aynı zamanda", "usulca"],
        "closings": [
            "Bütün bunlar birlikte güzel bir kompozisyon oluşturuyor.",
            "Bu etki kareyi daha da güçlü kılıyor.",
            "Ortaya sade ama etkili bir görüntü çıkıyor.",
            "Bu nedenle fotoğraf güzel bir atmosfer kuruyor."
        ],
        "cadence": "kısa gözlem / görsel öğeleri sıralama / zarif olumlu kapanış",
    },
    "Sevgin Cingöz": {
        "lexicon": [
            "kadraj", "tonlar", "etkili ışık", "etkili kompozisyon", "güzel fotoğraf",
            "güzel portre", "seyir keyfi yüksek", "beden dili", "ifade aktarımı",
            "hareket netsizliği", "grafik soslar", "güzel zamanlama", "o an fotoğrafı"
        ],
        "openings": [
            "Kadrajı ve tonlarıyla dikkat çeken bir kare:",
            "Etkili ışık ve kompozisyonun öne çıktığı bu fotoğrafta:",
            "Modelin beden dili ve ifade aktarımıyla öne çıkan bu karede:"
        ],
        "connectors": ["ile", "aynı zamanda", "birlikte", "özellikle"],
        "closings": [
            "Güzel bir kompozisyon. Emeğinize sağlık.",
            "Etkili bir fotoğraf, tebrikler.",
            "Seyir keyfi yüksek bir kare olmuş."
        ],
        "cadence": "kısa teknik giriş / beden dili vurgusu / olumlu kapanış",
    },
    "Mürşide Çilengir": {
        "lexicon": [
            "yakınlık", "kırılganlık", "içtenlik", "insani çekirdek", "sessiz temas", "duygu taşıyıcısı",
            "yalnızlık", "aidiyet", "masumiyet", "kaygı", "sessiz çığlık", "yaşamın yükü",
            "güçlü an", "duygu aktarımı", "belgesel fotoğraf", "güçlü portre", "çok katmanlı anlatım",
            "çevresel portre", "mistik duygu"
        ],
        "openings": [
            "Bu karede önce insana ait sessiz bir yük hissediliyor:",
            "Burada bana önce yüzün ve bedenin taşıdığı duygu geliyor:",
            "Fotoğrafın kalbi, insanın içinden geçen şeyi görünür kıldığı yerde açılıyor:"
        ],
        "connectors": ["orada", "sanki", "bir taraftan", "aynı anda", "bir bakıma"],
        "closings": [
            "Duygu aktarımı güçlü bir fotoğraf olmuş.",
            "Bu nedenle kare güçlü bir portre etkisi bırakıyor.",
            "Bütün bunlar birlikte çok etkili bir belgesel an kuruyor.",
            "Ortaya içtenliği yüksek, güçlü bir görüntü çıkıyor."
        ],
        "cadence": "insani giriş / duygusal eksen / kısa ve etkili kapanış",
    },
    "Gülcan Ceylan Çağın": {
        "lexicon": [
            "mimari", "taş doku", "perspektif", "kadim şehir", "şehre dair",
            "çok katmanlı", "film tadında", "zamansız", "masalsı", "güzel kompozisyon",
            "tarihi mekan", "yaşam öğesi", "etkili"
        ],
        "openings": [
            "Tarihi mekânın atmosferiyle dikkat çeken bu karede:",
            "Perspektifin güçlü etkisiyle kurulan bu görüntüde:",
            "Şehre dair detayların öne çıktığı bu fotoğrafta:"
        ],
        "connectors": ["ile", "birlikte", "karşın", "aynı anda"],
        "closings": [
            "Ortaya zamansız bir kompozisyon çıkmış.",
            "Şehre dair etkileyici bir anlatım oluşmuş.",
            "Çok katmanlı ve estetik bir kare olmuş."
        ],
        "cadence": "mekânsal giriş / anlatı vurgusu / estetik kapanış",
    },
}
...
def _editor_issue_sentence(editor_name: str, profile: Optional["SceneProfile"], scores: Dict[str, float]) -> str:
    ...
    if editor_name == "Gülcan Ceylan Çağın":
        return _score_signal(
            scores,
            "editoryal_deger",
            "Mekân, ışık ve yaşam öğesi birlikte etkili bir cümle kurmuş; bu haliyle kare estetik olarak güçlü duruyor.",
            "Anlatı çalışıyor ama bazı yerlerde biraz daha sadeleşirse fotoğrafın mekânsal etkisi daha temiz duyulur.",
            f"Özellikle {getattr(profile, 'distraction_region', 'yan alan')} tarafındaki fazlalık hafiflediğinde, perspektif ve hikâye çok daha zamansız bir etki bırakacaktır."
        )
    if editor_name == "Sevgin Cingöz":
        return _score_signal(
            scores,
            "kompozisyon",
            "Kadraj ve ışık iyi çalışıyor; beden dilinin etkisiyle kompozisyon daha da güçlenmiş görünüyor.",
            "Fotoğrafın etkisi var ama bazı yerlerde yapı biraz gevşiyor; küçük bir toparlama seyir keyfini yükseltir.",
            f"Özellikle {getattr(profile, 'secondary_region', 'ikinci hat')} ile {getattr(profile, 'distraction_region', 'yan alan')} arasındaki ilişki biraz daha netleşirse kompozisyon çok daha etkili görünür."
        )
    if editor_name == "Güler Ataşer":
        return _score_signal(
            scores,
            "duygusal_yogunluk",
            "Bu etkiyi çok zorlamadan korursan kare kendi havasını usulca taşımayı sürdürecektir.",
            "Etki güzel ama bazı yerlerde hava biraz dağılıyor; küçük bir temizlik görüntünün şiirselliğini daha da belirginleştirir.",
            f"Özellikle {getattr(profile, 'distraction_region', 'yan tarafta')} biriken görsel ses hafifçe geri itilirse sahnenin nefesi daha saf duyulur ve kompozisyon daha dengeli görünür."
        )
    if editor_name == "Mürşide Çilengir":
        return _score_signal(
            scores,
...
def main() -> None:
    ...
    header_left, header_right = st.columns([0.22, 0.78])
    with header_left:
        circular_logo_url = get_circular_brand_logo_data_url(size=420, y_offset_ratio=0.10)
        st.markdown(
            f"""
            <div style="display:flex;justify-content:center;align-items:flex-start;padding-top:24px;">
                <img src="{circular_logo_url}" style="width:100%;max-width:260px;aspect-ratio:1/1;border-radius:50%;display:block;object-fit:cover;" />
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            <div class="hero">
                <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                    <h1 style="margin-bottom:0;">ÇOFSAT Fotoğraf Ön Değerlendirme</h1>
                </div>
                <p>
...
def _compose_grounded_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    if editor_name == 'Selahattin Kalaycı':
        ...
        parts = [
            _clean_sentence("Bu karede önce atmosfer hissediliyor; ışık ve tonlar görüntünün havasını usulca kuruyor"),
            detail3,
            _clean_sentence(f"{_lower_first(detail1[:-1])}; bu dokusal yapı kareye daha derin bir etki veriyor"),
            _clean_sentence(f"{profile.secondary_region.capitalize()} tarafındaki ikinci hat, sertleşmeden kompozisyonun ritmini uzatıyor ve görüntüde merak uyandıran bir denge kuruyor"),
            _clean_sentence("Bütün bunlar birlikte sade ama etkili bir kompozisyon oluşturuyor"),
        ]
    elif editor_name == 'Sevgin Cingöz':
        parts = [
            _clean_sentence("Kadrajı ve tonlarıyla dikkat çeken bir kare görüyorum"),
            detail2,
            _clean_sentence(f"{profile.human_action.capitalize()} etkisi ve beden dili, fotoğrafın ifade aktarımını belirgin biçimde güçlendiriyor"),
            _clean_sentence(f"{_lower_first(detail1[:-1])}; bu yüzden kompozisyonun seyir keyfi yüksek ve etkisi doğrudan hissediliyor"),
            _clean_sentence("Güzel bir kompozisyon. Emeğinize sağlık"),
        ]
    elif editor_name == 'Mürşide Çilengir':
        parts = [
            _clean_sentence("Ben burada önce insanın taşıdığı duygunun sessiz ağırlığına bakıyorum"),
            person,
            _clean_sentence(f"{profile.human_action.capitalize()} hissi, fotoğrafa yalnızlıkla yakınlığın aynı anda duyulduğu içten bir katman veriyor"),
            _clean_sentence(f"{_lower_first(detail3[:-1])}; bu yüzden sahne yalnızca görünmüyor, içte kalan bir duyguya da dönüşüyor"),
            _clean_sentence("Duygu aktarımı güçlü bir fotoğraf olmuş"),
        ]
    else:
        parts = [
            _clean_sentence("Bu karede önce mekânın ve ışığın kurduğu görsel cümle dikkat çekiyor"),
            detail1,
            _clean_sentence(f"{_lower_first(detail2[:-1])}; bu nedenle sahne yalnızca estetik değil, şehre ve hayata dair de bir anlatı kuruyor"),
            _clean_sentence(f"{profile.secondary_region.capitalize()} ile {profile.subject_position} çevresinde oluşan ilişki, fotoğrafa çok katmanlı ve yer yer film tadında bir etki veriyor"),
            _clean_sentence("Ortaya zamansız ve estetik bir kompozisyon çıkmış"),
        ]
...
def _deoverlap_editor_comments(comments: Dict[str, str], profile: Optional["SceneProfile"] = None) -> Dict[str, str]:
    ...
    closers = {
        ...
        'Sevgin Cingöz': 'Benim için burada beden dili ve kompozisyon birlikte çalışıyor.',
        'Mürşide Çilengir': 'Bence fotoğrafın duygusal kapısı tam burada açılıyor.',
        'Gülcan Ceylan Çağın': 'Bence bu kare mekânıyla birlikte zamansız bir etki kuruyor.',
    }
    for name in EDITOR_NAMES:
