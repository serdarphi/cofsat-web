...
st.markdown("""
<style>
/* --- Pool minimal cleanup --- */
.candidate-score-row-minimal {
    gap: .24rem !important;
    margin: .22rem 0 .18rem 0 !important;
}
.candidate-score-row-minimal .score-pill {
    font-size: .62rem !important;
    padding: .18rem .42rem !important;
    background: rgba(255,255,255,.045) !important;
    border: 1px solid rgba(255,255,255,.08) !important;
}
.candidate-score-row-minimal .score-pill.strong {
    background: rgba(198,120,67,.18) !important;
    border-color: rgba(198,120,67,.32) !important;
}
.candidate-meta-grid-minimal {
    gap: .24rem !important;
    margin: .1rem 0 .3rem 0 !important;
}
.candidate-meta-grid-minimal div {
    padding: .28rem .2rem !important;
    background: rgba(255,255,255,.025) !important;
}
.candidate-meta-grid-minimal span {
    font-size: .54rem !important;
}
.candidate-meta-grid-minimal strong {
    font-size: .72rem !important;
}
.candidate-rank-label {
    font-size: .70rem !important;
...
def render_pool_editor_score_controls(entry: dict, active_editor_name: str) -> None:
    ...
    st.markdown(
        ...
        <div class='pool-score-head'>
            <div class='pool-score-title'>⭐ {escape(str(active_editor_name))} · puan ver</div>
            <div class='pool-score-current'>{(str(current_value) + " / 100") if current_value else "Puan yok"}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<div class='pool-score-note'>Seçince otomatik kaydeder.</div>",
        unsafe_allow_html=True,
    )
...
def _render_single_photo_pool(context: str, now: datetime, pool_date: str) -> None:
    ...
    for idx, entry in enumerate(entries[:9]):
        ...
        with col:
            ...
            comment_count = sum(1 for v in (entry.get('editor_comments', {}) or {}).values() if str(v or '').strip()) if isinstance(entry.get('editor_comments'), dict) else 0
            st.markdown(
                f"""
                <div class='candidate-score-row candidate-score-row-minimal'>
                    <span class='score-pill strong'>★ {final_score:.1f}</span>
                    <span class='score-pill'>#{idx+1}</span>
                    <span class='score-pill'>💬 {comment_count}</span>
                </div>
                """,
                unsafe_allow_html=True,
            )
            with st.expander("Detay", expanded=False):
                st.markdown(
                    f"""
                    <div class='candidate-meta-grid candidate-meta-grid-minimal'>
                    ...
                    <div class='candidate-time candidate-time-minimal'>⏱️ {candidate_time if candidate_time else 'Saat bilgisi yok'}</div>
                    """,
                    unsafe_allow_html=True,
                )
            ...
            if context == "main" and user_is_admin():
                ...
                if st.button(
                    ...
                    delete_photo_entry(str(entry.get("image_hash", "")), pool_date)
                    st.rerun()
                st.caption("Kartı havuzdan kaldırır.")
    if context == "main" and user_is_admin():
