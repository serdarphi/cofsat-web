...
def render_editor_comments(image_hash: str, result: CritiqueResult, selected_editor_name: str, ai_report: Optional[Dict] = None) -> None:
    ...
    editor_name_to_show = selected_editor_name if selected_editor_name in EDITOR_NAMES else (EDITOR_NAMES[0] if EDITOR_NAMES else "")
    if not editor_name_to_show:
        return

    st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
    st.markdown(f"<div class='section-title'>{editor_name_to_show}</div>", unsafe_allow_html=True)
    st.markdown(
        f"<div class='mini-note' style='white-space: normal; line-height:1.8; font-size:1.02rem;'>{escape(comment_for(editor_name_to_show))}</div>",
        unsafe_allow_html=True,
    )
