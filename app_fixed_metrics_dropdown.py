...
def find_spiral_template_file() -> Optional[str]:
    candidates = [
        "fibonacci-dizisi-ve-fotograf-fibonacci-spiral.png",
        "/mnt/data/fibonacci-dizisi-ve-fotograf-fibonacci-spiral.png",
        "comp.png",
        "/mnt/data/comp.png",
    ]
    for name in candidates:
...
def get_spiral_template_rgba() -> Optional[Image.Image]:
    template_path = find_spiral_template_file()
    if not template_path:
        return None

    tpl = Image.open(template_path).convert("RGBA")
    arr = np.array(tpl, dtype=np.uint8)
    alpha = arr[:, :, 3].astype(np.uint8)

    if alpha.max() < 8:
        gray = np.array(tpl.convert("L"), dtype=np.uint8)
        alpha = np.clip((255 - gray.astype(np.int16) - 24) * 5, 0, 255).astype(np.uint8)
    else:
        alpha = np.clip((alpha.astype(np.float32) ** 0.82) * 1.35, 0, 255).astype(np.uint8)
...
def draw_glow_line(draw: ImageDraw.ImageDraw, coords: Tuple[int, int, int, int], color: Tuple[int, int, int, int], glow: Tuple[int, int, int, int], width: int, glow_width: int) -> None:
    draw.line(coords, fill=glow, width=glow_width)
    draw.line(coords, fill=color, width=width)


def draw_glow_ellipse(draw: ImageDraw.ImageDraw, box: Tuple[int, int, int, int], outline: Tuple[int, int, int, int], glow: Tuple[int, int, int, int], width: int, glow_width: int, fill: Optional[Tuple[int, int, int, int]] = None) -> None:
    draw.ellipse(box, outline=glow, width=glow_width, fill=fill)
...
def draw_phi_grid(img: Image.Image) -> Image.Image:
    ...
    line_color = (120, 220, 255, 255)
    glow_color = (120, 220, 255, 90)

    for x in (x1, x2):
        draw_glow_line(draw, (x, 0, x, h), line_color, glow_color, width=4, glow_width=10)
    for y in (y1, y2):
        draw_glow_line(draw, (0, y, w, y), line_color, glow_color, width=4, glow_width=10)

    for x in (x1, x2):
        for y in (y1, y2):
            r1, r2 = 14, 7
            draw_glow_ellipse(draw, (x-r1, y-r1, x+r1, y+r1), line_color, glow_color, width=3, glow_width=8)
            draw.ellipse((x-r2, y-r2, x+r2, y+r2), fill=(120, 220, 255, 255))

    border = ImageDraw.Draw(overlay)
    border.rounded_rectangle((6, 6, w-6, h-6), radius=14, outline=(120, 220, 255, 80), width=2)
    return Image.alpha_composite(out, overlay).convert("RGB")
...
def draw_golden_diagonals(img: Image.Image) -> Image.Image:
    ...
    primary = (255, 170, 92, 255)
    secondary = (255, 170, 92, 185)
    glow = (255, 170, 92, 82)

    major_lines = [(0, 0, w, h), (w, 0, 0, h)]
    minor_lines = [
        (x1, 0, w, y2),
        (0, y1, x2, h),
        (x2, 0, 0, y2),
    ...
    for line in major_lines:
        draw_glow_line(draw, line, primary, glow, width=4, glow_width=10)
    for line in minor_lines:
        draw_glow_line(draw, line, secondary, glow, width=3, glow_width=8)

    draw.rounded_rectangle((6, 6, w-6, h-6), radius=14, outline=(255, 170, 92, 75), width=2)
    return Image.alpha_composite(out, overlay).convert("RGB")
...
def draw_golden_spiral(img: Image.Image) -> Image.Image:
    ...
    template = get_spiral_template_rgba()
    if template is not None:
        tpl_w, tpl_h = template.size
        tpl_ratio = tpl_w / max(tpl_h, 1)
        canvas_ratio = w / max(h, 1)

        if canvas_ratio >= tpl_ratio:
            target_h = h
            target_w = int(h * tpl_ratio)
        else:
            target_w = w
            target_h = int(w / tpl_ratio)

        offset_x = int((w - target_w) / 2)
        offset_y = int((h - target_h) / 2)

        resized = template.resize((target_w, target_h), Image.Resampling.LANCZOS)
        alpha = np.array(resized.getchannel("A"), dtype=np.uint8)
        alpha_soft = np.clip(alpha.astype(np.float32) * 0.45, 0, 255).astype(np.uint8)
        alpha_main = np.clip(alpha.astype(np.float32) * 1.15, 0, 255).astype(np.uint8)

        glow_rgba = np.zeros((target_h, target_w, 4), dtype=np.uint8)
        glow_rgba[..., 0] = 159
        glow_rgba[..., 1] = 241
        glow_rgba[..., 2] = 103
        glow_rgba[..., 3] = alpha_soft
        glow_img = Image.fromarray(glow_rgba, mode="RGBA").filter(ImageFilter.GaussianBlur(radius=2.2))

        main_rgba = np.zeros((target_h, target_w, 4), dtype=np.uint8)
        main_rgba[..., 0] = 195
        main_rgba[..., 1] = 255
        main_rgba[..., 2] = 135
        main_rgba[..., 3] = alpha_main
        main_img = Image.fromarray(main_rgba, mode="RGBA")

        overlay.alpha_composite(glow_img, dest=(offset_x, offset_y))
        overlay.alpha_composite(main_img, dest=(offset_x, offset_y))

        draw = ImageDraw.Draw(overlay)
        draw.rounded_rectangle((offset_x+4, offset_y+4, offset_x+target_w-4, offset_y+target_h-4), radius=16, outline=(195, 255, 135, 110), width=2)
        return Image.alpha_composite(out, overlay).convert("RGB")
    ...
    ox = (w - target_w) // 2
    oy = (h - target_h) // 2
    draw.rounded_rectangle((ox+4, oy+4, ox+target_w-4, oy+target_h-4), radius=16, outline=(195,255,135,140), width=3)
    return Image.alpha_composite(out, overlay).convert("RGB")


def point_zone(x: int, y: int, w: int, h: int) -> str:
    horizontal = "sol" if x < w / 3 else "sağ" if x > (2 * w / 3) else "merkez"
    vertical = "üst" if y < h / 3 else "alt" if y > (2 * h / 3) else "orta"
    if horizontal == "merkez" and vertical == "orta":
        return "merkez bölge"
    if horizontal == "merkez":
        return f"{vertical} merkez"
    if vertical == "orta":
        return f"{horizontal} orta"
    return f"{vertical} {horizontal}"


def describe_focus_analysis(
    main_points: List[Tuple[int, int]],
    distraction_points: List[Tuple[int, int]],
    w: int,
    h: int,
    ...
    if not main_points:
        return (
            "Odak merkezi net seçilemiyor.",
            "Göz akışı yorumlanamadı.",
            ["Bu karede baskın bir odak noktası daha belirgin kurulursa okuma daha güçlü olur."],
        )

    primary = main_points[0]
    primary_zone = point_zone(primary[0], primary[1], w, h)

    if len(main_points) >= 2:
        dx = main_points[1][0] - main_points[0][0]
        dy = main_points[1][1] - main_points[0][1]
        if abs(dx) > abs(dy):
            flow = "soldan sağa" if dx > 0 else "sağdan sola"
        else:
            flow = "yukarıdan aşağıya" if dy > 0 else "aşağıdan yukarıya"
    else:
        flow = "tek odak üzerinde yoğunlaşıyor"

    summary = (
        f"Ana odak {primary_zone} bölgesinde toplanıyor. "
        f"İlk bakışta göz en hızlı burada tutunuyor."
    )

    flow_text = (
        f"Göz akışı {flow} yönünde ilerliyor. "
        f"Bu da izleyicinin kareyi okuma sırasını belirleyen ana hattı oluşturuyor."
    )

    bullets = [
        f"1. odak noktası: {primary_zone}. Bu alan sahnenin en baskın görsel ağırlığını taşıyor.",
        f"Göz akışı: {flow}. Sarı oklar ana dikkat merkezleri arasındaki muhtemel geçişi gösteriyor.",
        f"Dikkat dağıtan bölge sayısı: {len(distraction_points)}. Kırmızı kutular ana okumayı bölebilecek ikinci ağırlıkları işaretliyor.",
    ]

    if len(main_points) >= 3:
        tertiary_zone = point_zone(main_points[2][0], main_points[2][1], w, h)
        bullets.append(f"3. odak: {tertiary_zone}. Bu bölge ikinci bakışta devreye giren destekleyici katmandır.")

    bullets.append("Yorum mantığı: Sarı alanlar yalnızca parlaklık değil; kontrast, kenar yoğunluğu ve konumsal ağırlık birleşimiyle seçilir.")
    return summary, flow_text, bullets
...
def render_header(logo_file: Optional[str], selected_mode: str, selected_editor_mode: str) -> None:
    logo_src = image_file_to_base64(logo_file) if logo_file else None
    logo_html = f'<img src="{logo_src}" class="header-logo" />' if logo_src else ''
    st.markdown(
        f"""
        <div class="header-row">
            <div class="header-brand">{logo_html}</div>
...
def inject_css() -> None:
    st.markdown(
        ...
        .header-row {display:flex; align-items:center; gap:1rem; margin-bottom:1.1rem;}
        .header-brand {flex:0 0 auto; display:flex; align-items:center; justify-content:center;}
        .header-logo {width:92px; max-width:92px; height:auto; display:block; object-fit:contain; filter: drop-shadow(0 10px 20px rgba(0,0,0,.20));}
        .header-hero {flex:1 1 auto; margin:0;}
...
def main() -> None:
    ...
    render_header(logo_file, selected_mode, selected_editor_mode)
    ...
    components.html(
        """
        <script>
        const anchor = window.parent.document.getElementById("analysis-results");
        if (anchor) {
    ...
    with tab1:
        focus_summary, flow_text, focus_bullets = describe_focus_analysis(
            main_points, distraction_points, image.size[0], image.size[1]
        )
        ...
        with c1:
            ...
            st.caption("Sarı daireler ana odak merkezlerini, kırmızı kutular görsel dikkat kaçağı yaratabilecek bölgeleri, sarı oklar ise gözün muhtemel izleme sırasını gösterir.")
            make_download_button(overlay_img, "Overlay indir", "cofsat_overlay.png", "dl_overlay")
        with c2:
            st.markdown("<div class='panel-title'>Odak açıklaması</div>", unsafe_allow_html=True)
            st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
            st.markdown(f"<div class='section-title'>İlk tutunma</div><div class='mini-note'>{focus_summary}</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='section-title'>Akış yönü</div><div class='mini-note'>{flow_text}</div>", unsafe_allow_html=True)
            st.markdown("<div class='section-title'>Nasıl okunmalı?</div>", unsafe_allow_html=True)
            for bullet in focus_bullets:
                st.markdown(f"<div class='mini-note' style='margin-bottom:.45rem;'>• {bullet}</div>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with c3:
            ...
            st.caption("Turuncu yoğun alanlar görsel enerjinin ve dikkatin toplandığı bölgeleri temsil eder. Overlay ile birlikte okunduğunda hangi alanın baskın, hangisinin destekleyici olduğu daha net anlaşılır.")
            make_download_button(heatmap_img, "Heatmap indir", "cofsat_heatmap.png", "dl_heatmap")
        with c4:
            st.markdown("<div class='panel-title'>Odak katmanını yorumlama rehberi</div>", unsafe_allow_html=True)
            st.markdown(
                """
                <div class='panel-card'>
                    <div class='mini-note'>
                        <strong>Nasıl okumalı?</strong><br><br>
                        1) Önce sarı daireye bak: sistem fotoğrafın en baskın odak merkezini burada görüyor.<br>
                        2) Sonra sarı okları izle: gözün ikinci ve üçüncü durağı neresi, bunu anlamaya çalış.<br>
                        3) Kırmızı kutulara dön: bu alanlar ana hikâyeyi bölüyorsa ton, kadraj veya bekleme kararıyla geri itilebilir.<br><br>
                        <strong>Pratik kullanım:</strong> Eğer sarı akış konu yerine arka plana gidiyorsa karede hiyerarşi zayıflamış demektir.
                    </div>
                </div>
                """,
                unsafe_allow_html=True,
    ...
    rg1, rg2, rg3 = st.columns(3, gap="large")
    with rg1:
        st.markdown("<div class='panel-title'>Altın oran ızgarası</div>", unsafe_allow_html=True)
        st.image(phi_grid_img, use_container_width=True)
        make_download_button(phi_grid_img, "Izgarayı indir", "cofsat_phi_grid.png", "dl_phi_bottom")
    with rg2:
        st.markdown("<div class='panel-title'>Altın diyagonaller</div>", unsafe_allow_html=True)
        st.image(diagonal_img, use_container_width=True)
        make_download_button(diagonal_img, "Diyagonalleri indir", "cofsat_diagonal.png", "dl_diag_bottom")
    with rg3:
        st.markdown("<div class='panel-title'>Altın spiral</div>", unsafe_allow_html=True)
        st.image(spiral_img, use_container_width=True)
        make_download_button(spiral_img, "Spirali indir", "cofsat_spiral.png", "dl_spiral_bottom")

    st.markdown("## Eleştiri ve Okuma")
