...
def create_cofsat_reading_poster_cached(image_bytes: bytes, language: str = "tr") -> Dict:
    """Yüklenen fotoğraftan ChatGPT/GPT Image mantığıyla ÇOFSAT analiz posteri üretir."""
    api_key = get_openai_api_key()
    if not api_key:
        return {"error": "OPENAI_API_KEY bulunamadı. Secrets veya Environment Variables bölümüne OPENAI_API_KEY ekleyin."}

    safe_image = _cofsat_prepare_image_for_image_api(image_bytes)
    lang_line = "Poster dili tamamen Türkçe olsun." if language == "tr" else "Poster language must be English, but the title must remain ÇOFSAT."
    prompt = f"""
Yüklenen fotoğrafı referans alarak ÇOFSAT için premium bir fotoğraf okuma posteri üret.

ZORUNLU KURALLAR:
- Başlık yalnızca: ÇOFSAT
- Başlığın altında başka alt başlık yazma.
- {lang_line}
- Fotoğrafı kesme, uzatma, yamultma veya genişletme. Orijinal kadraj oranını koru.
- Ana fotoğraf poster içinde gerçek oranıyla yerleşsin; gerekirse siyah pano içinde letterbox/padding kullan, ama fotoğraf deforme olmasın.
- Fotoğrafın içindeki insan, yüz, nesne ve mekân detaylarını değiştirme; sadece analiz posteri oluştur.
- Siyah, premium, editorial bir arayüz kullan.
- Renkli overlayler belirgin olsun: mavi negatif alan, turuncu ana özne, mor ikincil özne/detay, yeşil yönlendirici çizgiler, gri çerçeve/sınır.
- Alttaki küçük geometrik okuma kutularında overlay renkleri kalın ve görünür olsun; siyah-beyaz fotoğraf üzerinde kaybolmasın.
- Sağ alttaki yıldızlı puan, “mükemmel portre” vb. değerlendirme alanı ekleme.
- “Öneriler” kutusu ekleme. Sadece ÇOFSAT sonuç olabilir.
- Metinler kısa, okunabilir, sıcak ve samimi ÇOFSAT dilinde olsun.
- Fotoğrafı ahlaki yargılamadan, belgesel ve editoryal bir fotoğraf okuması olarak ele al.

POSTER YAPISI:
Üstte: ÇOFSAT başlığı ve renk legendı.
Ortada: orijinal fotoğraf, renkli overlay ve 3x3 kılavuz.
Sol paneller: Kompozisyon, Denge, Negatif Alan, Ton / Kontrast, Duygu / Anlatım.
Sağ paneller: Odak Noktası, İkincil Özne, Yönlendirici Çizgiler, Çevre / Atmosfer, Işık, ÇOFSAT Sonuç.
Altta: Geometrik Okuma — Üçler Kuralı. Beş küçük kutu: Kesişim Noktası, Diyagonal Yön, Üçgensel Yapı, Görsel Ağırlık, Çerçeveleme.

Amaç: ChatGPT içindeki örnek ÇOFSAT analiz renderlarıyla aynı mantıkta, temiz, öğretici, duygusu olan bir analiz panosu üretmek.
""".strip()

    files = {
        "image": ("cofsat_input.jpg", safe_image, "image/jpeg"),
    }
    data = {
    ...
    try:
        ...
        if response.status_code >= 400:
            # Bazı hesaplarda high kalite/model/size desteklenmeyebilir; daha güvenli ayarla bir kez dene.
            fallback_data = dict(data)
            fallback_data["quality"] = "medium"
            fallback_data["size"] = "1024x1024"
            response = requests.post(
                "https://api.openai.com/v1/images/edits",
                headers={"Authorization": f"Bearer {api_key}"},
                files={"image": ("cofsat_input.jpg", safe_image, "image/jpeg")},
                data=fallback_data,
                timeout=240,
            )
        response.raise_for_status()
        poster_bytes = _extract_openai_image_bytes(response.json())
        poster = Image.open(io.BytesIO(poster_bytes)).convert("RGB")
        out = io.BytesIO()
        poster.save(out, format="PNG", optimize=True)
        return {"image_bytes": out.getvalue(), "model": COFSAT_IMAGE_MODEL}
    except Exception as exc:
        return {"error": str(exc)}
