...
def render_pool_editor_score_controls(entry: dict, active_editor_name: str) -> None:
    ...
    st.markdown("<div class='pool-score-grid'>", unsafe_allow_html=True)
    ...
    for idx, score_value in enumerate(range(10, 101, 10)):
        with score_cols[idx % 4]:
            ...
            if st.button(
                f"{score_value}",
                key=f"pool_editor_score_{image_hash}_{active_editor_name}_{score_value}",
                use_container_width=True,
                type="primary" if is_active else "secondary",
            ):
                update_photo_editor_score(image_hash, active_editor_name, score_value, target_date)
                st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown(
        "<div class='pool-score-note'>Bu fotoğraf havuzdan puanlanır. Sonuç puanı otomatik olarak editör ortalaması %60 + sistem puanı %40 şeklinde güncellenir.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
