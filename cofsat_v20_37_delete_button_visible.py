...
def _render_single_photo_pool(context: str, now: datetime, pool_date: str) -> None:
    ...
    with leader_cols[1]:
        ...
        st.markdown("<div class='pod-subtitle'>Sistem puanı, editör ortalaması ve editör yorumlarının yönü birlikte okunarak lider kare belirlenir.</div>", unsafe_allow_html=True)
        st.markdown(
            f"""
    ...
    st.markdown("<div class='muted-note' style='margin-bottom:.8rem;'>Kartlar, editör yorumlarından çıkan destek seviyesi ve toplam sıralama puanına göre otomatik dizilir.</div>", unsafe_allow_html=True)
    ...
    for idx, entry in enumerate(entries[:9]):
        ...
        with col:
            ...
            st.markdown(f"<div class='candidate-rank-chip'>#{idx+1}</div>", unsafe_allow_html=True)
            if entry.get("thumbnail"):
                st.image(entry["thumbnail"], use_container_width=True)
            ...
            if interactive_pool:
                ...
                if st.button("🗑️ Fotoğrafı Sil", key=f"{context}_delete_{pool_date}_{idx}_{entry.get('image_hash', '')}", use_container_width=True):
                    delete_photo_entry(str(entry.get("image_hash", "")), pool_date)
                    st.rerun()
                st.markdown("</div>", unsafe_allow_html=True)
    if interactive_pool:
...
def inject_css() -> None:
    st.markdown(
        ...
        .flow-step.done {
            ...
            color:#fff3e8;
            border-color: rgba(198,120,67,.35);
        }
        .flow-step.active {
        ...
        .flow-line {
            height:1px;
            width:24px;
            background: linear-gradient(90deg, rgba(198,120,67,.45), rgba(255,255,255,.08));
        }
        .pod-badge {
            display:inline-flex;
            align-items:center;
            ...
            font-weight:800;
            letter-spacing:.01em;
            box-shadow: 0 10px 24px rgba(198,120,67,.20);
            margin-bottom:.55rem;
        }
        .pod-leader-name {
            font-size:1.42rem;
        ...
        .candidate-actions div[data-testid="stButton"] > button {
            background: linear-gradient(135deg, #B56D42, #95512C) !important;
            color: #FFF7F0 !important;
            border: none !important;
        ...
        .candidate-actions div[data-testid="stButton"] > button:hover {
