...
Aşırı kuramsal, sert veya mekanik kelimelerden kaçın. Gülcan Ceylan Çağın sürekli reddeden bir ses olmasın; yapıcı ama seçici kalsın. Editörler ortak kelimelere abanmasın; her biri ayrı söz varlığıyla konuşsun.
John Berger, Susan Sontag ve Roland Barthes çizgilerinden gelen genel eleştiri reflekslerini kullanabilirsin; fakat bu isimleri anma, cümlelerini taklit etme, sadece düşünme biçimlerini dönüştür.
"""
...
- Berger/Sontag/Barthes çizgileri doğrudan isim verilmeden düşünme biçiminde hissedilsin.
- Gerekirse editörler birbiriyle aynı fikirde olmak zorunda değildir.
- Gerekiyorsa belirsizlik belirt ama uydurma yapma.
- Gülcan Ceylan Çağın önce çalışan tarafı teslim etsin, sonra editoryal kararını dengeli biçimde kursun.
- Aynı sıfatları, aynı fiilleri ve aynı kalıpları editörler arasında tekrar etme.
...
def _compose_v8_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    obs1 = observations[0] if len(observations) > 0 else "Ana görsel ağırlık kendini belli ediyor."
    obs2 = observations[1] if len(observations) > 1 else "İkinci bir dikkat hattı kadrajı destekliyor."
    obs3 = observations[2] if len(observations) > 2 else "Işık ve mekân birlikte bir hava kuruyor."

    opening = _voice_pick(editor_name, "openings", f"v8-open-{editor_name}-{obs1}")
    closing = _voice_pick(editor_name, "closings", f"v8-close-{editor_name}-{obs2}")
    focus_sentence = _editor_focus_sentence(editor_name, profile, scores)
    ...
    if editor_name == "Selahattin Kalaycı":
        parts = [
            opening,
            obs1,
            f"{obs2[:-1]} ve bu küçük yer değişimi bana görüntünün kendi iç cümlesini düşündürüyor.",
            focus_sentence,
            issue_sentence,
            closing,
        ]
        ...
        parts = [
            opening,
            obs3,
            f"{obs1[:-1]}; bu yüzden kare sertleşmeden, usulca kendi havasını kuruyor.",
            focus_sentence,
            issue_sentence,
            closing,
        ]
        ...
        parts = [
            opening,
            obs1,
            f"{obs3[:-1]} ve bu durum sahnenin insani çekirdeğini daha duyulur kılıyor.",
            focus_sentence,
            issue_sentence,
            closing,
        ]
        ...
        parts = [
            opening,
            f"Önce hakkını teslim edeyim: {obs1[0].lower() + obs1[1:] if obs1 else 'kare kendi merkezini kuruyor.'}",
            obs2,
            focus_sentence,
            issue_sentence,
            closing,
...
def _deoverlap_editor_comments(comments: Dict[str, str], profile: Optional["SceneProfile"] = None) -> Dict[str, str]:
    ...
    for name in EDITOR_NAMES:
        text = _normalize_phrase(comments.get(name, ""))
        sigs = EDITOR_SIGNATURES.get(name, {}).get("signature_phrases", [])
        for sig in sigs:
        ...
        for w in text.split():
            wl = w.strip('.,;:!?').lower()
            if wl in used and wl in {'fotoğraf','kare','sahne','kompozisyon','görüntü'}:
                continue
            words.append(w)
            if len(wl) > 4:
                used.add(wl)
        out[name] = _trim_sentence(" ".join(words))
    return out
...
def build_ai_reading_report(image_bytes: bytes, mode: str, editor_mode: str, result: CritiqueResult) -> Dict:
    ...
    fallback = {
        ...
        "editor_comments": _deoverlap_editor_comments({name: _compose_v8_editor_comment(name, result) for name in EDITOR_NAMES}, profile),
