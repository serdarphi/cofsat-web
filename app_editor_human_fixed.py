...
def build_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    if editor_name == "Selahattin Kalaycı":
        return (
            f"Bu karede göz önce {bits['primary']} taraftaki {bits['subject']} tarafına gidiyor; iyi olan yer burası. "
            f"Ama {bits['distraction']} bölgesi hemen devreye girip fotoğrafın kararını dağıtıyor. "
            f"{strength} "
            f"Ben olsam uzatmam, tek hamlede şuna yüklenirim: {move}."
        )
    if editor_name == "Güler Ataşer":
        return (
            f"Bu fotoğrafta bana ilk geçen şey {bits['mood']} bir his; özellikle {bits['primary']} taraftaki {bits['subject']} kareyi sıcak tutuyor. "
            f"Orada gerçek bir bağ var, o yüzden fotoğraf insana uzak durmuyor. "
            f"Ama {bits['distraction']} tarafı biraz sakinleşse duygu daha doğrudan geçer. "
            f"Küçük bir dokunuşla çok daha içten bir yere oturur: {move}."
        )
    if editor_name == "Sevgin Cingöz":
        return (
            f"Bu {bits['scene'].lower()} karede ana yapı {bits['primary']} ile {bits['secondary']} arasında kurulmuş ve seçki açısından bakınca niyet okunuyor. "
            f"Güçlü tarafı şu: {strength.lower()} "
            f"Fakat editoryal ağırlığın tam oturması için {bits['distraction']} bölgesindeki yük biraz geri çekilmeli. "
            f"Benim kısa notum şu olur: {move}."
        )
    if editor_name == "Mürşide Çilengir":
        return (
            f"Bu karede {bits['primary']} tarafta duran {bits['subject']}, sanki büyük bir duyguyu sessizce taşıyor. "
            f"{bits['light'].capitalize()} ışık ve {bits['mood']} hava fotoğrafın iç sesini kurmuş. "
            f"Yalnız {bits['distraction']} bölgesi biraz geri çekilse o sessiz hikâye daha derinden duyulur. "
            f"Bence bu fotoğrafı büyütecek tek şey şu: {move}."
        )
    if editor_name == "Gülcan Ceylan Çağın":
        return (
            f"Bu karede kompozisyonu taşıyan şey {bits['primary']} ile {bits['secondary']} arasındaki akış; özellikle {bits['complexity']} yapı bunu görünür kılıyor. "
            f"Gözün yönü tamamen boşa düşmüyor, bu iyi. "
            f"Ama {bits['distraction']} bölgesi biraz fazla ağırlık alınca denge kırılıyor. "
            f"Doğru dokunuş bence burada: {move}."
        )
    return result.editor_summary


def render_editor_snapshot_in_sidebar(selected_editor_name: str, result: CritiqueResult) -> None:
    with st.sidebar:
        st.markdown("<div class='sidebar-card'>", unsafe_allow_html=True)
        st.markdown("**Seçili editörün bu fotoğrafa yorumu**")
        st.markdown(f"<div class='ghost-badge' style='margin-bottom:.75rem;'>{selected_editor_name}</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='mini-note' style='white-space: pre-wrap; line-height: 1.65;'>{build_editor_comment(selected_editor_name, result)}</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
...
def render_editor_comments(result: CritiqueResult, selected_editor_name: str) -> None:
    ...
    st.markdown(f"<div class='ghost-badge' style='margin-bottom:.75rem;'>{selected_editor_name}</div>", unsafe_allow_html=True)
    st.write(build_editor_comment(selected_editor_name, result))
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div class='mini-note' style='margin:.35rem 0 .8rem 0;'>Aşağıdaki sekmelere basarak aynı fotoğraf için tüm editörlerin kısa yorumlarını karşılaştırabilirsiniz.</div>", unsafe_allow_html=True)
    ...
    for tab, name in zip(tabs, EDITOR_NAMES):
        with tab:
            ...
            st.write(build_editor_comment(name, result))
            st.markdown("</div>", unsafe_allow_html=True)
...
def main() -> None:
    ...
    render_editor_snapshot_in_sidebar(selected_editor_name, result)
    ...
    with top_right:
        ...
        with c1:
            render_compact_info_card("Genel Seviye", result.overall_level, result.overall_tag)
        with c2:
            render_compact_info_card("Önerilen Tür", result.suggested_mode, result.suggested_mode_reason)
        with c3:
            render_compact_info_card("Aktif Ton", selected_editor_mode)
        with c4:
            render_compact_info_card("Editör", selected_editor_name)
