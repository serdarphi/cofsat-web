...
def pick_strengths(scores_100: Dict[str, float], editor_mode: str) -> List[str]:
    ...
    mapping = {
        ...
        "sadelik": "Kadraj temiz kalmış; fazla bilgi ana etkiyi boğmuyor.",
...
def build_key_strength(scores_100: Dict[str, float], editor_mode: str) -> str:
    ...
    mapping = {
        ...
        "sadelik": "Bu karenin en güçlü tarafı sadeliği; kadraj gereksiz bilgiyle taşmıyor.",
...
def build_key_issue(scores_100: Dict[str, float], editor_mode: str) -> str:
    ...
    mapping = {
        ...
        "sadelik": "Bu karede en belirgin sorun yan bölgelerdeki bilgi fazlalığının ana etkiyi zayıflatması.",
...
def build_reading_prompts(scores_100: Dict[str, float]) -> List[str]:
    prompts = {
        ...
        "dikkat_dagitici_unsurlar": "Ana öznenin önüne geçen baskın bölgeler var mı?",
...
def build_scene_traits(metrics: ImageMetrics, mode: str) -> Dict[str, bool]:
    return {
        "very_bright": metrics.brightness_mean > 170,
        "very_dark": metrics.brightness_mean < 85,
        "high_contrast": metrics.contrast_std > 68,
        "low_contrast": metrics.contrast_std < 42,
        "busy": metrics.visual_noise_score > 0.60 or metrics.edge_density > 0.16,
        "calm": metrics.visual_noise_score < 0.34 and metrics.edge_density < 0.09,
        "centered": abs(metrics.center_of_mass_x - 0.5) < 0.09 and abs(metrics.center_of_mass_y - 0.5) < 0.09,
        "edge_weighted": metrics.center_of_mass_x < 0.28 or metrics.center_of_mass_x > 0.72 or metrics.center_of_mass_y < 0.28 or metrics.center_of_mass_y > 0.72,
        "symmetrical": metrics.symmetry_score > 0.78,
        "flat": metrics.dynamic_tension_score < 0.42,
        "wide": metrics.aspect_ratio > 1.45,
        "tall": metrics.aspect_ratio < 0.82,
        "mode_portrait": mode == "Portre",
        "mode_street": mode == "Sokak",
        "mode_doc": mode == "Belgesel",
        "mode_abstract": mode == "Soyut",
    }


def build_shooting_notes(metrics: ImageMetrics, scores_100: Dict[str, float], mode: str, editor_mode: str) -> List[str]:
    traits = build_scene_traits(metrics, mode)
    ordered = [k for k, _ in sorted(scores_100.items(), key=lambda x: x[1])]
    notes: List[str] = []

    for key in ordered:
        if key == "odak_ve_hiyerarsi":
            if traits["edge_weighted"]:
                notes.append("Ana ağırlık kadrajın kenarına fazla yaklaşıyor; çekim anında özneyi biraz daha kararlı bir noktaya almak fotoğrafı toplar.")
            elif traits["busy"]:
                notes.append("Özne çevresindeki bilgi yoğunluğu bakışı dağıtıyor; çekimde arka planı biraz daha temizleyen bir açı iyi çalışabilir.")
            else:
                notes.append("Bakışın ilk tutunduğu alan tam netleşmiyor; çekim anında ana özneyi daha belirgin bir yerleşime taşımak iyi olur.")
        elif key == "sadelik":
            if traits["busy"]:
                notes.append("Kadraj gereğinden fazla bilgi taşıyor; bir adım geri ya da yana kayarak sahneyi sadeleştirmek anlatıyı rahatlatabilir.")
            else:
                notes.append("Fotoğrafın ana fikri var; çekimde yan unsurları biraz daha ayıklamak etkiyi güçlendirir.")
        elif key == "kompozisyon":
            if traits["symmetrical"]:
                notes.append("Simetrik yapı hissediliyor; ekseni birkaç santim daha dikkatle oturtmak kompozisyonu çok daha güçlü kılar.")
            elif traits["wide"]:
                notes.append("Yatay kadrajın taşıdığı alan geniş; ana öğeler arasındaki mesafeyi daha bilinçli kurmak yapıyı sıkılaştırır.")
            else:
                notes.append("Çizgiler, boşluklar ve özne yerleşimi tek bir karar altında toparlanırsa kadraj daha ikna edici görünür.")
        elif key == "zamanlama":
            if traits["mode_street"] or traits["mode_doc"]:
                notes.append("Sahnenin en açık jestini, bakışını ya da karşılaşma anını biraz daha sabırla beklemek fotoğrafın gücünü artırabilir.")
            else:
                notes.append("Anın daha açık bir kırılma noktasını kollamak kareye daha belirgin bir vurgu kazandırabilir.")
        elif key == "derinlik_hissi":
            if traits["flat"]:
                notes.append("Kare biraz düz okunuyor; çekimde ön plan ile arka plan arasında daha net bir katman ilişkisi kurmak faydalı olur.")
            else:
                notes.append("Bakışı içeri taşıyacak bir ön plan ya da ara katman eklemek fotoğrafın hacmini büyütebilir.")
        elif key == "isik_yonu":
            if traits["very_bright"]:
                notes.append("Işık oldukça açık çalışıyor; çekim saatini ya da açıyı değiştirerek ana öznenin daha kontrollü ayrıştığı anı yakalamak iyi olabilir.")
            elif traits["very_dark"]:
                notes.append("Karanlık tonlar sahneyi taşıyor; çekimde ışığın özneyi bir kademe daha okunur kıldığı anı beklemek iyi sonuç verebilir.")
            else:
                notes.append("Işığın sahnedeki yönünü biraz daha belirgin kullanmak ana anlatıyı daha güçlü taşır.")
        elif key == "negatif_alan":
            if traits["wide"] or traits["tall"]:
                notes.append("Boş alanın miktarı dikkat çekiyor; onu ana özneyle daha bilinçli ilişkilendirmek kompozisyonu rafine eder.")
            else:
                notes.append("Boşlukları yalnızca boş bırakmak yerine anlatıya hizmet eden bir karar olarak kurmak fotoğrafı büyütür.")
        elif key == "duygusal_yogunluk":
            if traits["mode_portrait"]:
                notes.append("Portrede ifadenin en açık olduğu anı ya da yüzün daha rahat çözüldüğü mesafeyi bulmak duygusal etkiyi yükseltir.")
            else:
                notes.append("Sahnenin duygusunu daha net veren anı seçmek ya da özneyle arandaki mesafeyi biraz değiştirmek etkiyi artırabilir.")
        elif key == "dikkat_dagitici_unsurlar":
            notes.append("Ana öznenin önüne geçen bölgeler var; çekimde açıyı birkaç santim değiştirerek bu baskıyı azaltmak mümkün olabilir.")
        elif key == "ilk_etki":
            notes.append("Fotoğrafın giriş vurgusu biraz daha net kurulursa izleyiciye ilk anda daha güçlü temas eder.")
        if len(notes) >= 4:
            break

    mode_additions = {
        "Portre": "Portrede bakış, yüz açıklığı ve beden dili çoğu teknik ayrıntıdan daha belirleyicidir; çekimde bunu öne alan mesafe kararları işe yarar.",
        "Belgesel": "Belgesel karelerde bağlamı hafifçe genişletmek, fotoğrafın yalnızca anı değil koşulları da anlatmasını sağlayabilir.",
        "Sokak": "Sokak fotoğrafında jest kadar sahne içi ilişkiler de önemlidir; yalnız özneyi değil çevresindeki tepkiyi de kollamak iyi sonuç verir.",
        "Soyut": "Soyut okumada kadraja giren her biçim önem kazanır; çekimde fazlalıkları elemek ve ritmi korumak en büyük farkı yaratır.",
    }
    if mode in mode_additions:
        notes.append(mode_additions[mode])
    ...
    for n in notes:
        ...
        if t not in unique:
            unique.append(t)
    return unique[:5] if unique else ["Bu kare için en güçlü gelişim yolu, çekim kararını biraz daha netleştirmek gibi görünüyor."]
...
def build_editing_notes(metrics: ImageMetrics, scores_100: Dict[str, float], mode: str, editor_mode: str) -> List[str]:
    ...
    if metrics.highlight_clip_ratio > 0.05:
        notes.append("En parlak bölgeler biraz baskınlaşıyor; highlightları geri alıp beyazların dokusunu korumak iyi olur.")
    elif traits["very_bright"]:
        notes.append("Genel parlaklık yüksek; beyazları hafifçe sakinleştirip orta tonları korumak fotoğrafı daha dengeli gösterebilir.")

    if metrics.shadow_clip_ratio > 0.10:
        notes.append("Gölgelerde bilgi kaybı başlıyor; siyahları ezmeden gölge ayrımını biraz açmak ton zenginliğini artırır.")
    elif traits["very_dark"]:
        notes.append("Koyu alanlar fotoğrafın karakteri olabilir; yalnız ana öznenin kaybolduğu bölgelerde seçici aydınlatma faydalı olur.")

    if scores_100["odak_ve_hiyerarsi"] < 60:
        if traits["mode_portrait"]:
            notes.append("Portrede yüz ve göz çevresine hafif lokal parlaklık vermek bakışın daha hızlı toplanmasını sağlar.")
        else:
            notes.append("Ana öznenin bulunduğu bölgede sınırlı bir lokal kontrast artışı, kadraj içindeki hiyerarşiyi belirginleştirebilir.")

    if scores_100["dikkat_dagitici_unsurlar"] < 60:
        notes.append("Yan ve köşe bölgelerdeki parlak ya da sert alanları hafifçe geri itmek ana etkinin önünü açar.")

    if scores_100["duygusal_yogunluk"] < 60 and traits["low_contrast"]:
        notes.append("Fotoğraf biraz yumuşak kalıyorsa orta ton ayrımını hafifçe artırmak duygusal etkiyi görünür hale getirebilir.")
    elif scores_100["duygusal_yogunluk"] < 60:
        notes.append("Ton geçişlerini daha tutarlı kurmak, fotoğrafın his tarafını teknik görünümün önüne taşıyabilir.")

    if scores_100["kompozisyon"] < 60:
        if traits["wide"]:
            notes.append("Yatay kadrajda kenarlarda boş ya da ilgisiz alan kaldıysa hafif bir kırpma kompozisyonu toparlayabilir.")
        elif traits["tall"]:
            notes.append("Dikey kadrajda üst veya alt boşluğu yeniden dengeleyen küçük bir kırpma daha iyi çalışabilir.")
        else:
            notes.append("Çok küçük bir kırpma ile gözün sahneye girişini netleştirmek kadrajı daha sıkı hale getirebilir.")

    if traits["symmetrical"] and scores_100["kompozisyon"] >= 60:
        notes.append("Simetri üzerine kurulu bir sahneyse perspektif doğrulama ve çok hafif merkezleme fotoğrafı bir kademe yukarı taşıyabilir.")

    if not notes:
        notes.append("Bu karede ağır filtre yerine ton, kontrast ve küçük lokal müdahalelerle ilerlemek daha doğru görünüyor.")
...
def build_editor_summary(total: float, strengths: List[str], dev_areas: List[str], mode: str, editor_mode: str) -> str:
    ...
    text = (
        ...
        f"Öne çıkan güçlü yanı şu: {strengths[0].lower()} "
        f"En çok toparlanması gereken yer ise şu görünüyor: {dev_areas[0].lower()} "
        f"Fotoğrafın ana yönü korunarak yapılacak birkaç net müdahale, etkiyi belirgin biçimde yükseltebilir."
    )
    return tone_text(text, editor_mode)
