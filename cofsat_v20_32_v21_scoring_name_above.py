...
def calculate_editor_average(editor_scores: Dict[str, float] | None) -> float:
    if not isinstance(editor_scores, dict):
        return 0.0
    values = []
    for value in editor_scores.values():
        try:
...
def calculate_final_score(system_score: float, editor_average: float) -> float:
    system_score = float(system_score or 0.0)
    editor_average = float(editor_average or 0.0)
    if editor_average <= 0:
        return round(system_score, 2)
    return round((system_score * 0.40) + (editor_average * 0.60), 2)


def enrich_photo_entry_with_scores(entry: dict) -> dict:
    item = dict(entry or {})
    system_score = float(item.get("score", 0.0) or 0.0)
    ...
    for name in EDITOR_NAMES:
        raw = editor_scores.get(name, 0)
        try:
            normalized[name] = int(float(raw or 0))
        except Exception:
            normalized[name] = 0
    editor_average = calculate_editor_average(normalized)
    final_score = calculate_final_score(system_score, editor_average)
    item["editor_scores"] = normalized
    item["editor_average"] = editor_average
    item["final_score"] = final_score
    return item


def update_photo_editor_score(image_hash: str, editor_name: str, score_value: int, selected_date: str | None = None) -> None:
    if editor_name not in EDITOR_NAMES:
        return
    try:
        score_int = int(score_value)
    except Exception:
        return
    score_int = max(10, min(100, score_int))
    ...
    for item in data:
        if item.get("date") == target_date and item.get("image_hash") == image_hash:
            ...
            if not isinstance(editor_scores, dict):
                editor_scores = {}
            editor_scores[editor_name] = score_int
            item["editor_scores"] = editor_scores
            item["editor_average"] = calculate_editor_average(editor_scores)
            item["final_score"] = calculate_final_score(float(item.get("score", 0) or 0), item["editor_average"])
            changed = True
            break
    if changed:
...
def get_today_photo_entries(selected_date: str | None = None) -> list:
    ...
    data = _safe_json_load(PHOTO_HISTORY_FILE)
    todays = [enrich_photo_entry_with_scores(item) for item in data if item.get("date") == target_date]
    return sorted(todays, key=lambda x: (float(x.get("final_score", x.get("score", 0))), x.get("timestamp", "")), reverse=True)
...
def render_photo_of_day_candidates(context: str = "main") -> None:
    ...
    if context == "sidebar":
        ...
        st.markdown(
            f"<div class='mini-note'>Nihai: {float(pod.get('final_score', pod.get('score', 0))):.1f}/100 · Gün içi yükleme: {int(pod.get('entries', 0))}</div>",
            unsafe_allow_html=True,
        )
        if pod.get("ready"):
        ...
        for idx, entry in enumerate(entries[:6]):
            ...
            with cols[1]:
                ...
                st.caption(f"Skor: {float(entry.get('final_score', entry.get('score', 0))):.1f}/100")
    ...
    for idx, entry in enumerate(entries[:9]):
        ...
        with col:
            ...
            st.markdown(f"<div class='candidate-score-badge'>Skor: {float(entry.get('final_score', entry.get('score', 0))):.1f}/100</div>", unsafe_allow_html=True)
            ts = str(entry.get('timestamp', ''))
            if 'T' in ts:
                st.caption(f"Saat: {ts[11:16]}")
...
def render_editor_comments(result: CritiqueResult, selected_editor_name: str, current_image_hash: str, ai_report: Optional[Dict] = None) -> None:
    ai_comments = {}
    if isinstance(ai_report, dict) and isinstance(ai_report.get("editor_comments"), dict):
        ai_comments = ai_report.get("editor_comments") or {}
    ...
    def get_current_entry() -> dict:
        for item in get_today_photo_entries():
            if item.get("image_hash") == current_image_hash:
                return item
        return {}

    def render_score_row(editor_name: str) -> None:
        entry = get_current_entry()
        editor_scores = entry.get("editor_scores", {}) if isinstance(entry, dict) else {}
        if not isinstance(editor_scores, dict):
            editor_scores = {}
        current_value = int(float(editor_scores.get(editor_name, 0) or 0))
        editor_average = float(entry.get("editor_average", 0) or 0)
        final_score = float(entry.get("final_score", getattr(result, "total_score", 0)) or 0)
        system_score = float(getattr(result, "total_score", 0) or 0)

        chips = []
        for name in EDITOR_NAMES:
            raw = int(float(editor_scores.get(name, 0) or 0))
            value_html = f"<span style='color:#b91c1c;font-weight:800;margin-left:6px;'>{raw}</span>" if raw else "<span style='color:#94a3b8;font-weight:700;margin-left:6px;'>—</span>"
            chips.append(
                f"<span style='display:inline-block;padding:5px 9px;border-radius:999px;background:rgba(255,255,255,.92);border:1px solid rgba(198,120,67,.18);font-size:.80rem;font-weight:700;color:#334155;margin:0 6px 6px 0;'>{name}{value_html}</span>"
            )

        st.markdown("<div class='panel-card' style='margin-top:.75rem;padding:14px 16px;'>", unsafe_allow_html=True)
        st.markdown(
            f"""
            <div style='font-weight:800;color:#111827;margin-bottom:.35rem;'>{editor_name} · Editör Puanı</div>
            <div style='font-size:.84rem;color:#6b7280;margin-bottom:.45rem;'>
                Sistem: {system_score:.1f}/100 · Editör ort.: {editor_average:.1f}/100 · Nihai: {final_score:.1f}/100
            </div>
            <div style='font-size:.83rem;font-weight:700;color:#111827;margin-bottom:.3rem;'>Kim ne puan verdi?</div>
            <div style='margin-bottom:.45rem;'>{''.join(chips)}</div>
            """,
            unsafe_allow_html=True,
        )

        st.markdown(
            f"<div style='font-size:.84rem;font-weight:800;color:#111827;margin:.1rem 0 .45rem 0;'>{editor_name}</div>",
            unsafe_allow_html=True,
        )
        cols = st.columns(10, gap="small")
        for i, score_value in enumerate(range(10, 101, 10)):
            with cols[i]:
                if st.button(
                    str(score_value),
                    key=f"editor_score_{current_image_hash}_{editor_name}_{score_value}",
                    use_container_width=True,
                    type="primary" if current_value == score_value else "secondary",
                ):
                    update_photo_editor_score(current_image_hash, editor_name, score_value)
                    st.rerun()
        st.markdown(
            "<div class='mini-note' style='margin-top:.45rem;font-size:.82rem;'>Puan vermeyi unutursanız daha sonra tekrar gelip aynı editörü yeniden puanlayabilirsiniz.</div>",
            unsafe_allow_html=True,
        )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    for tab, name in zip(tabs, ordered_names):
        with tab:
            st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
            st.markdown(f"<div class='section-title' style='margin-bottom:.55rem;'>{name}</div>", unsafe_allow_html=True)
...
def main() -> None:
    ...
    with tab2:
        ...
        render_editor_comments(result, selected_editor_name, current_image_hash, ai_report)
