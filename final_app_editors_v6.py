...
def _critic_sentence(editor_name: str, bits: Dict, profile, result: "CritiqueResult") -> str:
    ...
    verdict_line = "Estetik karar ile tanıklık duygusu aynı çizgide buluşmuş." if editorial >= 68 else "Estetik karar ile tanıklık duygusu henüz aynı çizgide tam buluşmuyor."
    return (
        f"{verdict_line} {detail_1.capitalize()} güçlü bir yayın işareti veriyor ama "
        f"{detail_2} ve {historical_texture_hint} arasındaki seçim daha sert yapılmadığında editoryal eşik bulanıklaşıyor."
    )
