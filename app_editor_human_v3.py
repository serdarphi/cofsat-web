...
def build_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    human_hint = "yüz" if face_count >= 1 else subject
    if score >= 75:
        tension = "Kare büyük ölçüde yerine oturmuş."
    elif score >= 60:
        tension = "İçinde iyi bir damar var; biraz daha toparlanırsa çok daha etkili olur."
    else:
        tension = "Bir şey hissettiriyor ama cümlesi henüz tam kapanmıyor."

    if editor_name == "Selahattin Kalaycı":
        return (
            f"Göz ilk anda {primary} taraftaki {human_hint}e takılıyor; fotoğrafın asıl gücü orada. "
            f"Ama {distraction} tarafı hemen araya girince o net etki biraz dağılıyor. "
            f"{tension} "
            f"Ben olsam bu karede bir şeyi eksiltir, {subject} tarafını daha kararlı bırakırım."
        )

    if editor_name == "Güler Ataşer":
        return (
            f"Bu kare bana önce {mood} bir his veriyor; özellikle {primary} taraftaki {human_hint} insana dokunuyor. "
            f"Orada sıcak ve sahici bir temas var, fotoğrafı ayakta tutan yer biraz da bu. "
            f"Yalnız {distraction} sakinleşse o duygu daha temiz geçecek. "
            f"Küçük bir sadeleşmeyle bu kare çok daha içten olur."
        )

    if editor_name == "Sevgin Cingöz":
        return (
            f"Bu {scene} karede göz, {primary} ile {secondary} arasında rahatça dolaşıyor; bu iyi bir başlangıç. "
            f"{light.capitalize()} ışık da fotoğrafın havasını desteklemiş. "
            f"Ama {distraction} biraz fazla konuşunca ana vurgu geride kalıyor. "
            f"Kadraj bir parça sadeleşirse kare çok daha sağlam durur."
        )

    if editor_name == "Mürşide Çilengir":
        return (
            f"{primary.capitalize()} tarafta duran {human_hint}, sanki içine çektiği bir duyguyu sessizce taşıyor. "
            f"{light.capitalize()} ışık ve {mood} hava bu hissi güzelce büyütmüş. "
            f"Yalnız {distraction} geri çekilse o sessiz hikâye daha derinden duyulur. "
            f"Ben bu karede en çok o kırılgan tarafı hissettim."
        )

    if editor_name == "Gülcan Ceylan Çağın":
        return (
            f"Bu karede kompozisyonu taşıyan şey {primary} ile {secondary} arasındaki akış. "
            f"Özellikle {complexity} yapı ve {light} ışık, derinlik hissini desteklemiş. "
            f"Ama {distraction} biraz ağırlık alınca denge tam yerinde kalmıyor. "
            f"Kadraj küçük bir toparlanmayla çok daha güçlü görünür."
        )
