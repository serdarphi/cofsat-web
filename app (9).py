...
CRITIC_PROFILES = {
    "Selahattin Kalaycı": {
        "tag": "Editoryal • Kültürel hafıza",
        "chips": ["kompozisyon", "perspektif", "bağlam"],
    },
    "Güler Ataşer": {
        "tag": "Duygusal • İlişki odaklı",
        "chips": ["duygu", "bağ", "ışık"],
    },
    "Sevgin Cingöz": {
        "tag": "Kısa • Temiz • Pozitif",
        "chips": ["kompozisyon", "yerleşim", "atmosfer"],
    },
}

VARIATIONS = {
    "etkili": ["etkili", "güçlü", "karşılık bulan", "yerinde"],
    "kompozisyon": ["kompozisyon", "kadraj düzeni", "görsel yapı", "yerleşim"],
    "anlatı": ["anlatı", "hikâye", "duygu hattı"],
}
...
def inject_css() -> None:
    st.markdown(
        ...
        .critic-head {display:flex; align-items:center; justify-content:space-between; gap:.75rem; margin-bottom:.35rem;}
        .critic-name {font-size: 1rem; font-weight: 900; color: var(--text);}
        .critic-tag {font-size: .84rem; color: var(--muted);}
        .critic-chip {
            display:inline-flex; align-items:center; margin-right:.35rem; margin-bottom:.35rem;
            border-radius:999px; padding:.22rem .55rem; font-size:.78rem; font-weight:700;
            background: rgba(248,214,109,.11); color:#ffe7a3; border:1px solid rgba(248,214,109,.16);
        }
        .critic-text {font-size: 1rem; line-height: 1.8; color: #eef2ff; margin-top: .35rem;}
        .sidebar-critic-list .stRadio label {font-weight:700 !important;}
...
def _get_editor_memory() -> Dict[str, List[str]]:
    if "critic_memory" not in st.session_state:
        st.session_state["critic_memory"] = {name: [] for name in CRITIC_PROFILES}
    return st.session_state["critic_memory"]
...
def _pick_variant(base: str, critic_name: str) -> str:
    memory = _get_editor_memory().get(critic_name, [])
    options = VARIATIONS.get(base, [base])
    for opt in options:
        if opt not in memory[-12:]:
            return opt
    return options[0]


def build_critic_reviews(result: CritiqueResult) -> Dict[str, str]:
    sel_komp = _pick_variant("kompozisyon", "Selahattin Kalaycı")
    sel_anl = _pick_variant("anlatı", "Selahattin Kalaycı")
    gul_etk = _pick_variant("etkili", "Güler Ataşer")
    sev_etk = _pick_variant("etkili", "Sevgin Cingöz")

    reviews = {
        "Selahattin Kalaycı": (
            f"{result.structural_reading} {result.one_move_improvement} "
            f"Bu kare, {sel_komp} tarafında karar aldıkça {sel_anl} gücünü daha kalıcı hale getirebilir."
        ),
        "Güler Ataşer": (
            f"{result.first_reading} {result.key_strength} "
            f"Bu yüzden fotoğraf hem duygusal hem de görsel olarak {gul_etk} bir bütünlük kuruyor."
        ),
        "Sevgin Cingöz": (
            f"{result.key_strength} {result.one_move_improvement} "
            f"Genel etki {sev_etk}. Emeğinize sağlık."
        ),
    }

    memory = _get_editor_memory()
    for name, text in reviews.items():
        tokens = [w.strip('.,:;!?').lower() for w in text.split() if len(w) > 4]
        memory.setdefault(name, []).extend(tokens[:10])
        memory[name] = memory[name][-60:]
    st.session_state["critic_memory"] = memory
    return reviews


def render_critic_box(name: str, text: str) -> None:
    meta = CRITIC_PROFILES[name]
    chips = ''.join([f"<span class='critic-chip'>{c}</span>" for c in meta['chips']])
    st.markdown("<div class='critic-box'>", unsafe_allow_html=True)
    st.markdown(f"<div class='critic-head'><div><div class='critic-name'>✍️ {name}</div><div class='critic-tag'>{meta['tag']}</div></div></div>", unsafe_allow_html=True)
    st.markdown(chips, unsafe_allow_html=True)
    st.markdown(f"<div class='critic-text'>{text}</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)


def render_sidebar(selected_mode: str, selected_editor_mode: str, selected_critic: str, show_all_critics: bool) -> None:
    with st.sidebar:
        ...
        st.markdown("<hr style='border:none;height:1px;background:rgba(83,41,9,.14);margin:.9rem 0 .8rem 0'>", unsafe_allow_html=True)
        st.markdown("### ÇOFSAT Editörleri")
        st.markdown("<div class='mini-note'>Kavuniçi panelden editörü seç. Ana sayfa bozulmadan, eleştiri kartı aşağıda açılır.</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='ghost-badge'>{selected_critic}</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='mini-note'>Tüm kartlar: {'Açık' if show_all_critics else 'Kapalı'}</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.markdown("<div class='sidebar-critic-list'>", unsafe_allow_html=True)
        selected_critic = st.radio("Editör", ["Hepsi", *CRITIC_PROFILES.keys()], index=0)
        show_all_critics = st.checkbox("Tüm editör kartlarını göster", value=False)
        if st.button("Editör hafızasını sıfırla"):
            reset_editor_memory()
        st.markdown("</div>", unsafe_allow_html=True)

    render_sidebar(selected_mode, selected_editor_mode, selected_critic, show_all_critics)
    ...
    with st.spinner("Fotoğraf okunuyor, metrikler çıkarılıyor ve görsel katmanlar hazırlanıyor..."):
        ...
        critic_reviews = build_critic_reviews(result)
    ...
    with tab2:
        ...
        st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
        st.markdown("<div class='section-title'>ÇOFSAT Editörleri</div>", unsafe_allow_html=True)
        if show_all_critics or selected_critic == "Hepsi":
            for critic_name, critic_text in critic_reviews.items():
                with st.expander(critic_name, expanded=False):
                    render_critic_box(critic_name, critic_text)
        else:
            with st.expander(selected_critic, expanded=True):
                render_critic_box(selected_critic, critic_reviews[selected_critic])
        st.markdown("</div>", unsafe_allow_html=True)
