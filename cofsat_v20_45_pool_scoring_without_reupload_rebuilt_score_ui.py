...
def render_pool_editor_score_controls(entry: dict, active_editor_name: str) -> None:
    ...
    st.markdown(
        ...
        <div class='pool-score-head'>
            <div class='pool-score-title'>⭐ {escape(str(active_editor_name))} puanı</div>
            <div class='pool-score-current'>{(str(current_value) + " / 100") if current_value else "Henüz puan verilmedi"}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )

    score_cols = st.columns([1.25, 0.9], gap="small")
    with score_cols[0]:
        st.markdown("<div class='pool-score-inline-label'>Puan seç</div>", unsafe_allow_html=True)
    ...
    with score_cols[1]:
        st.markdown("<div class='pool-score-inline-label'>Kaydet</div>", unsafe_allow_html=True)
        st.markdown("<div class='pool-save-button'>", unsafe_allow_html=True)
        if st.button(
            "Kaydet",
            key=f"pool_score_save_{image_hash}_{active_editor_name}",
    ...
    st.markdown(
        "<div class='pool-score-note'>Fotoğrafı yeniden yüklemeden havuzdan puan verebilirsiniz. Sonuç puanı editör ortalaması %60 + sistem puanı %40 olarak güncellenir.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
        ...
        with col:
            ...
            st.markdown(
                ...
                <div class='candidate-score-row'>
                    <span class='score-pill strong'>Sıralama {rank_score:.1f}</span>
