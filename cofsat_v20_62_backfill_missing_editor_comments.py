...
def _render_single_photo_pool(context: str, now: datetime, pool_date: str) -> None:
    ...
    if context == "sidebar":
        ...
        if is_future_pool:
            st.info(
                f"Saat {PHOTO_OF_DAY_HOUR:02d}:00'dan sonra yüklenen fotoğraflar burada toplanır. "
                f"Saat {PHOTO_POOL_CLEANUP_HOUR:02d}:00'de bugünün havuzu temizlenir; bu panel ertesi gün adaylarını görünür tutar."
            )
        elif pod.get("ready"):
            st.success(f"Türkiye saatine göre {PHOTO_OF_DAY_HOUR:02d}:00 sonrası en yüksek sıralama puanlı fotoğraf günün fotoğrafı oldu.")
        else:
    ...
    if is_future_pool:
        ...
        status_text = (
            f"Bu panelde saat {PHOTO_OF_DAY_HOUR:02d}:00'dan sonra yüklenen fotoğraflar tutulur. "
            f"Editör yorumu ve nihai puan birlikte işlenir; seçim yarın {PHOTO_OF_DAY_HOUR:02d}:00 sonrası görünürleşir."
        )
    elif pod.get("ready"):
        process_state = "selected"
        status_text = (
            f"Türkiye saatine göre {PHOTO_OF_DAY_HOUR:02d}:00 sonrası en yüksek sıralama puanlı fotoğraf otomatik olarak günün fotoğrafı oldu. "
        ...
        status_text = (
            ...
            f"{PHOTO_OF_DAY_HOUR:02d}:00 sonrası lider kare günün fotoğrafı olur."
        )
...
def main() -> None:
    ...
    if hero_pod:
        ...
        hero_micro_html = (
            f"<div class='cofsat-hero-micro'>Türkiye saati {PHOTO_OF_DAY_HOUR:02d}:00 sonrası en yüksek sıralama puanlı fotoğraf üst vitrinde görünür.</div>"
            if hero_ready else
            f"<div class='cofsat-hero-micro'>Saat {PHOTO_OF_DAY_HOUR:02d}:00 sonrası seçim kesinleştiğinde günün fotoğrafı burada görünür.</div>"
        )
    else:
