...
def normalize_uploaded_image(image_bytes: bytes, max_size: int = MAX_ANALYSIS_SIZE) -> Image.Image:
    img = Image.open(io.BytesIO(image_bytes))
    img = ImageOps.exif_transpose(img).convert("RGB")
    img = safe_resize(img, max_size=max_size)
    normalized = Image.new("RGB", img.size)
    normalized.paste(img)
    return normalized
...
def extract_metrics_cached(image_bytes: bytes) -> Dict:
    img = normalize_uploaded_image(image_bytes)
...
def get_resized_rgb(image_bytes: bytes) -> Image.Image:
    return normalize_uploaded_image(image_bytes)
...
def main() -> None:
    ...
    components.html(
        ...
        const doScroll = () => {
            const el = mainDoc.getElementById("analysis-results-anchor");
            if (el) {
    ...
    st.markdown("<div class='section-title' style='margin-top:1.25rem;'>Aşağıda görünür özet</div>", unsafe_allow_html=True)
