...
EDITOR_PERSONAS = {
    "Selahattin Kalaycı": {"voice": "net ve yapıcı"},
    "Güler Ataşer": {"voice": "sıcak ve insancıl"},
    "Sevgin Cingöz": {"voice": "rafine ve editoryal"},
    "Mürşide Çilengir": {"voice": "duygusal ve şiirsel"},
    "Gülcan Ceylan Çağın": {"voice": "kompozisyon ve perspektif odaklı"},
}
...
def inject_css() -> None:
    st.markdown(
        ...
        .editor-name-badge {
            ...
            background: rgba(248,214,109,.12);
            color: var(--text);
            font-size:.9rem;
            font-weight:700;
        }
        ...
        .stat-grid-card .label {
            color: var(--muted);
            font-size: .82rem;
            margin-bottom:.45rem;
        }
        .stat-grid-card .value {
            color: var(--text);
            font-size: 1.05rem;
            font-weight: 800;
            line-height: 1.35;
...
def _scene_profile_from_result(result: CritiqueResult) -> Dict[str, str]:
    metrics = result.metrics if isinstance(result.metrics, dict) else {}
    profile = metrics.get("scene_profile", {}) if isinstance(metrics, dict) else {}
    return profile if isinstance(profile, dict) else {}


def _build_comment_context(result: CritiqueResult) -> Dict[str, str]:
    profile = _scene_profile_from_result(result)
    metrics = result.metrics if isinstance(result.metrics, dict) else {}
    face_count = int(profile.get("face_count", 0) or 0)
    subject = profile.get("subject_hint", "ana vurgu")
    mood = profile.get("visual_mood", "dengeli")
    primary = profile.get("primary_region", "merkez")
    secondary = profile.get("secondary_region", "ikinci alan")
    ...
    if face_count >= 1:
        subject_phrase = f"{primary} tarafta toplanan yüz ve ifade"
    elif scene_type == "Sokak":
        subject_phrase = f"{primary} bölgede ağırlık kazanan ana figür"
    elif scene_type == "Soyut":
        subject_phrase = f"{primary} çevresinde yoğunlaşan biçim ve boşluk ilişkisi"
    else:
        subject_phrase = f"{primary} bölgede toplanan ana vurgu"

    if complexity == "yoğun":
        scene_sentence = f"Kadroda özellikle {primary} ile {secondary} arasında yoğun bir akış var."
    elif complexity == "sade":
        scene_sentence = f"Kare gereksiz yük taşımadan daha sade bir yüzey kuruyor; bu da {subject} tarafını belirginleştiriyor."
    else:
        scene_sentence = f"Kare, {primary} çevresinde açılıp {secondary} tarafa doğru dağılan dengeli bir akış kuruyor."

    if metrics.get("edge_density", 0) > 0.16:
        distraction_sentence = f"Yalnız {distraction} tarafındaki hareket ve detay, zaman zaman ana odağın önüne geçiyor."
    else:
        distraction_sentence = f"{distraction.capitalize()} tarafı bütünü bozacak kadar güçlü değil ama biraz daha geri itildiğinde fotoğraf rahatlar."
    ...
    return {
        ...
        "distraction_sentence": distraction_sentence,
    }
...
def build_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ctx = _build_comment_context(result)
    subject = ctx["subject"]
    mood = ctx["mood"]
    primary = ctx["primary"]
    secondary = ctx["secondary"]
    distraction = ctx["distraction"]
    light = ctx["light"]
    subject_phrase = ctx["subject_phrase"]
    scene_sentence = ctx["scene_sentence"]
    distraction_sentence = ctx["distraction_sentence"]

    if editor_name == "Selahattin Kalaycı":
        return _join_comment_lines(
            f"İlk bakışta gözü alan yer net: {subject_phrase}. {scene_sentence}",
            f"Ama fotoğrafın kırıldığı yer de orasıyla bağlantılı. {result.key_issue} {distraction_sentence}",
            f"Yine de boş bir kare değil. {result.key_strength}",
            f"Ben burada tek bir şeyi netleştirirdim: {result.one_move_improvement}",
        )
    if editor_name == "Güler Ataşer":
        return _join_comment_lines(
            f"Bu karede insana değen bir şey var. Özellikle {subject_phrase}, fotoğrafla aramızda küçük ama gerçek bir bağ kuruyor.",
            f"Kare {mood} bir hava taşıyor; ışığın {light} yapısı da bu hissi nazikçe büyütüyor.",
            f"Ama tam bu yüzden fotoğraf biraz daha açıklık istiyor. {result.key_issue}",
            f"Biraz daha nefes açılırsa bu duygu izleyiciye daha doğrudan geçer.",
        )
    if editor_name == "Sevgin Cingöz":
        return _join_comment_lines(
            f"Seçki gözüyle bakınca bu fotoğrafın tutunduğu yer {subject_phrase}. {scene_sentence}",
            f"Yapısal tarafta çalışan şey belli: {result.key_strength}",
            f"Ama editoryal ağırlığı büyütmek için kararın biraz daha keskinleşmesi gerekiyor. {result.key_issue}",
            f"Özellikle {primary} ile {secondary} arasındaki ilişki netleştikçe kare çok daha sağlam bir yerde durur.",
        )
    if editor_name == "Mürşide Çilengir":
        return _join_comment_lines(
            f"Bu kare, küçük bir bedenin ya da küçük görünen bir anın içinde büyük bir duygu saklıyor gibi. {subject_phrase.capitalize()} sessizce konuşuyor.",
            f"Işığın {light} karakteri ve karede dolaşan {mood} hava, o iç sesi daha da görünür kılıyor.",
            f"Yine de fotoğraf tam kalbe oturacağı yerde biraz dağılıyor. {result.key_issue}",
            f"Bir parça daha arındığında, bu görüntü susarak çok daha fazla şey söyleyebilir.",
        )
    if editor_name == "Gülcan Ceylan Çağın":
        return _join_comment_lines(
            f"Bu karede ilk gözüme gelen şey kompozisyonun {primary} çevresinde kurduğu ağırlık merkezi. {scene_sentence}",
            f"Perspektif ve yerleşim duygusu tamamen dağılmıyor; hatta {result.key_strength.lower()}",
            f"Ama {distraction} tarafındaki enerji biraz fazla baskın kalınca denge tam temiz görünmüyor.",
            f"Ben bu fotoğrafı bir adım ileri taşımak için şuraya dokunurdum: {result.one_move_improvement}",
        )
    return result.editor_summary


def render_editor_comment_panel(selected_editor_name: str, result: CritiqueResult) -> None:
    comment = build_editor_comment(selected_editor_name, result)
    st.markdown(
        f"""
        <div class="panel-card">
            <div class="section-title">Aktif editör yorumu</div>
            <div class="editor-name-badge">{selected_editor_name}</div>
            <div class="editor-comment-text">{comment}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_all_editor_tabs(result: CritiqueResult, selected_editor_name: str) -> None:
    ...
    for tab, name in zip(tabs, EDITOR_PERSONAS.keys()):
        with tab:
            if name == selected_editor_name:
                st.markdown("<div class='mini-note'>Aktif seçim</div>", unsafe_allow_html=True)
            st.markdown(
                f"<div class='panel-card'><div class='editor-title'>{name}</div><div class='editor-comment-text'>{build_editor_comment(name, result)}</div></div>",
                unsafe_allow_html=True,
            )
...
def main() -> None:
    ...
    if uploaded_file is None:
        ...
        with st.sidebar:
            st.markdown("<div class='sidebar-editor-comment'><div class='section-title'>Editör yorumu</div><div class='mini-note'>Fotoğraf yüklendiğinde seçtiğiniz editörün bu kareye özel yorumu burada görünecek.</div></div>", unsafe_allow_html=True)
        return
    ...
    with st.sidebar:
        st.markdown(
            f"<div class='sidebar-editor-comment'><div class='section-title'>Editör yorumu</div><div class='editor-title'>{selected_editor_name}</div><div class='mini-note'>{build_editor_comment(selected_editor_name, result)}</div></div>",
            unsafe_allow_html=True,
        )
    ...
    with top_right:
        ...
        st.markdown(
            ...
            <div class="stat-grid-4">
                <div class="stat-grid-card"><div class="label">Genel seviye</div><div class="value">{result.overall_level}</div><div class="mini-note">{result.overall_tag}</div></div>
                <div class="stat-grid-card"><div class="label">Önerilen tür</div><div class="value">{result.suggested_mode}</div></div>
                <div class="stat-grid-card"><div class="label">Aktif ton</div><div class="value">{selected_editor_mode}</div></div>
                <div class="stat-grid-card"><div class="label">Aktif editör</div><div class="value">{selected_editor_name}</div></div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    render_editor_comment_panel(selected_editor_name, result)
    render_all_editor_tabs(result, selected_editor_name)
