...
def build_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    mood = bits["mood"]
    light = bits["light"]
    scene = bits["scene"].lower()
    complexity = bits["complexity"]

    human_hint = "insan yüzü" if face_count >= 1 else subject
    if score >= 75:
        tension = "Fotoğrafın niyeti büyük ölçüde yerine oturmuş."
    elif score >= 60:
        tension = "İyi bir damar var ama tam oraya yerleşmesi için biraz daha netlik istiyor."
    else:
        tension = "Bir şey söylüyor ama henüz tam kararını vermiş gibi durmuyor."

    if editor_name == "Selahattin Kalaycı":
        return (
            f"Göz önce {primary} taraftaki {human_hint} tarafına gidiyor; fotoğrafın asıl yükü orada. "
            f"Sorun şu ki {distraction} bölgesi hemen devreye girip o ilk etkiyi dağıtıyor. "
            f"{tension} "
            f"Ben burada lafı uzatmam; {subject} daha net tutulursa kare kendini çok daha temiz anlatır."
        )

    if editor_name == "Güler Ataşer":
        return (
            f"Bu kare bende önce {mood} bir his bırakıyor; özellikle {primary} taraftaki {human_hint} fotoğrafı insana yaklaştırıyor. "
            f"Orada sıcak bir temas var, o yüzden görüntü sadece bir kayıt gibi durmuyor. "
            f"Ama {distraction} biraz sakinleşse bu bağ daha doğrudan geçecek. "
            f"Küçük bir sadeleşmeyle fotoğraf çok daha içten bir yere oturur."
        )

    if editor_name == "Sevgin Cingöz":
        return (
            f"Bu {scene} karede yapı {primary} ile {secondary} arasında kuruluyor ve ilk bakışta niyet okunuyor. "
            f"{light.capitalize()} ışıkla gelen atmosfer seçki açısından avantajlı bir zemin hazırlamış. "
            f"Fakat {distraction} bölgesindeki görsel yük biraz fazla kalınca editoryal ağırlık tam kapanmıyor. "
            f"Ana vurgu sadeleşirse bu kare seçki içinde daha güvenli durur."
        )

    if editor_name == "Mürşide Çilengir":
        return (
            f"{primary.capitalize()} tarafta duran {human_hint}, sanki büyük bir duyguyu içine çekip susarak taşıyor. "
            f"{light.capitalize()} ışık ve {mood} hava fotoğrafın iç sesini güzel kurmuş. "
            f"Yalnız {distraction} biraz geri çekilse o sessiz hikâye daha derinden duyulacak. "
            f"Ben bu kareyi en çok o kırılgan tarafıyla sevdim."
        )

    if editor_name == "Gülcan Ceylan Çağın":
        return (
            f"Bu karede kompozisyonu taşıyan şey {primary} ile {secondary} arasındaki akış; göz boşa düşmeden kadrajın içinde dolaşıyor. "
            f"Özellikle {complexity} yapı ve {light} ışık düzeni derinliği desteklemiş. "
            f"Ama {distraction} biraz ağırlık alınca denge tam kararında kalmıyor. "
            f"Kadraj küçük bir toparlanmayla çok daha güçlü bir kompozisyona dönüşür."
        )
...
def main() -> None:
    ...
    with top_right:
        ...
        for tab, name in zip(quick_tabs, EDITOR_NAMES):
            with tab:
                st.markdown(f"<div class='mini-note' style='white-space: normal; line-height:1.68; font-size:.98rem;'>{escape(build_editor_comment(name, result))}</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
