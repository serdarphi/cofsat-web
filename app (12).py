...
def render_editor_visibility_block(selected_mode: str, selected_editor_mode: str, selected_character_level: int, result: CritiqueResult) -> None:
    ...
    with p1:
        st.markdown(
            f"<div class='summary-card'><div class='editor-title'>Aktif editör</div><div class='mini-note'><strong>{selected_mode}</strong> personası aktif. {persona_text}</div></div>",
            unsafe_allow_html=True,
        )
    with p2:
        st.markdown(
            f"<div class='summary-card'><div class='editor-title'>Ton</div><div class='mini-note'><strong>{selected_editor_mode}</strong> tonunda yazıyor. {EDITOR_MODES[selected_editor_mode]['ending']}</div></div>",
            unsafe_allow_html=True,
        )
    with p3:
        st.markdown(
            f"<div class='summary-card'><div class='editor-title'>Karakter seviyesi</div><div class='mini-note'><strong>{character_meta['label']}</strong> — {character_meta['note']}</div></div>",
            unsafe_allow_html=True,
        )
...
def render_sidebar(selected_mode: str, selected_editor_mode: str, selected_character_level: int) -> None:
    character_meta = get_editor_character_meta(selected_character_level)
    with st.sidebar:
...
def main() -> None:
    ...
    render_sidebar(selected_mode, selected_editor_mode, selected_character_level)
    ...
    render_editor_visibility_block(selected_mode, selected_editor_mode, selected_character_level, result)
