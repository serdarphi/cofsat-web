...
def build_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    def subject_word() -> str:
        ...
        if face_count >= 1:
            return "figür"
        if "figür" in lowered or "insan" in lowered:
            return "figür"
        if "boşluk" in lowered:
            return "boşluk"
        if "biçim" in lowered or "form" in lowered:
            return "biçim"
        if "ışık" in lowered:
            return "ışık"
        return "ana vurgu"
    ...
    def subject_group() -> str:
        lowered = subject.lower()
        if face_count >= 1:
            return "figür ile çevresi arasındaki ilişki"
        if "figür" in lowered or "insan" in lowered:
            return "figür ile çevresi arasındaki ilişki"
        if "boşluk" in lowered:
            return "boşluk ile ana vurgu arasındaki ilişki"
        if "biçim" in lowered or "form" in lowered:
            return "biçim ile ritim arasındaki ilişki"
        if "ışık" in lowered:
            return "ışık ile gölge arasındaki ilişki"
        return "ana ağırlık"
    ...
    def opening_subject_detail() -> str:
        ...
        if face_count >= 1:
            return pick([
                f"{p.capitalize()} beliren figür, fotoğrafın duygusunu hemen görünür kılıyor.",
                f"Bu kareyi taşıyan ilk şey, {p} tarafta toplanan insan ağırlığı oluyor.",
                f"İlk anda {p} taraftaki figür, bütün sahnenin duygusal merkezine dönüşüyor.",
            ], "detail-face")
        if "figür" in lowered or "insan" in lowered:
            return pick([
                f"{p.capitalize()} duran figür, kadrajın yönünü ve duygusunu birlikte belirliyor.",
        ...
        if "boşluk" in lowered:
            return pick([
                f"{p.capitalize()} kurulan boşluk duygusu, fotoğrafın asıl etkisini sessizce büyütüyor.",
                f"Fotoğrafın en dikkat çekici yanı, {p} tarafında büyüyen boşluk hissi oluyor.",
                f"İlk anda {p} hattında açılan boşluk, görüntünün tonunu belirliyor.",
            ], "detail-space")
        if "biçim" in lowered or "form" in lowered:
            return pick([
                f"{p.capitalize()} kurulan biçim ilişkisi, görüntünün omurgasını ayakta tutuyor.",
                f"Fotoğrafın en güçlü yanı, {p} tarafında toplanan biçim duygusu.",
                f"İlk anda {p} hattındaki form ilişkisi, sahnenin düzenini veriyor.",
            ], "detail-form")
        if "ışık" in lowered:
            return pick([
                f"{p.capitalize()} tarafta biriken ışık, kadrajın asıl cümlesini kuruyor.",
                f"Fotoğraf en çok {p} toplanan aydınlık üzerinden açılıyor.",
                f"İlk anda {p} taraftaki ışık vurgusu görüntünün ağırlığını belirliyor.",
            ], "detail-light")
        return pick([
            f"Bu karede ilk ağırlık {p} tarafında toplanıyor.",
            f"Fotoğraf kapısını en çok {p} tarafındaki ana vurgu üzerinden açıyor.",
            f"İlk bakışta {p} tarafında duran ana vurgu öne çıkıyor.",
        ], "detail-default")
    ...
    if editor_name == "Selahattin Kalaycı":
        sentences = [
            pick([
                ...
                _build_scene_relation_phrase(result, bits).capitalize() + ".",
            ], "sel-open"),
            pick([
                flow_sentence(),
                light_sentence(),
