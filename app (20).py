...
def build_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    if editor_name == "Selahattin Kalaycı":
        return (
            f"Bu karede ilk çalışan şey {result.key_strength.lower()} "
            f"Ama asıl kırılma noktası {result.key_issue.lower()} "
            f"Göz özellikle {primary} tarafında tutunuyor; {distraction} tarafı biraz geri itilirse fotoğraf daha temiz konuşur. "
            f"Benim tek hamle önerim şu: {result.one_move_improvement}"
        )
    if editor_name == "Güler Ataşer":
        return (
            f"Bu fotoğrafın içinde hemen hissedilen bir taraf var; {subject} üzerinden izleyiciyle küçük bir bağ kuruyor. "
            f"Kare {mood} bir hava taşıyor ve bu güzel. "
            f"Yalnız {result.key_issue.lower()} Fotoğraf biraz daha nefes alırsa duygusu daha rahat geçecek."
        )
    if editor_name == "Sevgin Cingöz":
        return (
            f"Fotoğrafa seçki gözüyle bakınca yapısal olarak en çok öne çıkan taraf {result.key_strength.lower()} "
            f"Ancak editoryal ağırlığı artırmak için {result.key_issue.lower()} "
            f"{primary.capitalize()} bölgesindeki karar netleştikçe kare daha sağlam durur."
        )
    if editor_name == "Mürşide Çilengir":
        return (
            f"Bu karede küçük görünen şeyin içinde büyük bir duygu saklı. "
            f"{subject.capitalize()} sanki sessizce konuşuyor; ışığın {light} karakteri ve {mood} hava bu hissi büyütüyor. "
            f"Yine de {result.key_issue.lower()} Biraz daha arınırsa fotoğraf çok daha derinden dokunur."
        )
    if editor_name == "Gülcan Ceylan Çağın":
        return (
            f"Kompozisyon tarafında ilk dikkat çeken şey {primary} ile kurulan ağırlık dengesi. "
            f"{result.key_strength.capitalize()} Perspektif ve yerleşim duygusu çalışıyor. "
            f"Ama {distraction} bölgesinin etkisi azaltılırsa yapısal denge daha temiz görünür. "
            f"Bu kareyi bir adım ileri taşıyacak yer bence tam olarak şu: {result.one_move_improvement}"
        )
