...
def qwen_vision_critique_cached(image_bytes: bytes, mode: str, editor_mode: str) -> Dict:
    ...
    prompt = f"""
Sen ÇOFSAT için çalışan, sıcak ve insancıl fotoğraf eleştirisi yazan görsel yorum motorusun.

Bu fotoğrafı ÇOFSAT mantığıyla oku.
Yorumlarda:
- fotoğraftan somut ayrıntılar anılsın,
- teknik kadar anlam, duygu, ilişki ve kadraj içi gerilim konuşulsun,
- editörler birbirine benzemesin,
- her editör 4-5 cümle kursun,
- genellemelerden kaçın.
...
Fotoğrafı yalnızca teknik olarak değil, anlam, ilişki, hikâye, kadraj içi gerilim, ışığın sahneye etkisi ve duygusal yankı üzerinden oku.
Yorumlar sıcak, insani, somut ve fotoğrafın gerçekten okunduğunu ispatlayacak kadar ayrıntılı olmalı.
Asla genel geçer laf üretme. Asla iki editöre aynı giriş cümlesini verme.
Her editör yorumu 4 veya 5 cümle olsun.
Her editör yorumunda:
1) fotoğraftan en az bir somut ayrıntı anılsın,
2) teknik dışı anlam kurulsun,
3) bir kırılma noktası söylensin,
4) yargılayıcı değil yapıcı ve insancıl bir öneri verilsin.
...
def build_editor_comment_legacy(editor_name: str, result: CritiqueResult) -> str:
    ...
    scene = bits["scene"]
    detail_1 = bits.get("detail_1", "")
    detail_2 = bits.get("detail_2", "")
    detail_3 = bits.get("detail_3", "")
    ...
    def pick(options: List[str], salt: str) -> str:
        return _pick_by_seed(
            options,
            f"{editor_name}-{salt}-{subject}-{primary}-{secondary}-{distraction}-{mood}-{light}-{complexity}-{detail_signature}-{score:.1f}",
        )
    ...
    def relation_sentence() -> str:
        return capdot(_build_scene_relation_phrase(result, bits))
    ...
    def issue_sentence() -> str:
        ...
        if hierarchy < 58:
            return pick([
                "Ana etkinin nerede durduğu belli ama orada biraz daha kararlı kalması gerekiyor.",
                f"Göz önce {place(primary)} tarafına gidiyor ama orada daha net bir ağırlık istiyor.",
                "Bakışı tutan merkez var; yalnız o merkezin biraz daha sağlam kurulması iyi olur.",
            ], "issue-hier")
        if simplicity < 58:
            return pick([
                f"{place(distraction).capitalize()} tarafındaki görsel ses biraz çekilirse fotoğraf daha rahat konuşur.",
                "Yan alanlardaki fazlalık azaldığında asıl duygu daha temiz duyulur.",
                "Çevresel gürültü hafiflediğinde kare çok daha açık bir cümle kurar.",
            ], "issue-simple")
        if narrative < 60:
            return pick([
                "Fotoğraf bir şey hissettiriyor ama bunu biraz daha açık söyleyebilir.",
                "Duygu var; sadece daha görünür bir karar onu büyütebilir.",
                "Kare çalışıyor, ama akılda daha uzun kalması için cümlesi biraz daha netleşmeli.",
            ], "issue-narr")
        return pick([
            "Büyük bir sorun yok; daha çok küçük bir toparlama ihtiyacı var.",
            "Temel his iyi kurulmuş, yalnız son bir netlik fotoğrafı büyütür.",
            "Kare zaten iyi bir yerde; yalnız son küçük kararlar onu daha kalıcı yapar.",
        ], "issue-ok")

    def close_sentence() -> str:
        return pick([
            "Küçük bir düzeltmeyle bu kare çok daha uzun süre akılda kalır.",
            "Bence çekirdek güçlü; birkaç küçük karar onu çok daha görünür yapar.",
            "Fotoğrafın özü var, gerisi daha çok temiz bir toparlama meselesi.",
        ], "close")
    ...
    if editor_name == "Selahattin Kalaycı":
        sentences = [
            pick([
                capdot(detail_1),
                f"Ben bu karede önce {detail_1} tarafına bakıyorum.",
                f"Fotoğraf ilk cümlesini net biçimde {detail_1} üzerinden kuruyor.",
            ], "sel-open"),
            pick([
                capdot(detail_2),
                relation_sentence(),
    ...
    if editor_name == "Güler Ataşer":
        sentences = [
            pick([
                f"Beni içeri alan ilk şey, {detail_1}.",
                f"Bu karede önce teknik değil, {detail_1} hissi geliyor bana.",
                f"İlk anda fotoğrafın sıcak tarafı {detail_1} üzerinden açılıyor.",
            ], "gul-open"),
            pick([
                f"{detail_2.capitalize()}; bu da görüntüyü daha insani kılıyor.",
                f"{detail_3.capitalize()}; bence karenin duygusu biraz da buradan geliyor.",
                relation_sentence(),
            ], "gul-observe"),
            pick([
                issue_sentence(),
                "Sadece bakışı dağıtan küçük yükler azalırsa duygu daha doğrudan geçer.",
            ], "gul-issue"),
            pick([
                "İçinde gerçek bir temas var; onu biraz daha görünür bırakmak yeterli.",
                close_sentence(),
            ], "gul-close"),
        ]
        return " ".join(sentences[:4])
    ...
    if editor_name == "Sevgin Cingöz":
        sentences = [
            pick([
                f"Yapı açısından bakınca ilk güçlü veri şu: {detail_2}.",
                f"Bu kareyi ayakta tutan şeylerden biri {detail_1}.",
                f"Ben burada önce yerleşime bakıyorum; {detail_2} oldukça belirleyici.",
            ], "sev-open"),
            pick([
                relation_sentence(),
                f"{detail_3.capitalize()}; bu da akışın tek noktada sıkışmasını engelliyor.",
            ], "sev-observe"),
            pick([
                issue_sentence(),
                f"Özellikle {place(distraction)} tarafındaki ağırlık biraz toparlanırsa yapı çok daha berrak olur.",
            ], "sev-issue"),
            pick([
                "Temel kurgu sağlam; bu yüzden küçük müdahale büyük fark yaratır.",
                close_sentence(),
            ], "sev-close"),
        ]
        return " ".join(sentences[:4])
    ...
    if editor_name == "Mürşide Çilengir":
        sentences = [
            pick([
                f"Bu kare bende önce şu duyguyu bırakıyor: {detail_1}.",
                f"İlk anda gözümden çok içimde kalan şey {detail_3} oluyor.",
                f"Fotoğrafın sesi yüksek değil; ama {detail_1} kolay kolay geçmiyor.",
            ], "mur-open"),
            pick([
                f"{detail_2.capitalize()}; bu yüzden görüntü yalnızca görünmüyor, hissediliyor da.",
                relation_sentence(),
                f"{detail_3.capitalize()}; sanki fotoğrafın susarak söylediği şey biraz burada saklı.",
            ], "mur-observe"),
            pick([
                issue_sentence(),
                "Biraz daha açıklık kazanınca bu sessiz etki çok daha derine iner.",
            ], "mur-issue"),
            pick([
    ...
    if editor_name == "Gülcan Ceylan Çağın":
        sentences = [
            pick([
                f"Kompozisyon açısından en dikkat çekici nokta {detail_2}.",
                f"Bu karede bakışı hareket ettiren ilk şey {detail_3}.",
                f"Ben burada en çok {detail_1} üzerinden kurulan yerleşimi seviyorum.",
            ], "gulcan-open"),
            pick([
                relation_sentence(),
                f"{detail_2.capitalize()}; bu da kompozisyonun yön duygusunu güçlendiriyor.",
            ], "gulcan-observe"),
            pick([
                issue_sentence(),
                f"Eğer {place(distraction)} tarafındaki yoğunluk hafiflerse ana akış çok daha temiz okunur.",
            ], "gulcan-issue"),
            pick([
                "Yerleşim zaten güçlü; sadece fazlalık azaldığında etkisi daha da büyür.",
                close_sentence(),
            ], "gulcan-close"),
        ]
        return " ".join(sentences[:4])
...
def build_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    return build_editor_comment_legacy(editor_name, result)
...
def build_ai_reading_report(image_bytes: bytes, mode: str, editor_mode: str, result: CritiqueResult) -> Dict:
    ...
    fallback = {
        "scene_summary": (
            ...
            f"Göz en çok {profile.primary_region} bölgesinde tutunuyor; genel hava ise {profile.visual_mood}. "
            f"Işığın karakteri {profile.light_character} ve kadrajın yoğunluğu {profile.complexity_level} görünüyor."
        ),
        "concrete_details": [
            f"Ana görsel ağırlık {profile.primary_region} bölgesinde toplanıyor.",
            ...
            f"Fotoğrafın genel havası {profile.visual_mood} bir çizgide ilerliyor.",
            f"Işık yapısı {profile.light_character} karakter gösteriyor.",
        ],
        "meaning_layers": [
            f"Fotoğrafın ana cümlesi {profile.subject_hint} etrafında kuruluyor.",
            f"Görüntünün duygusu {profile.visual_mood} bir etki bırakıyor.",
            "Kadrajın içindeki ilişki, ana vurgu ile çevredeki alan arasındaki denge üzerinden okunuyor.",
        ],
...
def render_vision_debug_panel(report: Dict) -> None:
    debug = report.get("_debug", {}) if isinstance(report, dict) else {}
    api_key_present = "Var" if debug.get("api_key_present") else "Yok"
    status = str(debug.get("status") or "bilinmiyor")
    success = bool(debug.get("success"))
    model = str(debug.get("model") or OPENAI_VISION_MODEL)
    error = str(debug.get("error") or "").strip()

    status_label = "Çalışıyor" if success else "Çalışmıyor"
    st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
    st.markdown("<div class='section-title'>Sistem Durumu</div>", unsafe_allow_html=True)

    c1, c2, c3 = st.columns(3, gap="small")
    with c1:
        render_compact_info_card("API Key", api_key_present)
    with c2:
        render_compact_info_card("Durum", status_label, status)
    with c3:
        render_compact_info_card("Model", model)

    if success:
        st.success("Vision AI çağrısı başarılı. Şu an ekrandaki yorumlar Vision katmanından geliyor.")
    else:
        st.error("Vision AI şu anda aktif çalışmıyor. Ekranda yerel özet görünüyor; editör yorumları Vision dönene kadar beklemede kalır.")
        if error:
            st.markdown(f"<div class='mini-note' style='white-space:normal; line-height:1.7;'>{escape(error)}</div>", unsafe_allow_html=True)
...
def render_ai_reading_report(report: Dict) -> None:
    ...
    source_label = "Vision AI okuması" if source == "vision" else "Yerel okuma özeti"
...
def main() -> None:
    ...
    with top_right:
        ...
        st.info((ai_report.get('global_summary') if isinstance(ai_report, dict) and ai_report.get('global_summary') else 'Vision AI özeti bekleniyor.'))
        render_pill_row(result.tags)
        st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
    st.markdown("<div class='section-title'>Editörler bu kareye ne diyor?</div>", unsafe_allow_html=True)
    st.markdown("<div class='mini-note' style='margin-bottom:.7rem;'>Bu alan yalnızca Vision AI dönen yorumları gösterir.</div>", unsafe_allow_html=True)
    _report_editor_comments = ai_report.get("editor_comments", {}) if isinstance(ai_report, dict) else {}
    _vision_success = bool(ai_report.get("_debug", {}).get("success")) if isinstance(ai_report, dict) else False
    ...
    for tab, name in zip(quick_tabs, EDITOR_NAMES):
        with tab:
            ...
            if _vision_success and isinstance(_quick_comment, str) and _quick_comment.strip():
                st.markdown(f"<div class='mini-note' style='white-space: normal; line-height:1.72; font-size:1rem;'>{escape(_quick_comment)}</div>", unsafe_allow_html=True)
            else:
                st.markdown("<div class='mini-note' style='white-space: normal; line-height:1.72; font-size:1rem;'>Vision AI yanıtı gelmeden bu bölüm içerik göstermez.</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
def critique_image(image_bytes: bytes, mode: str, editor_mode: str) -> CritiqueResult:
    ...
    metrics_payload = {**asdict(metrics), "scene_profile": asdict(profile)}
    if vision_analysis:
