...
CHARACTER_LEVELS = {
    1: {"label": "Yumuşak", "note": "Nazik, cesaret kırmayan, daha kapsayıcı editör yaklaşımı."},
    2: {"label": "Dengeli", "note": "Yapıcı ama daha net; güçlü ve zayıf tarafları eşit ağırlıkla söyler."},
    3: {"label": "Net", "note": "Editoryal dili belirginleştirir, lafı dolaştırmadan söyler."},
    4: {"label": "Keskin", "note": "Gelişim alanlarını daha baskın ve daha açık biçimde söyler."},
    5: {"label": "Acımasız Netlik", "note": "Sert ama hâlâ fotoğrafı ileri taşımaya odaklı editör tavrı."},
}

EDITOR_PERSONAS = {
    "Selahattin Kalaycı": {
        "role": "Sahne editörü",
        "focus": "Sahnedeki ana gerilim, ritim, kadraj içi akış ve fotoğrafın niyeti.",
        "sidebar_comment": "Bu editör fotoğrafın önce neden var olduğuna, sonra sahnenin bunu gerçekten taşıyıp taşımadığına bakar.",
        "result_comment": "Önce fotoğrafın niyetini yoklar; ardından sahne, akış ve görsel ağırlık merkezinin bu niyeti destekleyip desteklemediğini okur.",
    },
    "Güler Ataşer": {
        "role": "Duygu ve ilişki editörü",
        "focus": "İfade, bakış, psikolojik temas, özne-arka plan ilişkisi ve duygusal geçirgenlik.",
        "sidebar_comment": "Bu editör karedeki insanî temasın ne kadar sahici geçtiğine ve duygunun ne kadar görünür olduğuna odaklanır.",
        "result_comment": "Öznenin duygusal okunurluğunu, yüzeyde kalan etki ile gerçekten içeri geçen etki arasındaki farkı gözeterek yorumlar.",
    },
    "Sevgin Cingöz": {
        "role": "Editöryel denge editörü",
        "focus": "Seçkiye girme potansiyeli, sadelik, editöryel ağırlık, ton disiplini ve tekrar bakma isteği.",
        "sidebar_comment": "Bu editör fotoğrafın sadece güzel olup olmadığına değil, seçki içinde kendine yer açıp açamayacağına bakar.",
        "result_comment": "Kareyi editöryel ağırlık, seçki değeri, tonal disiplin ve fazlalıkları ayıklama becerisi üzerinden değerlendirir.",
    },
}
...
def inject_css() -> None:
    st.markdown(
        ...
        .editor-card, .aktif-editor-card {border-radius: 16px; padding: .7rem .85rem; margin: .35rem 0; border: 1px solid rgba(255,255,255,.14);}
        .editor-card {background: rgba(255,255,255,.05);}
        .aktif-editor-card {background: rgba(255,179,71,.18); border-color: rgba(255,179,71,.55); box-shadow: 0 0 0 1px rgba(255,179,71,.08) inset;}
        .rubric-row {
            margin-bottom: .75rem;
        }
        .rubric-head {
...
def render_editor_visibility_block(selected_mode: str, selected_editor_mode: str, selected_character_level: int, selected_editor_name: str, result: CritiqueResult) -> None:
    character_meta = get_editor_character_meta(selected_character_level)
    persona_text = {
        "Sokak": "Sahne, ritim, an ve katman ilişkisine odaklanır.",
        "Portre": "İfade, bakış, psikolojik temas ve özne çevresine odaklanır.",
        "Belgesel": "Bağlam, tanıklık, dürüstlük ve editöryel ağırlığa odaklanır.",
        "Soyut": "Biçim, boşluk, ritim, yüzey ve tonal organizasyona odaklanır.",
    }.get(selected_mode, "Fotoğrafın ana görsel mantığını okumaya odaklanır.")

    editor_meta = EDITOR_PERSONAS.get(selected_editor_name, {})
    p1, p2, p3 = st.columns(3, gap="large")
    with p1:
        st.markdown(
            f"<div class='summary-card'><div class='editor-title'>Aktif editör</div><div class='mini-note'><strong>{selected_editor_name}</strong><br>{editor_meta.get('role', 'Editör')} · {editor_meta.get('result_comment', '')}</div></div>",
            unsafe_allow_html=True,
        )
    with p2:
        st.markdown(
            f"<div class='summary-card'><div class='editor-title'>Fotoğraf okuma ekseni</div><div class='mini-note'><strong>{selected_mode}</strong> personası aktif. {persona_text} {editor_meta.get('focus', '')}</div></div>",
            unsafe_allow_html=True,
        )
    with p3:
        st.markdown(
            f"<div class='summary-card'><div class='editor-title'>Ton ve karakter</div><div class='mini-note'><strong>{selected_editor_mode}</strong> · <strong>{character_meta['label']}</strong><br>{character_meta['note']}</div></div>",
            unsafe_allow_html=True,
        )

    st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
    st.markdown("<div class='section-title'>Editör yorumu · görünür özet</div>", unsafe_allow_html=True)
    st.markdown(f"**Editör özeti:** {result.editor_summary}")
    st.markdown(f"**İlk okuma:** {result.first_reading}")
    st.markdown(f"**Yapısal okuma:** {result.structural_reading}")
    st.markdown(f"**Editöryel sonuç:** {result.editorial_result}")
    st.markdown("</div>", unsafe_allow_html=True)


def render_sidebar(selected_mode: str, selected_editor_mode: str, selected_character_level: int, selected_editor_name: str) -> None:
    character_meta = get_editor_character_meta(selected_character_level)
    with st.sidebar:
        ...
        for editor_name, meta in EDITOR_PERSONAS.items():
            active = "aktif-editor-card" if editor_name == selected_editor_name else "editor-card"
            st.markdown(
                f"<div class='{active}'><strong>{editor_name}</strong><br><span class='mini-note'>{meta['role']}</span></div>",
                unsafe_allow_html=True,
            )
            with st.expander(f"{editor_name} yorumu", expanded=editor_name == selected_editor_name):
                st.markdown(f"**Bakış:** {meta['focus']}")
                st.markdown(meta['sidebar_comment'])
        ...
        st.markdown(
            "- Fotoğrafı yükle\n- Tür, ton ve karakter seviyesini seç\n- Heatmap, göz akışı ve altın oran katmanlarını incele\n- Editör okuması, çekim ve düzenleme notlarını uygula"
        )
        st.markdown("</div>", unsafe_allow_html=True)
...
def main() -> None:
    ...
    render_sidebar(selected_mode, selected_editor_mode, selected_character_level, selected_editor_name)
    ...
    st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
    st.markdown("<div class='section-title'>Ana editör özeti</div>", unsafe_allow_html=True)
    st.write(result.editor_summary)
    st.markdown(f"<div class='mini-note'><strong>En güçlü taraf:</strong> {result.key_strength}</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='mini-note'><strong>En kritik sorun:</strong> {result.key_issue}</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='mini-note'><strong>Tek hamle öneri:</strong> {result.one_move_improvement}</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

    render_editor_visibility_block(selected_mode, selected_editor_mode, selected_character_level, selected_editor_name, result)
    ...
    with tab2:
        st.markdown("<div class='section-title' style='margin-bottom:.6rem;'>Editör Okuması</div>", unsafe_allow_html=True)
