...
def _cofsat_default_reading() -> Dict:
    return {
        "composition": "Kadraj, ana figür ve çevre arasındaki ilişkiyi sade ama güçlü bir merkez etrafında topluyor. Göz önce insana, sonra mekânın sessiz ayrıntılarına gidiyor.",
        "balance": "Görsel ağırlık ana özne ile çevredeki boşluklar arasında kurulmuş. Bu denge kareye hem açıklık hem de içe dönük bir sakinlik veriyor.",
        "negative_space": "Boş alanlar konuyu yalnızlaştırmıyor; aksine fotoğrafın nefes aldığı yeri kuruyor ve ana duyguyu güçlendiriyor.",
        "tone_contrast": "Siyah-beyaz tonlar, ışık ve gölge arasındaki ayrımı belirginleştiriyor. Doku, form ve atmosfer aynı anda okunabiliyor.",
        "story": "Bu karede asıl etki, gündelik bir anın sessizce derinleşmesinden geliyor. Fotoğraf bağırmadan, kendi iç sesiyle konuşuyor.",
        "focus": "Odak, ana öznenin duruşu ve sahne içindeki konumu üzerinden kuruluyor. Bakış doğal biçimde bu merkeze yerleşiyor.",
        "secondary": "İkincil detaylar hikâyeyi büyütüyor; fakat ana öznenin önüne geçmeden sahneye bağlam katıyor.",
        "leading": "Çizgiler ve yönler gözün kadraj içinde dolaşmasını sağlıyor. Bu akış fotoğrafın ritmini daha okunur hale getiriyor.",
        "environment": "Mekân, yalnızca arka plan değil; fotoğrafın duygusunu taşıyan sessiz bir yüzey gibi çalışıyor.",
        "light": "Işık, ana bölgeyi ayırırken gölgeleri tamamen yok etmiyor. Bu sayede fotoğraf hem sert hem de insani kalıyor.",
        "result": "Bu kare, kompozisyon, atmosfer ve görsel akış açısından etkili bir okuma alanı kuruyor. Hikâye sade ama iz bırakacak kadar güçlü.",
...
def _cofsat_openai_reading_json(image_bytes: bytes, language: str = "tr") -> Dict:
    ...
    data_url = safe_openai_data_url(image_bytes)
    system_prompt = """
Sen ÇOFSAT fotoğraf okuma sistemi için çalışan bir görsel okuma motorusun.
Görevin poster çizmek değil; fotoğrafı okuyup kısa, düzgün Türkçe paneller ve yaklaşık overlay koordinatları üretmek.
Sadece geçerli JSON döndür. Uydurma kelime üretme. Metinler samimi, net ve ÇOFSAT dilinde olsun.
"""
    user_prompt = """
Bu fotoğraf için ÇOFSAT analiz panosu metinlerini hazırla.
Metinlerin tamamı TÜRKÇE olsun. Her alan 1-2 kısa cümle olsun.
...
def create_cofsat_reading_poster(image_bytes: bytes, language: str = "tr") -> Dict:
    ...
    thumb_texts = [
        "Ana dikkat noktası üçler kuralının doğal alanına yakın duruyor.",
        "Diyagonal akış gözü sahnenin içinden geçirerek özneye taşıyor.",
        "Özne, detay ve çevre arasında sakin bir üçgen yapı kuruluyor.",
        "Göz önce merkeze, sonra ikincil detaya ve atmosfere dolaşıyor.",
        "Kadrajın iç sınırları konuyu toparlayıp etkiyi güçlendiriyor.",
    ]
