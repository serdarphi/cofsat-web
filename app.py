...
def main() -> None:
    ...
    components.html(
        ...
        const scrollToResults = () => {
            ...
            const candidates = Array.from(doc.querySelectorAll('div, section')).filter(el =>
                (el.innerText || '').includes('Genel sonuç') || (el.innerText || '').includes('Ana güçlü taraf')
            );
            if (candidates.length) {
                candidates[0].scrollIntoView({behavior: 'smooth', block: 'start'});
                return;
            }
        ...
        setTimeout(scrollToResults, 1100);
        </script>
        """,
        height=0,
    )
    ...
    st.markdown("<div class='section-title' style='margin-top:1.25rem;'>Aşağıda görünür eleştiri özeti</div>", unsafe_allow_html=True)

    visible_c1, visible_c2 = st.columns(2, gap="large")
    with visible_c1:
        render_bullets("Güçlü yönler", result.strengths, "✅")
        render_bullets("Gelişim alanları", result.development_areas, "⚠️")
        render_bullets("Çekim notları", result.shooting_notes, "📷")
    with visible_c2:
        render_bullets("Düzenleme notları", result.editing_notes, "🎛️")
        render_bullets("Okuma soruları", result.reading_prompts, "•")
        st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
        st.markdown("<div class='section-title'>Altın oran önerisi</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='scheme-name'>{best_scheme}</div>", unsafe_allow_html=True)
        st.write(scheme_reason)
        st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
    st.markdown("<div class='section-title'>Editör okuması</div>", unsafe_allow_html=True)
    ...
    visible_left, visible_right = st.columns(2, gap="large")
    with visible_left:
        render_rubric_scores(result.rubric_scores)
    with visible_right:
        render_bullets("Çekim notları", result.shooting_notes, "📷")
