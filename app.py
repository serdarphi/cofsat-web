...
def draw_golden_spiral(img: Image.Image) -> Image.Image:
    ...
    # Görsel ne kadar farklı oranda olursa olsun önce içine en büyük altın dikdörtgeni oturt.
    if w / max(h, 1) >= PHI:
        rect_h = h
        rect_w = int(h * PHI)
    else:
    ...
    direction = 0  # 0:left, 1:bottom, 2:right, 3:top
    squares: List[Tuple[int, int, int, int, int]] = []
    ...
    for _ in range(12):
        ...
        if direction == 0:  # soldaki kare
            square = (x, y, x + side, y + side, direction)
            x = x + side
            rw = rw - side
        elif direction == 1:  # alttaki kare
            square = (x, y + rh - side, x + side, y + rh, direction)
            rh = rh - side
        elif direction == 2:  # sağdaki kare
            square = (x + rw - side, y, x + rw, y + side, direction)
            rw = rw - side
        else:  # üstteki kare
            square = (x, y, x + side, y + side, direction)
            y = y + side
    ...
    # Altın dikdörtgen çerçevesi ve kare bölünmeleri
    draw.rounded_rectangle(
        (offset_x, offset_y, offset_x + rect_w, offset_y + rect_h),
        outline=(159, 241, 103, 150),
        width=2,
        radius=10,
    )

    arc_map = {
        0: (180, 270),  # soldaki kareden sağa açılan akış
        1: (270, 360),  # alttaki kareden yukarı açılan akış
        2: (0, 90),     # sağdaki kareden sola açılan akış
        3: (90, 180),   # üstteki kareden aşağı açılan akış
    }

    for i, (x1, y1, x2, y2, phase) in enumerate(squares):
        draw.rounded_rectangle((x1, y1, x2, y2), outline=(159, 241, 103, 80), width=1, radius=4)
        start, end = arc_map[phase]
        draw.arc((x1, y1, x2, y2), start=start, end=end, fill=(159, 241, 103, 245), width=4)

        # spiral merkezi kuvvetli görülsün diye ilk kareye küçük bir vurgu
        if i == 0:
            cx = (x1 + x2) // 2
            cy = (y1 + y2) // 2
            draw.ellipse((cx - 6, cy - 6, cx + 6, cy + 6), fill=(159, 241, 103, 220))
