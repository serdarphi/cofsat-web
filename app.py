...
def _build_object_relation_phrase(result: CritiqueResult, bits: Dict[str, str]) -> str:
    ...
    if "at" in subject:
        relations.extend([
            f"{p(primary)} duran at ile {p(secondary)} açılan boşluk arasında güçlü bir gerilim var",
            f"atın ağırlığı ile {p(secondary)} tarafında kalan alan birbirini iyi tamamlıyor",
            f"atın gövdesi ile çevredeki açıklık arasında temiz bir görsel ilişki kurulmuş",
        ])

    if face_count >= 1:
        relations.extend([
            f"{p(primary)} görünen yüz ile çevredeki alan arasında hemen hissedilen bir insan mesafesi oluşuyor",
            f"yüzün topladığı dikkat ile arka plandaki hareket arasında belirgin bir karşıtlık var",
            f"yüz ile geride kalan çevre birbirine yaslanıyor; biri duygu veriyor, diğeri bağlam",
        ])

    if scene == "Portre":
        relations.extend([
            f"figür ile arkasında kalan tonlar birbirini itmeden taşıyor",
            f"özne ile arka plan arasındaki ayrım duyguyu daha rahat duyuruyor",
        ])

    if scene == "Sokak":
        relations.extend([
            f"ana figür ile çevrede akan hayat arasında canlı bir temas kurulmuş",
            f"ön taraftaki vurgu ile arkadaki hareket birbirini besliyor",
        ])

    if scene == "Belgesel":
        relations.extend([
            f"insan unsuru ile sahnenin bağlamı aynı cümlede buluşuyor",
            f"özne ile çevresi birbirinden kopmadan aynı hikâyeyi taşıyor",
        ])

    if complexity == "yoğun":
        relations.extend([
            f"ana unsur ile {p(distraction)} tarafındaki kalabalık arasında sürekli bir çekişme var",
            f"öndeki vurgu ile çevredeki görsel ses yan yana duruyor; bu da fotoğrafı diri tutuyor",
        ])
    elif complexity == "sade":
        relations.extend([
            f"ana unsur ile çevrede bırakılan boşluk arasında rahat bir nefes alanı açılmış",
            f"özne ile çevresindeki sakin alan birbirini boğmadan destekliyor",
        ])

    if "sert" in light:
        relations.extend([
            f"ışığın sertliği, ana unsur ile arka plan arasındaki ayrımı daha görünür kılıyor",
            f"parlaklık ile koyuluk arasındaki karşıtlık, sahnedeki ilişkiyi daha açık hale getiriyor",
        ])
    else:
        relations.extend([
            f"yumuşak ışık, ana unsur ile çevresindeki tonları aynı duyguda topluyor",
            f"ışığın sakin akışı, öndeki vurgu ile geride kalan alanı birbirine bağlıyor",
        ])

    if not relations:
        relations = [
            f"{p(primary)} taraftaki ana vurgu ile {p(secondary)} hattı arasında okunur bir ilişki kurulmuş",
            f"fotoğrafın ana unsuru ile çevresi birbirine karşı çalışmak yerine birbirini taşıyor",
        ]

    seed = f"objrel-{subject}-{primary}-{secondary}-{distraction}-{scene}-{complexity}-{light}-{face_count}"
...
def build_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    def pick(options: List[str], salt: str) -> str:
        return _pick_by_seed(
            options,
            f"{editor_name}-{salt}-{subject}-{primary}-{secondary}-{distraction}-{mood}-{light}-{complexity}-{score:.1f}",
        )

    def place(region: str) -> str:
        return _region_to_place(region)

    def subject_word() -> str:
        ...
        if "insan" in lowered:
            return "insan"
        if "boşluk" in lowered:
            return "boşluk"
        if "biçim" in lowered:
            return "biçim"
        return "ana unsur"

    def subject_group() -> str:
        lowered = subject.lower()
        if "at" in lowered:
            return "ata bakan ana kütle"
        if face_count >= 1:
            return "yüz ve çevresindeki ağırlık"
        if "figür" in lowered:
            return "figür ve etrafındaki boşluk"
        if "insan" in lowered:
            return "insan varlığı ve çevresi"
        if "boşluk" in lowered or "biçim" in lowered:
            return "biçim ve boşluk ilişkisi"
        return "ana vurgu"

    def opening_subject_detail() -> str:
        lowered = subject.lower()
        p = place(primary)
        if "at" in lowered:
            return pick([
                f"{p.capitalize()} duran at, kadrajın en güçlü cümlesini tek başına kuruyor.",
                f"Bu karede {p} duran at hemen dikkat topluyor; görüntü onun etrafında şekilleniyor.",
                f"İlk bakışta {p} taraftaki at bütün sahnenin yükünü omuzluyor.",
            ], "detail-at")
        if face_count >= 1:
            return pick([
                f"{p.capitalize()} görünen yüz, fotoğrafın insani tarafını hemen açıyor.",
                f"Bu kareyi taşıyan ilk şey {p} tarafta beliren yüz oluyor.",
                f"İlk anda {p} taraftaki yüz, bütün görüntüyü duygusal olarak topluyor.",
            ], "detail-face")
        if "figür" in lowered:
            return pick([
                f"{p.capitalize()} yer alan figür, kadrajın yönünü ve ağırlığını belirliyor.",
                f"Fotoğraf önce {p} taraftaki figür üzerinden nefes almaya başlıyor.",
                f"Bu karede {p} duran figür, görüntünün bütün ritmini ayarlıyor.",
            ], "detail-figure")
        if "insan" in lowered:
            return pick([
                f"{p.capitalize()} taraftaki insan varlığı, fotoğrafı kuru bir görüntü olmaktan çıkarıyor.",
                f"İlk olarak {p} yerleşen insan dokusu göze çarpıyor.",
                f"Bu karede insan izi en çok {p} tarafta hissediliyor.",
            ], "detail-human")
        if "boşluk" in lowered or "biçim" in lowered:
            return pick([
                f"{p.capitalize()} kurulan biçim ve boşluk ilişkisi, fotoğrafı ayakta tutuyor.",
                f"Fotoğrafın en güçlü yanı, {p} hattında kurduğu biçim duygusu.",
                f"İlk anda {p} taraftaki biçim dili, görüntünün omurgasını veriyor.",
            ], "detail-form")
        return pick([
            f"Bu karede ilk ağırlık {p} tarafında toplanıyor.",
            f"Fotoğraf kapısını en çok {p} tarafındaki ana vurgu üzerinden açıyor.",
            f"İlk bakışta {p} tarafında duran ana unsur öne çıkıyor.",
        ], "detail-default")

    def light_sentence() -> str:
        left = float(metrics.get("left_brightness", 0))
        right = float(metrics.get("right_brightness", 0))
        top = float(metrics.get("top_brightness", 0))
        bottom = float(metrics.get("bottom_brightness", 0))
        ...
        if abs(top - bottom) > max(abs(left - right), 10):
            if top > bottom:
                return pick([
                    "Üstten gelen açıklık, sahnedeki ana vurguyu boğmadan görünür kılıyor.",
                    "Yukarıdan gelen ışık, kareye hafif ama etkili bir açıklık veriyor.",
                    "Üst bölümdeki aydınlık, fotoğrafın ağırlığını ezmeden taşıyor.",
                ], "light-top")
            return pick([
                "Alt bölümdeki koyuluk, görüntüye ciddi bir basınç katıyor.",
                "Aşağıdaki koyuluk ile üstteki açıklık birleşince sahne daha katmanlı duruyor.",
                "Alt hattın koyu kalması, fotoğrafa sessiz bir ağırlık veriyor.",
            ], "light-bottom")
        if abs(left - right) > 10:
            if left > right:
                return pick([
                    "Soldan gelen ışık, bakışın yönünü doğal biçimde belirliyor.",
                    "Aydınlığın solda açılması, görüntünün ağırlığını o tarafa topluyor.",
                    "Işığın soldan gelişi, sahnedeki ana vurguya daha hızlı ulaşmamızı sağlıyor.",
                ], "light-left")
            return pick([
                "Sağ taraftaki açıklık, görüntünün nefesini o yönde açıyor.",
                "Işığın sağda açılması, kadrajın diğer yarısını da oyuna katıyor.",
                "Sağdan gelen açıklık, bakışı tek noktaya kilitlemeden dolaştırıyor.",
            ], "light-right")
        if hi > 0.04:
            return pick([
                "Parlak alanlar biraz sertleşse de ışık hâlâ fotoğrafın hissine hizmet ediyor.",
                "Yer yer sert parlaklıklar var ama bunlar sahnenin duygusunu tamamen bozmuyor.",
                "Işık tam pürüzsüz değil; yine de görüntünün asıl hissini taşıyabiliyor.",
            ], "light-hi")
        if "sert" in light:
            return pick([
                "Işığın sertliği burada kaba durmuyor; sahneyi biraz daha diri kılıyor.",
                "Sert ışık, görüntüyü zorlamak yerine daha net bir karakter veriyor.",
                "Buradaki ışık yumuşak değil ama bu sertlik sahneyi ayık tutuyor.",
            ], "light-hard")
        return pick([
            "Işık yumuşak aktığı için sahne kendini zorlamadan açıyor.",
            "Yumuşak ışık, görüntünün duygusunu bastırmadan taşıyor.",
            "Işığın sakin akışı, fotoğrafın insani tarafını daha rahat duyuruyor.",
        ], "light-soft")

    def flow_sentence() -> str:
        return pick([
            f"Göz önce {place(primary)} tarafına oturuyor, sonra {place(secondary)} hattına doğru açılıyor.",
            f"Bakışın yolu, {place(primary)} ile {place(secondary)} arasında kuruluyor.",
            f"Kadrajın akışı tek noktada durmuyor; {place(primary)} tarafından başlayıp {place(secondary)}e uzuyor.",
        ], "flow")

    def density_sentence() -> str:
        if complexity == "yoğun":
            return pick([
                "Arka plandaki kalabalık yapı, fotoğrafı canlı tutuyor ama yer yer ana duyguyu bölüyor.",
                "Kadrajdaki yoğunluk sahneye hayat veriyor; yine de göz bazen fazlaca oyalanıyor.",
                "Yoğun çevre, görüntüyü zenginleştirirken ana etkiyi biraz zorluyor.",
            ], "density-rich")
        if complexity == "sade":
            return pick([
                "Kadrajın sade kalması, görüntünün asıl duygusunu daha temiz duyuruyor.",
                "Boşlukların açık bırakılması, bu karenin nefesini rahatlatıyor.",
                "Sadeliğin korunması, fotoğrafı gereksiz yükten uzak tutuyor.",
            ], "density-clean")
        return pick([
            "Ne fazla kalabalık ne de fazla boş; kadraj kendi dengesini büyük ölçüde koruyor.",
            "Orta yoğunluktaki yapı, bakışın sahne içinde rahatça gezmesine izin veriyor.",
            "Kadrajın yoğunluğu kontrollü; bu da fotoğrafın akışını fazla yormuyor.",
        ], "density-mid")
    ...
    def issue_sentence() -> str:
        ...
        if hierarchy < 58:
            return pick([
                f"Asıl sorun, ana vurgunun {place(primary)} tarafında biraz daha netleşmeye ihtiyaç duyması.",
                "Göz nereye tutunacağını biliyor ama orada daha kararlı kalmak istiyor.",
                "Ana etki orada ama henüz tam kilitlenmiş değil; biraz daha belirginleşebilir.",
            ], "issue-hier")
        if simplicity < 58:
            return pick([
                f"{place(distraction).capitalize()} taraftaki görsel ses azalsa fotoğraf çok daha rahat okunur.",
                f"Bakışı en çok zorlayan yer {place(distraction)}; orası hafifleyince görüntü hemen toparlanır.",
                f"{place(distraction).capitalize()} bölümünde küçük bir sadeleşme, ana etkiyi daha görünür kılar.",
            ], "issue-simple")
        if narrative < 60:
            return pick([
                "Fotoğraf bir şey söylüyor ama cümlesi biraz daha açılırsa çok daha uzun kalır.",
                "His var; sadece bunu biraz daha açık duyurursa etki büyür.",
                "Görüntünün içinde iyi bir duygu var, ama daha net bir karar istiyor.",
            ], "issue-narr")
        return pick([
            "Büyük bir sorun yok; sadece fotoğrafın kalbi biraz daha görünür olabilir.",
            "Küçük bir toparlama ile bu kare çok daha rahat nefes alır.",
            "Ana yapı çalışıyor, ama son küçük kararlarla daha temiz bir etki bırakabilir.",
        ], "issue-ok")

    def meaning_sentence() -> str:
        if scene == "Portre":
            return pick([
                "Burada asıl mesele yalnızca yüzü göstermek değil; o yüzün içinde kalan duyguyu da duyurmak.",
                "Fotoğraf portre olarak çalışıyor çünkü sadece görünüşe değil, ruh hâline de yaslanıyor.",
                "Bu kare, insana yakın durduğu için etkisini teknik taraftan çok his üzerinden kuruyor.",
            ], "meaning-portrait")
        if scene == "Sokak":
            return pick([
                "Gündelik hayatın içinden küçük ama gerçek bir an çekilip alınmış gibi duruyor.",
                "Bu kareyi güçlü kılan şey, sıradan akışın içinde kısa bir temas yakalaması.",
                "Sokak tarafı burada, sahnenin kendiliğinden gelen enerjisinden besleniyor.",
            ], "meaning-street")
        if scene == "Belgesel":
            return pick([
                "Fotoğrafın kıymeti, olanı büyütmeden ama eksiltmeden hissettirebilmesinde.",
                "Bu kare, sahneye dışarıdan hükmetmek yerine onun içinde kalmayı seçiyor.",
                "Burada önemli olan şey, görüntünün tanıklık duygusunu kaybetmemesi.",
            ], "meaning-doc")
        return pick([
            "Buradaki etki, büyük bir olaydan değil küçük ilişkilerin bir araya gelişinden doğuyor.",
            "Fotoğrafın gücü, biçimle his arasında kurduğu sessiz dengeden geliyor.",
            "Bu kare, fazla konuşmadan akılda kalan bir görsel ilişki kuruyor.",
        ], "meaning-abs")

    def suggestion_sentence() -> str:
        hierarchy = float(scores.get("odak_ve_hiyerarsi", 0))
        simplicity = float(scores.get("sadelik", 0))
        if hierarchy < 58:
            return pick([
                "Ana vurguyu biraz daha kararlı kurarsan fotoğraf çok daha net konuşur.",
                "Gözün ilk tutunduğu yeri biraz daha belirginleştirmek bu kareyi büyütür.",
                "Ana etkiyi yarım adım daha öne almak fotoğrafın gücünü hemen artırır.",
            ], "sugg-hier")
        if simplicity < 58:
            return pick([
                "Çevredeki sesi biraz geri çekmek bu karenin bütün ağırlığını ortaya çıkarır.",
                "Küçük bir sadeleşme, görüntünün duygusunu çok daha doğrudan geçirir.",
                "Yan alanlardaki yük hafiflediğinde fotoğrafın asıl cümlesi daha temiz duyulur.",
            ], "sugg-simple")
        return pick([
            "Çok büyük müdahale istemiyor; küçük bir toparlama bu kareye yeter.",
            "Fotoğraf zaten iyi bir yere basıyor, sadece son dokunuşunu bekliyor.",
            "Temel hissi güçlü; birkaç küçük karar onu daha kalıcı yapar.",
        ], "sugg-ok")

    # Editor-specific separate flows
    if editor_name == "Selahattin Kalaycı":
        sentences = [
            pick([
                f"Ben burada önce {subject_group()} görüyorum; fotoğraf ilk ağırlığını oraya bırakmış.",
                f"Bu kare bana ilk anda {subject_group()} üzerinden açıldı; ana baskı orada.",
                opening_subject_detail(),
                _build_object_relation_phrase(result, bits).capitalize() + ".",
            ], "sel-open"),
            pick([
                flow_sentence(),
                light_sentence(),
            ...
            pick([
                "Güzel tarafı şu: fotoğrafın çekirdeği var, sadece daha net karar bekliyor.",
                suggestion_sentence(),
            ], "sel-close"),
        ]
    ...
    if editor_name == "Güler Ataşer":
        sentences = [
            pick([
                f"Ben bu kareye bakınca önce {subject_word()} ile kurulan insani yakınlığı hissediyorum.",
                f"Bu fotoğrafta beni içeri alan şey, {place(primary)} tarafındaki o küçük temas oluyor.",
                f"İlk anda fotoğrafın teknik tarafı değil, bende bıraktığı sıcaklık dikkat çekiyor.",
            ], "gul-open"),
            pick([
                _build_object_relation_phrase(result, bits).capitalize() + ".",
                meaning_sentence(),
            ...
            pick([
                "Bence içinde güzel bir duygu var; biraz daha açıklık kazanınca daha doğrudan geçer.",
                suggestion_sentence(),
            ], "gul-close"),
        ]
        return " ".join(sentences[:4])
    ...
    if editor_name == "Sevgin Cingöz":
        sentences = [
            pick([
                f"Ben burada önce yapıya bakıyorum; kadraj {place(primary)} ile {place(secondary)} arasında kurulmuş.",
                f"Bu kareyi taşıyan şey bence yerleşim; ağırlık tek yerde kalmayıp sahneye yayılıyor.",
                f"İlk gözüme çarpan şey, fotoğrafın akışı nasıl kurduğu oldu.",
            ], "sev-open"),
            pick([
                _build_object_relation_phrase(result, bits).capitalize() + ".",
                flow_sentence(),
            ...
            pick([
                "Temel yapı sağlam; küçük bir toparlama ile çok daha berrak bir sonuç verir.",
                suggestion_sentence(),
            ], "sev-close"),
        ]
        return " ".join(sentences[:4])
    ...
    if editor_name == "Mürşide Çilengir":
        sentences = [
            pick([
                f"Bu kare bende önce bir duygu bırakıyor; {place(primary)} tarafındaki {subject_word()} sessiz ama ağır duruyor.",
                f"İlk anda fotoğrafın yüksek sesini değil, içine kapanan tarafını hissediyorum.",
                f"{opening_subject_detail()} Ama burada asıl kalan şey görüntünün iç sesi oluyor.",
            ], "mur-open"),
            pick([
                _build_object_relation_phrase(result, bits).capitalize() + ".",
                meaning_sentence(),
            ...
            pick([
                "Ben yine de bu karenin bağırmadan konuşmasını kıymetli buluyorum.",
                suggestion_sentence(),
            ], "mur-close"),
        ]
        return " ".join(sentences[:4])
    ...
    if editor_name == "Gülcan Ceylan Çağın":
        sentences = [
            pick([
                f"Bu karede ilk dikkatimi çeken şey kompozisyonun {place(primary)} tarafından kurduğu yön oldu.",
                f"Ben burada en çok yerleşimi seviyorum; bakış doğal olarak {place(primary)} hattından akmaya başlıyor.",
                f"Fotoğrafın taşıyıcı gücü bence kompozisyonda; ana akış oldukça okunur.",
            ], "gulcan-open"),
            pick([
                _build_object_relation_phrase(result, bits).capitalize() + ".",
                flow_sentence(),
            ...
            pick([
                "Yerleşim zaten güçlü; küçük bir sadeleşmeyle çok daha etkili bir görüntü olur.",
                suggestion_sentence(),
            ], "gulcan-close"),
        ]
        return " ".join(sentences[:4])
