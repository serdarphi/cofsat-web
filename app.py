...
def draw_golden_spiral(img: Image.Image) -> Image.Image:
    ...
    if template is not None:
        ...
        alpha = np.clip(alpha.astype(np.float32) * 0.96, 0, 255).astype(np.uint8)

        tint = np.zeros((target_h, target_w, 4), dtype=np.uint8)
        tint[..., 0] = 159
        tint[..., 1] = 241
        tint[..., 2] = 103
        tint[..., 3] = alpha

        tinted_template = Image.fromarray(tint, mode="RGBA")
        overlay.alpha_composite(tinted_template, dest=(offset_x, offset_y))
        return Image.alpha_composite(out, overlay).convert("RGB")
    ...
    offset_x = int((w - rect_w) / 2)
    offset_y = int((h - rect_h) / 2)

    x, y, rw, rh = offset_x, offset_y, rect_w, rect_h
    placements = ["left", "top", "left", "bottom", "right", "top", "left", "bottom", "right", "top"]
    squares: List[Tuple[int, int, int, int, str]] = []
    ...
    for placement in placements:
        ...
        if placement == "left":
            square = (x, y, x + side, y + side, placement)
            x = x + side
            rw = rw - side
        elif placement == "top":
            square = (x, y, x + side, y + side, placement)
            ...
            square = (x + rw - side, y, x + rw, y + side, placement)
            rw = rw - side
        else:
            square = (x, y + rh - side, x + side, y + rh, placement)
            rh = rh - side
    ...
    draw.rounded_rectangle(
        (offset_x, offset_y, offset_x + rect_w, offset_y + rect_h),
        outline=(159, 241, 103, 120),
        width=2,
        radius=10,
    )

    for x1, y1, x2, y2, placement in squares:
        draw.rectangle((x1, y1, x2, y2), outline=(159, 241, 103, 90), width=1)
        start, end = arc_map[placement]
...
def describe_golden_ratio_fit(main_points: List[Tuple[int, int]], w: int, h: int) -> Tuple[str, str]:
    ...
    if d_center < min(w, h) * 0.08:
        return (
            "Altın spiral",
            "Dikkat merkezi, referans spiral şemasındaki iç çekim noktasına yakın davranıyor. Göz dış kavisten iç merkeze doğru kıvrılarak ilerlediği için spiral okuması bu karede daha doğal çalışıyor.",
        )
    return (
        "Altın diyagonaller",
