...
5) yargılayıcı değil yapıcı ve insancıl bir öneri verilsin.
...
EDITOR_PERSONAS = {
    "Selahattin Kalaycı": {
        ...
        "tone": "düşünsel ve sorgulayıcı",
        "entry_style": "soru",
        "decision_style": "yorumu anlam üzerinden açar",
    },
    "Güler Ataşer": {
        "focus": ["ışık", "atmosfer", "dokusal incelik", "renk-ton geçişi"],
        "tone": "şiirsel ve yumuşak",
        "entry_style": "duygu",
        "decision_style": "yorumu atmosfer üzerinden örer",
    },
    "Sevgin Cingöz": {
        "focus": ["kompozisyon", "denge", "figürün yeri", "göz akışı"],
        "tone": "analitik ve net",
        "entry_style": "tespit",
        "decision_style": "yerleşim ve yapı üzerinden karar verir",
    },
    "Mürşide Çilengir": {
        "focus": ["insan", "yakınlık", "duygu", "sessiz gerilim"],
        "tone": "empatik ve insancıl",
        "entry_style": "duygu",
        "decision_style": "yorumunu insani temas üzerinden büyütür",
    },
    "Gülcan Ceylan Çağın": {
        "focus": ["editoryal değer", "yayınlanabilirlik", "seçki gücü", "karar netliği"],
        "tone": "profesyonel ve seçici",
        "entry_style": "karar",
        "decision_style": "yayın mantığıyla konuşur",
    },
}
...
def build_editor_comment_legacy(editor_name: str, result: CritiqueResult) -> str:
    ...
    if hierarchy < 58:
        issue_core = f"Bakış önce {place(primary)} tarafına gidiyor ama orada biraz daha kararlı bir merkez istiyor."
    elif simplicity < 58:
        issue_core = f"{place(distraction).capitalize()} tarafındaki görsel ses hafiflediğinde ana cümle çok daha temiz duyulur."
    elif narrative < 60:
        issue_core = "Fotoğraf bir şey hissettiriyor ama bunu daha açık bir cümleye dönüştürme şansı hâlâ var."
    elif editorial < 62:
        issue_core = "Kare güçlü bir çekirdek taşıyor; fakat seçki içinde daha belirgin durması için son kararlarını biraz daha netleştirmesi gerekiyor."
    else:
        issue_core = "Büyük bir kırılma yok; burada ihtiyaç daha çok küçük ama yerinde bir toparlama."

    if editor_name == "Selahattin Kalaycı":
        sentences = [
            pick([
                f"Bu fotoğraf neden önce {subject_position} duran ağırlık üzerinden konuşuyor?",
                f"Bu kare niçin ilk cümlesini {detail_1} üzerinden kuruyor gibi görünüyor?",
            ], "sel-open"),
            pick([
                f"{detail_2.capitalize()}; bu yüzden görüntü yalnızca görünen şeyi değil, kadraj içindeki niyeti de açığa çıkarıyor.",
                f"{relation} Özellikle {historical_texture_hint} duygusu bu okumayı büyütüyor.",
            ], "sel-obs"),
            pick([
                f"{environment_type.capitalize()} içinde beliren {human_action}, fotoğrafın anlamını basit bir kayıt olmaktan çıkarıyor.",
                f"{light_type_detail.capitalize()} ışık ve {dominant_shape} hissi, sahnenin iç gerilimini görünenden daha derin kılıyor.",
            ], "sel-meaning"),
            issue_core,
            pick([
                "Ben bu karede asıl değeri, neyi gösterdiğinden çok niçin böyle kurulduğunu düşündürmesinde buluyorum.",
                "Küçük bir netleşmeyle bu fotoğraf yalnızca bakılan değil, üstünde durulan bir kareye dönüşebilir.",
            ], "sel-close"),
        ]
    ...
    if editor_name == "Güler Ataşer":
        sentences = [
            pick([
                f"Beni içeri alan ilk şey {detail_1}; orada yumuşak ama kalıcı bir atmosfer var.",
                f"Fotoğrafın bana ilk dokunan tarafı {detail_2}; çünkü sahnenin havası orada açılıyor.",
            ], "gul-open"),
            pick([
                f"{light_type_detail.capitalize()} ışık, {subject_position} duran ağırlığı sertleştirmeden belirginleştiriyor.",
                f"{detail_3.capitalize()}; bu yüzden görüntü yalnızca kurulmuş değil, hissedilmiş de duruyor.",
            ], "gul-obs"),
            pick([
                f"{environment_type.capitalize()} ve {historical_texture_hint}, karenin tonunu sessizce derinleştiriyor.",
                f"{relation} Ben bu karenin duygusunu biraz da bu dokusal geride kalışta buluyorum.",
            ], "gul-meaning"),
            pick([
                issue_core,
                "Bakışı dağıtan küçük yükler azaldığında duygu izleyiciye daha doğrudan geçer.",
            ], "gul-issue"),
            pick([
                "İçinde gerçek bir hava var; onu biraz daha açık bırakmak çoğu şeyi kendiliğinden tamamlar.",
                "Bence bu kare bağırmadan etki kuruyor ve bu çok kıymetli.",
            ], "gul-close"),
        ]
        return " ".join(sentences[:5])
    ...
    if editor_name == "Sevgin Cingöz":
        sentences = [
            pick([
                f"Yerleşim açısından ilk belirleyici veri şu: ana figür ya da ağırlık {subject_position} duruyor.",
                f"Bu kareyi taşıyan ilk yapısal karar, ana etkinin {subject_position} yerleşmiş olması.",
            ], "sev-open"),
            pick([
                f"{detail_2.capitalize()}; bu yüzden göz {place(primary)} ile {place(secondary)} arasında kontrollü dolaşıyor.",
                f"{dominant_shape.capitalize()} hissi, bakışı tek noktada kilitlemek yerine kadraj boyunca yönlendiriyor.",
            ], "sev-obs"),
            pick([
                f"{light_type_detail.capitalize()} ışık ile {visual_tension} düzeydeki gerilim birbirini desteklediği için kompozisyon dağılmıyor.",
                f"{relation} Özellikle {detail_3} yapının ikinci katmanını kuruyor.",
            ], "sev-meaning"),
            pick([
                issue_core,
                f"Özellikle {place(distraction)} tarafındaki ağırlık toparlanırsa göz akışı çok daha berrak olur.",
            ], "sev-issue"),
            pick([
                "Temel kurgu sağlam; o yüzden küçük bir kadraj disiplini büyük fark yaratır.",
                "Ben burada problemi değil, çözülebilir yapısal fazlalığı görüyorum.",
            ], "sev-close"),
        ]
        return " ".join(sentences[:5])
    ...
    if editor_name == "Mürşide Çilengir":
        sentences = [
            pick([
                f"Bu kare bende önce {detail_1} duygusunu bırakıyor.",
            ...
            pick([
                f"{environment_type.capitalize()} içinde kalan {historical_texture_hint}, bu yalnızlık ya da yakınlık duygusunu daha inandırıcı yapıyor.",
                f"{relation} Ben bu karenin kalbini biraz da {subject_position} duran o kırılgan merkezde buluyorum.",
            ], "mur-meaning"),
            pick([
                issue_core,
            ...
            pick([
                ...
                "Fotoğrafın insani çekirdeği yerinde; gerisi onu daha görünür bırakma meselesi.",
            ], "mur-close"),
        ]
        return " ".join(sentences[:5])
    ...
    if editor_name == "Gülcan Ceylan Çağın":
        sentences = [
            pick([
                f"Editoryal açıdan ilk karar noktam {detail_2} oluyor.",
                f"Seçki mantığıyla bakınca bu karede beni tutan ilk şey {detail_1}.",
            ], "gulcan-open"),
            pick([
                f"Ana ağırlığın {subject_position} durması ve {light_type_detail} ışık, fotoğrafın okunmasını hızlandırıyor.",
                f"{relation} Bu ilişki olduğu için kare sıradan bir kayıt seviyesinde kalmıyor.",
            ], "gulcan-obs"),
            pick([
                f"{environment_type.capitalize()} ve {historical_texture_hint}, fotoğrafın editoryal değerini yükselten arka zemini kuruyor.",
                f"{dominant_shape.capitalize()} akış ile {visual_tension} gerilim düzeyi, karenin seçkide tutunma şansını artırıyor.",
            ], "gulcan-meaning"),
            pick([
                issue_core,
                "Fazlalık biraz azalırsa bu kare yayına girecek netliği daha rahat kazanır.",
            ], "gulcan-issue"),
            pick([
                "Ben bu fotoğrafı olmuş saymıyorum ama ciddiye alınacak bir çekirdek gördüğümü net söyleyebilirim.",
                "Seçkiye girip girmemesi son temizliğe bağlı; fakat burada editöryal bir potansiyel kesinlikle var.",
            ], "gulcan-close"),
        ]
        return " ".join(sentences[:5])
