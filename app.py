...
def inject_css() -> None:
    st.markdown(
        ...
        .tab-hint {
            ...
            background: linear-gradient(135deg, rgba(255,159,67,.16), rgba(255,255,255,.04));
            box-shadow: 0 10px 24px rgba(0,0,0,.18);
        }
        .tab-hint strong {color: #ffd7aa; font-size: .98rem;}
        .tab-hint span {color: rgba(255,255,255,.82); font-size: .92rem;}
        ...
        .stTabs [data-baseweb="tab"]:hover {
            background: linear-gradient(180deg, rgba(255,159,67,.16), rgba(255,255,255,.04));
            border-color: rgba(255,159,67,.28);
            transform: translateY(-1px);
        }
        .stTabs [aria-selected="true"] {
            background: linear-gradient(180deg, rgba(255,159,67,.26), rgba(255,255,255,.08)) !important;
            border: 1px solid rgba(255,159,67,.42) !important;
            box-shadow: 0 8px 24px rgba(255,159,67,.12), inset 0 -2px 0 rgba(255,95,95,.92);
            color: #fff !important;
        }
        </style>
        """,
    ...
    components.html(
        ...
        const scrollToResults = () => {
            ...
            if (anchor) {
                ...
                window.parent.scrollTo({top: 760, behavior: 'smooth'});
            }
        };
        setTimeout(scrollToResults, 120);
        setTimeout(scrollToResults, 420);
...
EDITOR_PERSONAS = {
    "Selahattin Kalaycı": "selahattin",
    "Güler Ataşer": "guler",
    "Sevgin Cingöz": "sevgin",
}


EDITOR_CHARACTER_LEVELS = {
    1: {"label": "Yumuşak", "note": "Daha koruyucu, daha nazik ve açıcı bir dil kullanır."},
    2: {"label": "Dengeli", "note": "Eleştiriyi yumuşatmadan ama kırmadan kurar."},
    3: {"label": "Net", "note": "Güçlü ve zayıf tarafları açık, sade ve doğrudan söyler."},
    4: {"label": "Keskin", "note": "Sorunları daha belirgin ve baskılı biçimde öne çıkarır."},
    5: {"label": "Acımasız Netlik", "note": "En açık, en sert ve en filtrelenmemiş editör tonudur."},
}


def get_editor_character_meta(level: int) -> Dict[str, str]:
    try:
        return EDITOR_CHARACTER_LEVELS[int(level)]
    except Exception:
        return EDITOR_CHARACTER_LEVELS[3]


def get_scene_profile_from_result(result: CritiqueResult) -> Dict:
    metrics = result.metrics if isinstance(result.metrics, dict) else {}
    profile = metrics.get("scene_profile", {}) if isinstance(metrics, dict) else {}
    return profile if isinstance(profile, dict) else {}


def soften_for_editor(text: str) -> str:
    return text.replace("Bu karede", "Bu fotoğrafta").replace("kare", "fotoğraf")
...
def _editor_seed(result: CritiqueResult, editor_name: str, level: int) -> int:
    metrics = result.metrics if isinstance(result.metrics, dict) else {}
    width = int(metrics.get("width", 0)) if isinstance(metrics, dict) else 0
    height = int(metrics.get("height", 0)) if isinstance(metrics, dict) else 0
    base = f"{editor_name}|{level}|{result.total_score}|{result.overall_tag}|{width}x{height}|{result.suggested_mode}"
    return abs(hash(base))


def _character_opening(level: int, editor_name: str) -> str:
    if editor_name == "Selahattin Kalaycı":
        mapping = {
            1: [
                "Burada çalışan bir taraf var; yine de fotoğraf biraz daha toparlanmak istiyor.",
                "Fotoğraf bana tamamen kapalı gelmiyor, ama cümlesi henüz tam sıkışmamış.",
            ],
            2: [
                "Fotoğrafın niyeti hissediliyor ama bazı kararlarını biraz daha temiz duymak istiyorum.",
                "Burada bir omurga var, fakat her şey aynı netlikte yerine oturmamış.",
            ],
            3: [
                "Fotoğrafın kuvveti var, ama kırıldığı yerler de saklanmıyor.",
                "İyi bir çekirdek taşıyor; buna rağmen bazı temel kararlar daha açık olmalı.",
            ],
            4: [
                "Fotoğraf iyi niyetle yetinmesin; neyi savunduğunu daha net kurması gerekiyor.",
                "Burada mesele beğenmek değil; fotoğrafın kendi kararını daha kararlı vermesi gerekiyor.",
            ],
            5: [
                "Fotoğrafın bahaneye değil, daha sert karara ihtiyacı var.",
                "Potansiyel var demek yetmiyor; fotoğrafın asıl iddiası daha açık görünmeli.",
            ],
        }
    elif editor_name == "Güler Ataşer":
        mapping = {
            1: [
                "Burada ince bir his var; ben önce onu kaybetmemeye bakıyorum.",
                "Fotoğraf sessizce açılıyor ve o sessizliği korumak önemli.",
            ],
            2: [
                "Fotoğraf izleyiciye yumuşak bir kapı açıyor; biraz daha görünür olsa çok güzel taşıyacak.",
                "İlk anda sıcak bir temas kuruyor, sonra biraz geri çekiliyor.",
            ],
            3: [
                "Samimi bir tarafı var ve o samimiyetin daha iyi taşınabileceği yerler de görünüyor.",
                "Bana kalırsa fotoğrafın duygusu gerçek; sadece onu koruyan yapı biraz daha incelmeli.",
            ],
            4: [
                "Fotoğrafın duygusu güzel, ama ona eşlik etmeyen bölgeler bu duyguyu dağıtıyor.",
                "Ben burada hissi seviyorum; yine de his tek başına yetmiyor, onu taşıyan kararlar da güçlenmeli.",
            ],
            5: [
                "Buradaki asıl his kıymetli, ama fotoğraf bunu koruyamıyorsa bunu açık söylemek gerekir.",
                "Duygusu var; fakat o duyguyu taşıyamayan bölümler geri çekilmezse etki yarım kalır.",
            ],
        }
    else:
        mapping = {
            1: [
                "Fotoğraf nazik bir editoryal öneri sunuyor.",
                "Kare sakin bir yerden konuşuyor ve bunu bir avantaj olarak görüyorum.",
            ],
            2: [
                "Yapısı okunuyor; fakat seçki içinde daha belirgin bir pozisyon isteyebilir.",
                "Bu fotoğrafın editoryal zemini var, ama yerini biraz daha net tarif etmeye ihtiyacı olabilir.",
            ],
            3: [
                "İyi bir editoryal çekirdeği var, ama bütün kararlar aynı güçte değil.",
                "Bana göre fotoğrafın omurgası sağlam; yine de seçki değeri birkaç net karara daha bağlı.",
            ],
            4: [
                "Seçkiye girecekse yapısal kararlarını daha ikna edici hale getirmesi gerekir.",
                "Editoryal tarafta çalışıyor, ama daha güçlü durması için ağırlık merkezini netleştirmeli.",
            ],
            5: [
                "Editoryal açıdan bakınca, bu fotoğrafın cümlesi henüz tam keskinleşmemiş.",
                "Bu fotoğrafı savunmak isterim, ama şu haliyle seçki içinde daha sert bir elekten geçer.",
            ],
        }
    choices = mapping.get(int(level), mapping[3])
    return _pick(choices, int(level) + len(editor_name))


def build_editor_comment(result: CritiqueResult, editor_name: str, selected_character_level: int) -> str:
    profile = get_scene_profile_from_result(result)
    subject_hint = profile.get("subject_hint", "ana vurgu")
    primary_region = profile.get("primary_region", "merkez")
    distraction_region = profile.get("distraction_region", "yan alan")
    mood = profile.get("visual_mood", "dengeli")
    light_character = profile.get("light_character", "yumuşak")
    scene_type = str(profile.get("scene_type", result.suggested_mode or "Genel"))
    eye_flow = profile.get("eye_flow_direction", "göz akışı merkezde toplanıyor")
    opening = _character_opening(selected_character_level, editor_name)
    strength = _clean_sentence(result.key_strength)
    issue = _clean_sentence(result.key_issue)
    move = _clean_sentence(result.one_move_improvement)
    structural = _clean_sentence(soften_for_editor(result.structural_reading))
    editorial = _clean_sentence(soften_for_editor(result.editorial_result))
    first_reading = _clean_sentence(soften_for_editor(result.first_reading))
    seed = _editor_seed(result, editor_name, selected_character_level)

    if editor_name == "Selahattin Kalaycı":
        second = _pick([
            f"Ben ilk olarak {primary_region} tarafına bakıyorum; çünkü fotoğrafın yükü orada toplanıyor.",
            f"Fotoğraf beni önce {primary_region} bölgesine çağırıyor ve bu kötü bir başlangıç değil.",
            f"İlk tutunduğum yer {primary_region}; ana vurgu {subject_hint} çevresinde kurulmuş.",
        ], seed)
        third = _pick([
            f"Ama oradan sonra şu sorun görünür oluyor: {issue}.",
            f"Sonra fotoğrafın aksayan yanı kendini ele veriyor: {issue}.",
            f"Ardından kırılma noktası beliriyor: {issue}.",
        ], seed + 1)
        fourth = _pick([
            f"Yapısal tarafta bana görünen şey şu: {structural}.",
            f"Kadrajın iskeletine baktığımda şunu görüyorum: {structural}.",
            f"Biraz geri çekilip bakınca tablo netleşiyor: {structural}.",
        ], seed + 2)
        fifth = _pick([
            f"Özellikle {distraction_region} bölgesi fazla konuşunca fotoğrafın ana cümlesi gevşiyor ve {eye_flow}.",
            f"{distraction_region.capitalize()} tarafı baskınlaştığında fotoğraf odağını kaybetmeye başlıyor; böyle olunca {eye_flow}.",
            f"Beni en çok zorlayan şey {distraction_region} alanının gereğinden fazla ağırlık taşıması; orada {eye_flow}.",
        ], seed + 3)
        close_soft = _pick([
            f"Buna rağmen iyi çalışan tarafı var: {strength}.",
            f"Hakkını vereyim, fotoğrafın güçlü yanı da açık: {strength}.",
        ], seed + 4)
        close_hard = _pick([
            "Kısacası burada malzeme var, ama fotoğrafın kararını biraz daha sert vermesi gerekiyor.",
            "Benim derdim fotoğrafı ezmek değil; potansiyelini boşa bırakmaması için daha kararlı olmasını istemek.",
        ], seed + 5)
        return "\n\n".join([
            opening,
            second,
            third,
            fourth,
            f"Benim için asıl toparlayıcı hamle şu: {move}.",
            fifth,
            close_hard if selected_character_level >= 4 else close_soft,
        ])

    if editor_name == "Güler Ataşer":
        second = _pick([
            f"Ben bu fotoğrafta önce bıraktığı {mood} havaya giriyorum; göz de doğal olarak {primary_region} bölgesine gidiyor.",
            f"Bende ilk oluşan şey {mood} bir his ve bu his bakışı yavaşça {primary_region} tarafına taşıyor.",
            f"Fotoğrafın ilk nefesi {mood}; sonra bakış {primary_region} çevresinde durmak istiyor.",
        ], seed)
        third = _pick([
            f"Orada bana geçen şey şu oluyor: {strength}.",
            f"Tam o noktada fotoğrafın güzel tarafı açılıyor: {strength}.",
            f"Oradaki temasın kıymeti şu cümlede toplanıyor: {strength}.",
        ], seed + 1)
        fourth = _pick([
            f"Bu yüzden fotoğraf izleyiciyle küçük ama gerçek bir bağ kurabiliyor; özellikle {subject_hint} çevresinde.",
            f"Bence fotoğrafın samimiyeti en çok {subject_hint} tarafında hissediliyor ve bu çok değerli.",
            f"Fotoğrafı taşıyan şey de bu: {subject_hint} çevresinde kurulan dürüst temas.",
        ], seed + 2)
        fifth = _pick([
            f"Yine de {distraction_region} tarafı öne çıktığında bu incelik biraz dağılıyor ve {issue.lower()}.",
            f"Ama {distraction_region} bölgesi fazla belirince kurulan bağ hafifçe zedeleniyor; çünkü {issue.lower()}.",
            f"Beni hafifçe dışarı atan yer de orası: {distraction_region} baskınlaştığında {issue.lower()}.",
        ], seed + 3)
        sixth = _pick([
            f"Işığın {light_character} karakteri güzel bir temel veriyor. Ben olsam şu dokunuşu önemserim: {move}.",
            f"Işığın {light_character} hali aslında fotoğrafı destekliyor; küçük ama etkili müdahale bence şu olur: {move}.",
            f"Bu fotoğrafı biraz daha inceltecek şey bana göre şu: {move}. Işığın {light_character} yapısı zaten buna alan açıyor.",
        ], seed + 4)
        close_soft = _pick([
            "Küçük bir sadeleşmeyle bu fotoğraf çok daha içten ve kalıcı bir etki bırakabilir.",
            "Biraz nefes açılırsa fotoğrafın duygusu izleyicide daha uzun kalır.",
        ], seed + 5)
        close_hard = _pick([
            "Ben sertleşmeyi sevmem ama saklamam da: duyguyu taşıyamayan bölgeler geri çekilmezse fotoğraf yarım kalır.",
            "Duygusu kıymetli; o yüzden onu zayıflatan yerleri biraz daha cesurca kısmak gerekiyor.",
        ], seed + 6)
        return "\n\n".join([opening, second, third, fourth, fifth, sixth, close_hard if selected_character_level >= 4 else close_soft])

    second = _pick([
        f"Ben bu fotoğrafı {scene_type.lower()} bir damarda okuyorum; çünkü ana kurgu {subject_hint} etrafında şekilleniyor.",
        f"Bu kare bende {scene_type.lower()} çizgiye yakın bir izlenim bırakıyor; merkezde yine {subject_hint} var.",
        f"Editoryal okumada ilk his şu: fotoğraf {scene_type.lower()} tarafta bir yere oturuyor ve bunu {subject_hint} üzerinden kuruyor.",
    ], seed)
    third = _pick([
        f"İlk temas cümlem şöyle olurdu: {first_reading}.",
        f"İlk okuma bana şunu söylüyor: {first_reading}.",
        f"Benim ilk editoryal notum şu olur: {first_reading}.",
    ], seed + 1)
    fourth = _pick([
        f"Biraz daha açınca yapısal tablo beliriyor: {structural}.",
        f"Sonra iş yapıya geliyor ve orada gördüğüm şey şu: {structural}.",
        f"Fotoğrafın iskeletini düşündüğümde karşıma çıkan cümle şu: {structural}.",
    ], seed + 2)
    fifth = _pick([
        f"Seçki mantığında karar düğümü bana göre {primary_region} ile {distraction_region} arasındaki ağırlık dağılımında oluşuyor.",
        f"Fotoğrafın seçki içindeki kaderi biraz da {primary_region} ile {distraction_region} arasındaki dengeye bağlı.",
        f"Editoryal kıymeti artıracak yer tam burada: {primary_region} ile {distraction_region} arasındaki ağırlık ilişkisi.",
    ], seed + 3)
    sixth = _pick([
        f"Bu yüzden ben fotoğrafı şu cümleyle özetlerim: {editorial}.",
        f"Toparlayıcı editoryal karşılığı bende şu cümlede toplanıyor: {editorial}.",
        f"Bence fotoğrafın editoryal değeri en çok şu cümlede görünür oluyor: {editorial}.",
    ], seed + 4)
    seventh = _pick([
        f"Öncelikli müdahale olarak ben {move} derim.",
        f"Benim ilk önerim net: {move}.",
        f"Fotoğrafı bir kademe yukarı taşıyacak ilk karar bana göre şu olur: {move}.",
    ], seed + 5)
    close = _pick([
        f"Doğru eşleşmelerle {result.suggested_mode.lower()} bir seçkide daha da iyi çalışabilir." if result.suggested_mode else "Doğru eşleşmelerle seçki içinde daha iyi nefes alabilir.",
        f"Yan yana geleceği fotoğraflar doğru seçilirse etkisi belirgin biçimde büyür.",
    ], seed + 6)
    return "\n\n".join([opening, second, third, fourth, fifth, sixth, seventh, close])


def render_editor_comments(selected_editor_name: str, selected_character_level: int, result: CritiqueResult) -> None:
    st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
    st.markdown("<div class='section-title'>Editör yorumları</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='mini-note'><strong>Aktif editör:</strong> {selected_editor_name}</div>", unsafe_allow_html=True)
    active_comment = build_editor_comment(result, selected_editor_name, selected_character_level)
    st.write(active_comment)
    st.markdown("</div>", unsafe_allow_html=True)

    tabs = st.tabs(list(EDITOR_PERSONAS.keys()))
    for tab, name in zip(tabs, EDITOR_PERSONAS.keys()):
        with tab:
            st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
            st.markdown(f"<div class='section-title'>{name}</div>", unsafe_allow_html=True)
            st.write(build_editor_comment(result, name, selected_character_level))
            st.markdown("</div>", unsafe_allow_html=True)


def render_sidebar_vfinal(selected_mode: str, selected_editor_mode: str, selected_character_level: int, selected_editor_name: str) -> None:
    character_meta = get_editor_character_meta(selected_character_level)
    with st.sidebar:
        st.markdown("<div class='sidebar-card'>", unsafe_allow_html=True)
        st.markdown("### ÇOFSAT Kontrol Alanı")
        st.markdown(
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("**Karakter seviyesi**")
        st.markdown(f"<div class='ghost-badge'>{character_meta['label']}</div>", unsafe_allow_html=True)
        st.markdown(
            f"<div class='mini-note'>{character_meta['note']}</div>",
            unsafe_allow_html=True,
        )

        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("**Kavuniçi editörler**")
        for name in EDITOR_PERSONAS.keys():
            badge = "✓" if name == selected_editor_name else "•"
            st.markdown(f"<div class='ghost-badge' style='display:block; width:100%; margin-bottom:.4rem'>{badge} {name}</div>", unsafe_allow_html=True)
        st.markdown("<div class='mini-note'>Yukarıdaki seçim aktif editör yorumunu belirler. Aşağıda üç editörün de fotoğrafa özel yorumu ayrıca görünür.</div>", unsafe_allow_html=True)
...
def main_vfinal() -> None:
    ...
    with st.sidebar:
        selected_mode = st.selectbox("Tür", list(MODE_PROFILES.keys()), index=0)
        selected_editor_mode = st.selectbox("Ton", list(EDITOR_MODES.keys()), index=0)
        selected_character_level = st.select_slider(
            "Karakter seviyesi",
            options=[1, 2, 3, 4, 5],
            value=3,
            format_func=lambda x: get_editor_character_meta(x)["label"],
        )
        selected_editor_name = st.radio("Editör", list(EDITOR_PERSONAS.keys()), index=0)
        st.session_state["editor_character_level"] = selected_character_level
        st.session_state["selected_editor_name"] = selected_editor_name

    render_sidebar_vfinal(selected_mode, selected_editor_mode, selected_character_level, selected_editor_name)
        ...
        st.markdown(
            ...
            <div class="hero">
                ...
                <div class="hero-badges">
                    <span class="hero-badge">Aktif tür: {selected_mode}</span>
                    <span class="hero-badge">Ton: {selected_editor_mode}</span>
                    <span class="hero-badge">Karakter: {get_editor_character_meta(selected_character_level)["label"]}</span>
                    <span class="hero-badge">Editör: {selected_editor_name}</span>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    if uploaded_file is None:
        st.markdown(
            ...
            <div class="panel-card">
                <div class="section-title">Hazır olduğunda analiz burada görünecek</div>
                <div class="mini-note">
                    Sonuç ekranında genel skor, editör özeti, fotoğrafa özel editör yorumları, heatmap,
                    göz akışı, dikkat dağıtan alanlar, altın oran şemaları ve çekim notları yer alır.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with top_right:
        ...
        with c2:
            st.metric("Önerilen Tür", result.suggested_mode)
        with c3:
            st.metric("Aktif Editör", selected_editor_name)
        st.progress(result.total_score / 100.0)
        st.info(result.editor_summary)
        render_pill_row(result.tags)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    render_editor_comments(selected_editor_name, selected_character_level, result)

    st.markdown(
        "<div class='tab-hint'><strong>İnceleme sekmeleri</strong><span>Aşağıdaki başlıklara tıklayarak görsel katmanlar, editör yorumu ve teknik okuma arasında geçiş yapın.</span></div>",
        unsafe_allow_html=True,
    )
    ...
    with tab3:
        ...
        with right:
            ...
            st.markdown(f"<div class='mini-note'><strong>Önerilen tür nedeni:</strong> {result.suggested_mode_reason}</div>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)

    with tab4:
        c1, c2, c3 = st.columns(3, gap="large")
        with c1:
            st.markdown("<div class='panel-title'>Phi grid</div>", unsafe_allow_html=True)
            st.image(phi_grid_img, use_container_width=True)
        with c2:
            st.markdown("<div class='panel-title'>Altın diyagonaller</div>", unsafe_allow_html=True)
            st.image(diagonal_img, use_container_width=True)
        with c3:
            ...
            st.image(spiral_img, use_container_width=True)
        st.markdown(
            f"<div class='panel-card'><div class='section-title'>En uygun şema</div><div class='mini-note'><strong>{best_scheme}</strong> · {scheme_reason}</div></div>",
            unsafe_allow_html=True,
        )

    with tab5:
        st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
        st.markdown("<div class='section-title'>Teknik veriler</div>", unsafe_allow_html=True)
        metrics = result.metrics if isinstance(result.metrics, dict) else {}
        for key, value in metrics.items():
            ...
            if isinstance(value, float):
                ...
                display = str(value)
            st.markdown(f"<div class='mini-note'><strong>{key}</strong>: {display}</div>", unsafe_allow_html=True)
        profile = metrics.get("scene_profile", {}) if isinstance(metrics, dict) else {}
        if isinstance(profile, dict) and profile:
            st.markdown("<div class='section-title'>Sahne profili</div>", unsafe_allow_html=True)
            for key, value in profile.items():
                st.markdown(f"<div class='mini-note'><strong>{key}</strong>: {value}</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
