    ...
    for editor_name in ordered_names + remaining:
        ...
        st.markdown(
            ...
            <div class='pool-comment-card'>
                ...
                <div class='pool-comment-text'>{escape(raw_comment)}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def openai_editor_comment_cached(image_bytes: bytes, editor_name: str, mode: str, editor_mode: str) -> Dict:
    ...
    editor_rules = {
        ...
        "Gülcan Ceylan Çağın": {
            "focus": "kompozisyon, ışık, denge, katman, perspektif, ritim, mekânsal ilişki, modelin konumu ve bunların anlattığı duygu",
            "avoid": "sert editoryal hüküm, reddedici dil, kuru teknik rapor, diğer editörlerle aynı ses",
        },
    }
...
_GENERIC_COMMENT_PATTERNS = [
    (r"\bgüçlü bir atmosfer\b", "daha somut bir atmosfer"),
    (r"\bizleyiciyle bağ\b", "izleyicide karşılık"),
    (r"\bhikâye hissi\b", "anlatım duygusu"),
    (r"\bgüçlü yanı\b", "çalışan tarafı"),
    (r"\bbiraz daha\b", "bir parça daha"),
]


def _normalize_comment_texture(text: str) -> str:
    text = (text or '').strip()
    for pattern, repl in _GENERIC_COMMENT_PATTERNS:
        text = re.sub(pattern, repl, text, flags=re.IGNORECASE)
    text = re.sub(r'\s+', ' ', text).strip()
    return text


def _ensure_comment_breathing(text: str) -> str:
...
def _elevate_editor_comment(editor_name: str, text: str) -> str:
    text = _normalize_comment_texture(text)
    lower = text.lower()

    if editor_name == "Gülcan Ceylan Çağın":
        if "kompozisyon" not in lower and "kadraj" not in lower:
            text += " Kadraj içinde kurulan ilişki kompozisyonun etkisini görünür kılıyor."
        if not any(x in lower for x in ["ışık", "ton", "gölge"]):
            text += " Işık ve ton geçişleri de bu anlatımı destekliyor."
        if not any(x in lower for x in ["denge", "katman", "perspektif", "ritim"]):
            text += " Denge ve katman duygusu karenin daha derli toplu okunmasını sağlıyor."
        if "tebrikler" not in lower:
            text = text.rstrip('. ') + "; tebrikler."

    elif editor_name == "Güler Ataşer":
        if not any(x in lower for x in ["ışık", "ton", "doku"]):
            text += " Işık, ton ve doku ilişkisi fotoğrafın hissini belirginleştiriyor."
        if not any(x in lower for x in ["ifade", "beden dili", "bakış"]):
            text += " İfade ve beden dili de sahnenin duygusunu taşıyor."

    elif editor_name == "Sevgin Cingöz":
        if not any(x in lower for x in ["denge", "yerleşim", "göz akışı", "eksen"]):
            text += " Yerleşim ve göz akışı tarafında fotoğrafın ana kararı daha net okunuyor."

    elif editor_name == "Mürşide Çilengir":
        if not any(x in lower for x in ["insan", "yakınlık", "kırılgan", "beden dili"]):
            text += " İnsani temas ve kırılganlık hissi, bu karenin asıl duygusal çekirdeğini kuruyor."

    elif editor_name == "Ilkay Strebel-Ozmen":
        if not any(x in lower for x in ["ışık", "ton", "detay"]):
            text += " Işık, ton ve küçük detaylar fotoğrafın sıcaklığını büyütüyor."
        if not any(x in lower for x in ["yaşam", "emek", "çevresel"]):
            text += " Çevresel yapı ve yaşam hissi görüntüyü daha samimi kılıyor."

    elif editor_name == "Selahattin Kalaycı":
        if not any(x in lower for x in ["niyet", "eşik", "gerilim", "anlam"]):
            text += " Fotoğrafın asıl ağırlığı, görünenin altında kurduğu düşünsel gerilimde toplanıyor."

    return _ensure_comment_breathing(text)
...
def _finalize_editor_comment(editor_name: str, text: str, add_signature: bool = True) -> str:
    ...
    text = _elevate_editor_comment(editor_name, text)
    if editor_name == "Gülcan Ceylan Çağın":
        text = _soften_gulcan_comment(text)
    if add_signature:
        text = _apply_editor_signature(editor_name, text)
    text = _strip_trailing_editor_name(text)
    text = _ensure_comment_breathing(text)
...
def build_editor_comment_legacy(editor_name: str, result: CritiqueResult) -> str:
    ...
    if editor_name == "Gülcan Ceylan Çağın":
        verdict = "Burada çalışan güçlü bir çekirdek var." if editorial >= 70 and hierarchy >= 65 else "Burada iyi bir kompozisyon çekirdeği hissediliyor." if editorial >= 58 else "Bu kare küçük bir toparlamayla çok daha etkili bir anlatıma dönüşebilir."
        sentences = [
            pick([
                f"Bu karede önce {detail_1} ile {subject_position} kurulan görsel ilişki dikkat çekiyor; {verdict}",
                f"İlk anda {light_type_detail} ışık, {detail_2} ve ana ağırlığın {subject_position} duruşu birlikte etkili bir denge kuruyor; {verdict}",
            ], "gulcan-open"),
            pick([
                f"{detail_3.capitalize()} ve {dominant_shape} akış, kompozisyonun kendi ritmini kurmasına yardım ediyor.",
                f"{environment_type.capitalize()} ile {historical_texture_hint}, fotoğrafın yalnızca estetik değil anlatısal bir zemin de taşımasını sağlıyor.",
            ], "gulcan-obs"),
            pick([
                f"Özellikle ışık, ton geçişleri ve katman duygusu bir araya geldiğinde kare daha içten ve dengeli okunuyor.",
                f"Modelin/öznenin konumu ile arka plan arasındaki mekânsal ilişki, anlatımı güçlendiren taraflardan biri oluyor.",
            ], "gulcan-meaning"),
            pick([issue_core, f"Özellikle {place(distraction)} tarafındaki fazlalık hafiflediğinde ana vurgu, denge ve kompozisyon cümlesi daha temiz duyulur."], "gulcan-issue"),
            pick([
                "Ton geçişleri, kurulan denge ve kompozisyon birlikte düşünüldüğünde etkili ve sıcak bir anlatım çıkıyor; tebrikler.",
                "Güzel bir kompozisyon ve içten bir duygu aktarımı var; küçük bir toparlamayla etkisi daha da yükselir, tebrikler.",
            ], "gulcan-close"),
        ]
        return " ".join(sentences[:5])
