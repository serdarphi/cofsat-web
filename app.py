...
def _cofsat_text_block(draw, box, title, body, color="#F0B24D", title_color=None, max_lines=7):
    ...
    tf = _cofsat_font(23, True)
    bf = _cofsat_font(18, False)
...
def _cofsat_draw_overlay_on_placed(draw, placed, reading: Dict, strong: bool = True):
    ...
    if nb:
        overlay = Image.new("RGBA", (draw.im.size[0], draw.im.size[1]), (0, 0, 0, 0))
        od = ImageDraw.Draw(overlay)
        od.rounded_rectangle(nb, radius=10, outline=(0, 135, 255, 230), width=3 if strong else 2, fill=(0, 110, 220, 36 if strong else 22))
    ...
    if mb:
        overlay = Image.new("RGBA", (draw.im.size[0], draw.im.size[1]), (0, 0, 0, 0))
        od = ImageDraw.Draw(overlay)
        od.rounded_rectangle(mb, radius=12, outline=(255, 115, 20, 255), width=4 if strong else 3, fill=(255, 120, 20, 44 if strong else 25))
        draw.im.alpha_composite(overlay)
    # İkincil özne
    ...
    if sb:
        overlay = Image.new("RGBA", (draw.im.size[0], draw.im.size[1]), (0, 0, 0, 0))
        od = ImageDraw.Draw(overlay)
        od.rounded_rectangle(sb, radius=14, outline=(160, 40, 230, 255), width=4 if strong else 3, fill=(160, 40, 230, 38 if strong else 20))
        draw.im.alpha_composite(overlay)
    # Yön çizgisi
    try:
...
@st.cache_data(show_spinner=False)
def create_cofsat_reading_poster_cached(image_bytes: bytes, language: str = "tr") -> Dict:
    ...
    return {"image_bytes": out.getvalue(), "model": "cofsat-safe-poster-v2"}
...
def render_cofsat_reading_interface() -> None:
    ...
    if st.session_state.get("_cofsat_reading_result") is None:
        with st.spinner("ÇOFSAT fotoğrafı okuyor ve render hazırlıyor..."):
            st.session_state["_cofsat_reading_result"] = create_cofsat_reading_poster_cached(image_bytes, "tr")
