...
def build_editor_comment(result: CritiqueResult, editor_name: str, selected_character_level: int) -> str:
    ...
    if editor_name == "Selahattin Kalaycı":
        lines = [
            f"Bu fotoğrafta en net mesele şu: {result.key_issue}",
            f"İlk tutunan yer {primary_region} tarafı ve okuma {subject_hint} üzerinden açılıyor; ama fotoğrafın devamında aynı kuvvet korunmuyor.",
            soften_for_editor(result.structural_reading),
            f"{distraction_region.capitalize()} bölgesi gereğinden fazla ses çıkarıyorsa ana etki dağılıyor.",
            f"Burayı toparlayacak en doğru hamle şu olur: {result.one_move_improvement}",
        ]
        if selected_character_level >= 4:
            lines.insert(1, "Fotoğrafın iyi tarafı var ama kararlarını biraz daha netleştirmesi gerekiyor.")
        return "\n\n".join(lines)

    if editor_name == "Güler Ataşer":
        lines = [
            f"Bu fotoğrafın bende bıraktığı ilk duygu {mood} bir atmosfer kurduğu yönünde.",
            f"Özellikle {result.key_strength.lower()} Bu, izleyiciyle küçük ama gerçek bir temas kuruyor.",
            f"Bakışın {primary_region} bölgesinde toplanması güzel; fakat {distraction_region} tarafı zaman zaman bu hissi dağıtabiliyor.",
            f"Işığın {light_character} karakteri fotoğrafı destekliyor, ama fotoğraf biraz daha açılmak istiyor: {result.key_issue}",
            f"Küçük ama doğru bir dokunuşla çok daha etkili olabilir: {result.one_move_improvement}",
        ]
        return "\n\n".join(lines)

    lines = [
        f"Bu fotoğraf editoryal olarak bakıldığında önce {subject_hint} etrafında bir okuma kuruyor.",
        soften_for_editor(result.first_reading),
        soften_for_editor(result.structural_reading),
        f"Şu anda en kritik karar noktası {distraction_region} bölgesindeki görsel ağırlığın kontrolü ile ana odağın güçlendirilmesi arasında duruyor.",
        f"Bu yüzden benim kısa sonucum şu: {result.editorial_result}",
    ]
    if result.suggested_mode:
        lines.append(f"Bu kareyi ben en çok {result.suggested_mode.lower()} damarda çalışırken görüyorum.")
    return "\n\n".join(lines)
