...
def inject_css() -> None:
    st.markdown(
        ...
        div[data-baseweb="popover"] [role="option"][aria-selected="true"] {
            background: rgba(255,140,66,0.32) !important;
            color: #111111 !important;
...
def safe_describe_golden_ratio_fit(main_points: List[Tuple[int, int]], w: int, h: int) -> Tuple[str, str]:
    fn = globals().get("describe_golden_ratio_fit")
    if callable(fn):
        return fn(main_points, w, h)
    x1, x2, y1, y2 = phi_grid_positions(w, h)
    intersections = [(x1, y1), (x1, y2), (x2, y1), (x2, y2)]
    center = [(w / 2, h / 2)]
    d_intersections = distance_to_nearest(main_points, intersections)
    ...
    if d_center < min(w, h) * 0.08:
        return (
            "Altın spiral",
            "Dikkat ağırlığı merkez çevresinde toplanıyor. Spiral okuması bu kareyi daha iyi açıklıyor.",
        )
    return (
        "Altın diyagonaller",
        "Dikkat akışı köşeler arası yönleniyor. Diyagonal yapı bu kare için daha açıklayıcı görünüyor.",
    )
...
def main() -> None:
    ...
    with st.spinner("Fotoğraf okunuyor, metrikler çıkarılıyor ve görsel katmanlar hazırlanıyor..."):
        ...
        best_scheme, scheme_reason = safe_describe_golden_ratio_fit(main_points, image.size[0], image.size[1])
