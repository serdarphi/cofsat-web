...
def get_candidate_pool_date(now: datetime | None = None) -> str:
...
CANDIDATE_LEVEL_LABELS = {
    "cok_guclu_aday": "Çok güçlü aday",
    "guclu_aday": "Güçlü aday",
    "aday": "Aday",
    "zayif_aday": "Zayıf aday",
}

CANDIDATE_LEVEL_IMPACTS = {
    "cok_guclu_aday": 3.0,
    "guclu_aday": 1.5,
    "aday": 0.0,
    "zayif_aday": -2.0,
...
def derive_candidate_level_from_comment(comment: str) -> str:
    text = (comment or "").strip().lower()
    if not text:
        return "aday"

    strong_negative = [
        "zayıf", "ikna etmiyor", "tam oturmuyor", "dağınık", "kararsız", "fazla yük", "çalışmıyor"
    ]
    moderate_negative = [
        "toparlanır", "gelişebilir", "küçük düzenleme", "biraz daha", "potansiyel", "çekirdek"
    ]
    strong_positive = [
        "çok güçlü", "çok etkili", "ders niteliğinde", "çok güzel", "seyir keyfi yüksek",
        "oldukça güçlü", "yayın ihtimali belirgin", "çok katmanlı", "film tadında", "zamansız"
    ]
    positive = [
        "güzel", "etkili", "iyi", "başarılı", "güçlü", "yerinde", "dengeli", "merak uyandıran",
        "etkili bir kompozisyon", "duygu aktarımı güçlü", "güzel kompozisyon"
    ]

    if any(p in text for p in strong_negative):
        return "zayif_aday"
    if any(p in text for p in strong_positive):
        return "cok_guclu_aday"
    if any(p in text for p in moderate_negative):
        return "aday"
...
def summarize_candidate_levels(editor_reviews: dict) -> tuple[str, float, int, float]:
    values = []
    impacts = []
    for payload in (editor_reviews or {}).values():
        level = str((payload or {}).get("candidate_level") or "aday")
        values.append(CANDIDATE_LEVEL_VALUES.get(level, 2))
        impacts.append(float((payload or {}).get("impact_score", CANDIDATE_LEVEL_IMPACTS.get(level, 0.0)) or 0.0))
    if not values:
        return "aday", 2.0, 0, 0.0
    avg = sum(values) / len(values)
    avg_impact = sum(impacts) / len(impacts) if impacts else 0.0
    if avg >= 3.5:
        level = "cok_guclu_aday"
    elif avg >= 2.5:
        level = "guclu_aday"
    elif avg >= 1.5:
        level = "aday"
        ...
        level = "zayif_aday"
    return level, round(avg, 2), len(values), round(avg_impact, 2)


def save_photo_result(image_bytes: bytes, uploaded_name: str, result, editor_name: str = "", editor_comment: str = "") -> None:
    now = datetime.now(TURKEY_TZ)
    pool_date = get_candidate_pool_date(now)
    image_hash = hashlib.md5(image_bytes).hexdigest()
    base_score = float(getattr(result, "total_score", 0.0) or 0.0)
    data = _safe_json_load(PHOTO_HISTORY_FILE)

    existing = None
    remaining = []
    for item in data:
        if item.get("date") == pool_date and item.get("image_hash") == image_hash and existing is None:
            existing = dict(item)
        else:
            remaining.append(item)

    editor_reviews = {}
    if isinstance(existing, dict) and isinstance(existing.get("editor_reviews"), dict):
        editor_reviews = dict(existing.get("editor_reviews") or {})

    if editor_name:
        candidate_level = derive_candidate_level_from_comment(editor_comment)
        impact_score = CANDIDATE_LEVEL_IMPACTS.get(candidate_level, 0.0)
        editor_reviews[editor_name] = {
    ...
    aggregate_level, aggregate_value, editor_count, average_editor_impact = summarize_candidate_levels(editor_reviews)
    final_score = round(base_score + average_editor_impact, 2)

    editors = list(editor_reviews.keys())
    merged = {
        "date": pool_date,
        "image_hash": image_hash,
        "filename": (uploaded_name or (existing or {}).get("filename") or "fotoğraf"),
        "base_score": round(base_score, 2),
        "score": final_score,
        "final_score": final_score,
        "average_editor_impact": average_editor_impact,
        "aggregate_candidate_level": aggregate_level,
        "aggregate_candidate_value": aggregate_value,
        "mode": getattr(result, "suggested_mode", "") or (existing or {}).get("mode", ""),
        "level": getattr(result, "overall_level", "") or (existing or {}).get("level", ""),
        "timestamp": now.isoformat(),
        "thumbnail": _thumbnail_data_url(image_bytes) or (existing or {}).get("thumbnail", ""),
        "editors": editors,
        "editor_count": editor_count,
        "editor_reviews": editor_reviews,
    }
    if editor_name:
        merged["editor"] = editor_name
        merged["last_editor_comment"] = editor_comment

    remaining.append(merged)
    _safe_json_save(PHOTO_HISTORY_FILE, remaining[-500:])


def get_today_photo_entries(selected_date: str | None = None) -> list:
    target_date = selected_date or get_candidate_pool_date()
    data = _safe_json_load(PHOTO_HISTORY_FILE)
    todays = [item for item in data if item.get("date") == target_date]
    return sorted(
        todays,
        key=lambda x: (
            float(x.get("final_score", x.get("score", 0)) or 0),
            float(x.get("aggregate_candidate_value", 0) or 0),
            int(x.get("editor_count", 0) or 0),
            x.get("timestamp", ""),
        ),
        reverse=True,
    )
...
def select_photo_of_the_day(selected_date: str | None = None) -> dict:
    ...
    if target_date == today_str:
        winner["ready"] = now.time() >= time(PHOTO_OF_DAY_HOUR, 0)
    else:
        winner["ready"] = target_date < today_str
...
def render_photo_of_day_candidates(context: str = "main") -> None:
    now = get_turkey_now()
    target_date = get_candidate_pool_date(now)
    entries = get_today_photo_entries(target_date)
    if not entries:
        return

    pod = select_photo_of_the_day(target_date)
    if not pod:
        return
    ...
    if context == "sidebar":
        ...
        st.markdown(
            f"<div class='mini-note'>Seviye / Final: {escape(CANDIDATE_LEVEL_LABELS.get(str(pod.get('aggregate_candidate_level') or 'aday'), 'Aday'))} · Gün içi yükleme: {int(pod.get('entries', 0))}</div>",
            unsafe_allow_html=True,
        )
        if pod.get("ready"):
            st.success(f"Türkiye saatine göre {PHOTO_OF_DAY_HOUR:02d}:00 sonrası en güçlü aday günün fotoğrafı olarak seçildi.")
        else:
            st.info(f"Saat {PHOTO_OF_DAY_HOUR:02d}:00'a kadar adaylar burada görünür. Şu an Türkiye saati {now.strftime('%H:%M')}. {PHOTO_OF_DAY_HOUR:02d}:00 sonrası yüklenen fotoğraflar yarının aday havuzuna alınır.")
        ...
        for idx, entry in enumerate(entries[:6]):
            ...
            with cols[1]:
                ...
                st.caption(f"Aday seviyesi: {CANDIDATE_LEVEL_LABELS.get(str(entry.get('aggregate_candidate_level') or 'aday'), 'Aday')} · Final puan: {float(entry.get('final_score', entry.get('score', 0)) or 0):.1f} · Editör: {int(entry.get('editor_count', 0))}")
    ...
    status_text = (
        f"Türkiye saatine göre {PHOTO_OF_DAY_HOUR:02d}:00 sonrası en güçlü aday otomatik olarak günün fotoğrafı oldu."
        if pod.get("ready")
        else f"Şu an lider fotoğraf burada öne çıkıyor. Saat {PHOTO_OF_DAY_HOUR:02d}:00 olduğunda seçim otomatik kesinleşecek. Bu saatten sonra gelen yüklemeler yarının adaylarına yazılır."
    )
        ...
        st.markdown(
            ...
            <div class='pod-meta-grid'>
                <div class='pod-meta-card'>
                    <div class='pod-meta-label'>Final puan</div>
                    <div class='pod-meta-value'>{float(pod.get('final_score', pod.get('score', 0)) or 0):.1f}</div>
                </div>
                <div class='pod-meta-card'>
    ...
    for idx, entry in enumerate(entries[:9]):
        ...
        with col:
            ...
            st.markdown(f"<div class='candidate-score-badge'>{float(entry.get('final_score', entry.get('score', 0)) or 0):.1f} · {escape(CANDIDATE_LEVEL_LABELS.get(str(entry.get('aggregate_candidate_level') or 'aday'), 'Aday'))}</div>", unsafe_allow_html=True)
            ts = str(entry.get('timestamp', ''))
            if 'T' in ts:
                st.caption(f"Saat: {ts[11:16]}")
...
def build_facebook_caption(photo_entry: dict) -> str:
    ...
    return (
        ...
        f"Final puan: {float(photo_entry.get('final_score', photo_entry.get('score', 0)) or 0):.1f}\nAday seviyesi: {CANDIDATE_LEVEL_LABELS.get(str(photo_entry.get('aggregate_candidate_level') or 'aday'), 'Aday')}\n\n"
...
EDITOR_PERSONAS = {
    ...
    "Güler Ataşer": {
        "focus": ["kompozisyon", "ışık", "ton", "doku", "yansıma"],
        "tone": "analitik, dengeli, açıklayıcı",
        "entry_style": "kompozisyon ya da ışık tespitiyle açar",
        "decision_style": "somut görsel veriyi net ama yumuşak bir dille tartar",
        "must_do": ["en az iki somut ayrıntı an", "kompozisyonun neden çalıştığını kısa ve açık söyle"],
        "avoid": ["aşırı şiirsellik", "muğlak atmosfer cümleleri"],
    },
    "Sevgin Cingöz": {
        "focus": ["beden dili", "ifade aktarımı", "kadraj", "seyir keyfi"],
        "tone": "sıcak, akıcı, izleyici odaklı",
        "entry_style": "ifade ya da hareketten açar",
        "decision_style": "figürün taşıdığı duygu ve sahnenin okunurluğu üzerinden karar verir",
        "must_do": ["beden dili ya da bakışı mutlaka an", "olumlu ama sahici bir sonuç cümlesi kur"],
        "avoid": ["soğuk yapısal jargon", "gereksiz sertlik"],
    },
    "Mürşide Çilengir": {
        "focus": ["hikâye", "duygu", "belgesel katman", "atmosfer"],
        "tone": "şiirsel, anlatıcı, güçlü duygusal yoğunluk taşıyan",
        "entry_style": "sahnenin taşıdığı hikâye ya da duyguyla açar",
        "decision_style": "küçük bir ayrıntıdan insan hikâyesine ulaşır",
        "must_do": ["en az bir güçlü duygu cümlesi kur", "somut ayrıntıyı hikâye kapısına çevir"],
        "avoid": ["kuru teknik dil", "fazla kısa yorum"],
    },
    "Gülcan Ceylan Çağın": {
...
EDITOR_VOICE_ENGINES = {
    ...
    "Güler Ataşer": {
        "lexicon": ["kompozisyon", "ton", "netlik", "doku", "yansıma", "ışık kullanımı"],
        "openings": [
            "Bu karede ilk dikkat çeken şey kompozisyonun kendi dengesi:",
            "Önce ışık ve yerleşim ilişkisi konuşuyor:",
            "Kompozisyon amacı doğru yakalamış; ilk gördüğüm şey şu:"
        ],
        "connectors": ["özellikle", "bu yüzden", "aynı anda", "burada"],
        "closings": [
            "Kompozisyon amacı yakalamış ve etkili bir görüntü oluşmuş.",
            "Tonlar ve detaylar birlikte çalıştığı için kare daha da etkili oluyor.",
            "Güzel ışık kullanımı ve doku desteğiyle güçlü bir sonuç çıkmış."
        ],
        "cadence": "açık tespit / somut görsel veri / kısa sonuç",
    },
    "Sevgin Cingöz": {
        "lexicon": ["beden dili", "ifade aktarımı", "seyir keyfi", "etkili kadraj", "tonlar", "hareket"],
        "openings": [
            "Bu karede beni ilk yakalayan şey modelin beden dili:",
            "Kadrajın etkisi kadar ifade aktarımı da belirleyici:",
            "Seyir keyfi yüksek bir an duygusu var; ilk dikkat çeken taraf şu:"
        ],
        "connectors": ["özellikle", "aynı anda", "bu nedenle", "burada"],
        "closings": [
            "Etkili tonları ve ifade aktarımıyla güzel bir fotoğraf olmuş.",
            "Modelin katkısıyla seyir keyfi yüksek bir kompozisyon çıkmış.",
            "Güzel zamanlama ve beden diliyle kare akılda kalıyor."
        ],
        "cadence": "duygu odaklı giriş / figür okuması / sıcak sonuç",
    },
    "Mürşide Çilengir": {
        "lexicon": ["hikâye", "yalnızlık", "masumiyet", "sessizlik", "derin iz", "atmosfer"],
        "openings": [
            "Bu karede ilk hissettiğim şey sahnenin taşıdığı hikâye:",
            "Gözün gördüğünden önce fotoğrafın duygusu geliyor:",
            "Bu sahne sessiz ama çok şey anlatıyor; ilk açılan kapı şu:"
        ],
        "connectors": ["sanki", "bir taraftan", "orada", "bence"],
        "closings": [
            "Işık ve gölge hikâyeyi sessizce ama etkileyici anlatmış.",
            "Duygu aktarımı güçlü bir portre olmuş.",
            "Çok katmanlı anlatımıyla güzel bir fotoğraf çıkmış."
        ],
        "cadence": "şiirsel giriş / hikâye açılımı / güçlü duygusal kapanış",
    },
    "Gülcan Ceylan Çağın": {
...
EDITOR_SIGNATURES = {
    ...
    "Güler Ataşer": {
        "subject_word": "kare",
        "signature_phrases": ["kompozisyon amacı", "tonlar", "doku"],
    },
    "Sevgin Cingöz": {
        "subject_word": "fotoğraf",
        "signature_phrases": ["beden dili", "ifade aktarımı", "seyir keyfi"],
    },
    "Mürşide Çilengir": {
        "subject_word": "sahne",
        "signature_phrases": ["hikâye", "yalnızlık", "duygu aktarımı"],
    },
    "Gülcan Ceylan Çağın": {
...
EDITOR_SHARED_WORD_REMAP = {
    ...
    "Güler Ataşer": {"fotoğraf": "kare", "görüntü": "kare", "ana vurgu": "kompozisyon merkezi"},
    "Sevgin Cingöz": {"görüntü": "fotoğraf", "kompozisyon": "fotoğraf", "ana vurgu": "ifade merkezi"},
    "Mürşide Çilengir": {"fotoğraf": "sahne", "kare": "sahne", "ana vurgu": "hikâye çekirdeği"},
...
def build_editor_comment_legacy(editor_name: str, result: CritiqueResult) -> str:
    ...
    if editor_name == "Güler Ataşer":
        sentences = [
            pick([
                f"Bu karede ilk dikkat çeken şey {detail_1}; kompozisyon amacı doğru kurulmuş.",
                f"Önce {detail_2} göze geliyor; çünkü ışıkla yerleşim birbirini iyi tamamlıyor.",
            ], "gul-open"),
            pick([
                f"{light_type_detail.capitalize()} ışık, {subject_position} duran ağırlığı netleştirirken ton geçişlerini de kontrollü tutuyor.",
                f"{detail_3.capitalize()}; bu yüzden karede doku, netlik ve ton aynı anda çalışıyor.",
            ], "gul-obs"),
            pick([
                f"{environment_type.capitalize()} içinde {historical_texture_hint}, kompozisyonun etkisini büyüten ikinci katmanı kuruyor.",
                f"{relation} Özellikle {place(secondary)} tarafında kalan detaylar görüntüyü destekleyen bir derinlik veriyor.",
            ], "gul-meaning"),
            pick([issue_core, f"Özellikle {place(distraction)} tarafındaki küçük fazlalıklar toparlanırsa kompozisyon daha da temiz duyulur."], "gul-issue"),
            pick([
                "Kompozisyon amacı yakalamış ve etkili bir görüntü oluşmuş.",
                "Tonlar, netlik, doku ve ışık birlikte çalıştığı için kare daha da etkili oluyor.",
            ], "gul-close"),
        ]
        return " ".join(sentences[:5])
    ...
    if editor_name == "Sevgin Cingöz":
        sentences = [
            pick([
                f"Bu karede ilk yakalayan şey {detail_1}; modelin beden dili sahneyi hemen taşıyor.",
                f"Kadrajın etkisi kadar {detail_2} de belirleyici; ifade aktarımı ilk anda karşıya geçiyor.",
            ], "sev-open"),
            pick([
                f"{human_action.capitalize()} duygu ve {place(primary)} tarafındaki yerleşim, fotoğrafın seyir keyfini yükseltiyor.",
                f"{detail_3.capitalize()}; bu yüzden bakış yalnızca kompozisyonda kalmıyor, modelin taşıdığı an duygusuna da gidiyor.",
            ], "sev-obs"),
            pick([
                f"{light_type_detail.capitalize()} ışık ve {visual_tension} gerilim birlikte çalıştığında ifade daha görünür hale geliyor.",
                f"{relation} Özellikle {place(secondary)} tarafındaki ikinci figür ya da detay, ana etkiye destek veriyor.",
            ], "sev-meaning"),
            pick([issue_core, f"Yine de {place(distraction)} tarafındaki küçük yükler azalırsa beden dili ve ifade aktarımı daha güçlü okunur."], "sev-issue"),
            pick([
                "Etkili tonları ve ifade aktarımıyla güzel bir fotoğraf olmuş.",
                "Modelin katkısıyla seyir keyfi yüksek bir kompozisyon çıkmış.",
            ], "sev-close"),
    ...
    if editor_name == "Mürşide Çilengir":
        sentences = [
            pick([
                f"Bu karede ilk hissettiğim şey {detail_1}; sahnenin içinde saklı güçlü bir hikâye var.",
                f"Gözün gördüğünden önce {detail_3} geliyor; çünkü fotoğraf sessiz ama duygusu yüksek bir yerden konuşuyor.",
            ], "mur-open"),
            pick([
                f"{human_action.capitalize()} duygu, görüntünün insani tarafını derinleştiriyor ve sahneyi yalnızca bir kayıt olmaktan çıkarıyor.",
                f"{detail_2.capitalize()}; bu yüzden sahnede yalnızlık, masumiyet ya da kırılganlık gibi duygular daha görünür oluyor.",
            ], "mur-obs"),
            pick([
                f"{environment_type.capitalize()} içinde kalan {historical_texture_hint}, hikâyenin arka planını sessizce tamamlıyor.",
                f"{relation} Işık ve gölge birlikte çalışınca sahnenin duygusu daha etkileyici hale geliyor.",
            ], "mur-meaning"),
            pick([issue_core, "Küçük bir sadeleşmeyle bu duygusal çekirdek çok daha güçlü duyulabilir."], "mur-issue"),
            pick([
                "Işık ve gölge hikâyeyi sessizce ama çok etkileyici anlatmış.",
                "Duygu aktarımı güçlü, çok katmanlı bir fotoğraf olmuş.",
            ], "mur-close"),
        ]
        return " ".join(sentences[:5])
    ...
    if editor_name == "Gülcan Ceylan Çağın":
        ...
        sentences = [
            pick([
                ...
                f"Seçki açısından önce çalışan tarafı teslim edeyim: {verdict}",
            ], "gulcan-open"),
            pick([
                f"{detail_1.capitalize()} ve {light_type_detail} ışık, karenin yayın omurgasını hızlı kuruyor.",
                f"Ana ağırlığın {subject_position} durması ve {detail_2}, fotoğrafın ilk temasta elini güçlendiriyor.",
            ], "gulcan-obs"),
            pick([
                f"{environment_type.capitalize()} ile {historical_texture_hint}, çalışmaya çıplak estetikten öte editoryal bir zemin veriyor.",
                f"{dominant_shape.capitalize()} akış ve {visual_tension} gerilim düzeyi, seçkide tutunma ihtimalini artıran unsurlar.",
            ], "gulcan-meaning"),
            pick([issue_core, f"Özellikle {place(distraction)} tarafındaki fazlalık temizlenirse karar çok daha net olumluya döner."], "gulcan-issue"),
            pick([
                "Ben burada ciddiye alınacak bir çekirdek görüyorum; küçük bir toplamayla yayın ihtimali belirginleşir.",
                "Çalışan taraf korunup gereksiz yük ayıklanırsa kare seçki içinde rahatlıkla yer açabilir.",
            ], "gulcan-close"),
        ]
        return " ".join(sentences[:5])
...
def build_text_report(result: CritiqueResult, ai_report: Dict, dynamic_shooting_notes: List[str], dynamic_editing_notes: List[str], dynamic_strengths: List[str], dynamic_first_reading: str, dynamic_structural_reading: str, dynamic_editorial_result: str) -> str:
    ...
    lines = [
        ...
        f"Seviye / Final: {result.overall_level}",
...
def main() -> None:
    ...
    st.markdown(
        ...
        <div class="hero">
            <div style="display:flex;align-items:center;gap:18px;flex-wrap:nowrap;">
                <img src="{hero_logo_url}" style="width:120px;height:auto;display:block;flex:0 0 auto;" />
    ...
    with st.spinner(""):
        ...
        selected_comment = ""
        if isinstance(ai_report, dict) and isinstance(ai_report.get("editor_comments"), dict):
            selected_comment = str((ai_report.get("editor_comments") or {}).get(selected_editor_name) or "").strip()
        if not selected_comment:
            selected_comment = build_editor_comment(selected_editor_name, result)
        save_photo_result(image_bytes, uploaded_file.name, result, selected_editor_name, selected_comment)
