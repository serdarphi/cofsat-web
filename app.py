...
def render_photo_of_day_candidates(context: str = "main") -> None:
    ...
    rendered_dates = []
    for pool_date in [display_pool_date, upload_pool_date]:
        if pool_date in rendered_dates:
            continue
        if get_today_photo_entries(pool_date):
            _render_single_photo_pool(context, now, pool_date)
            rendered_dates.append(pool_date)
