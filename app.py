...
def _render_single_photo_pool(context: str, now: datetime, pool_date: str) -> None:
    ...
    for idx, entry in enumerate(entries[:9]):
        ...
        with col:
            ...
            if idx == 0:
                candidate_badge = "🏆 Günün Fotoğrafı" if pod.get("ready") and not is_future_pool else ("⭐ Şu An Lider" if not is_future_pool else "🕘 Ertesi Gün Lideri")
                st.markdown(f"<div class='candidate-pod-badge'>{candidate_badge}</div>", unsafe_allow_html=True)
            if entry.get("thumbnail"):
                st.image(entry["thumbnail"], use_container_width=True)
