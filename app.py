...
def summarize_candidate_levels(editor_reviews: dict) -> tuple[str, float, int]:
    values = []
    ...
    if avg >= 3.5:
        ...
        level = "zayif_aday"
    return level, round(avg, 2), len(values)
...
def save_photo_result(image_bytes: bytes, uploaded_name: str, result, editor_name: str = "", editor_comment: str = "") -> None:
    ...
    if isinstance(existing, dict):
        raw_editors = existing.get("editors", [])
        if isinstance(raw_editors, list):
            editors = [str(x).strip() for x in raw_editors if str(x).strip()]
        legacy_editor = str(existing.get("editor", "")).strip()
        if legacy_editor and legacy_editor not in editors:
            editors.append(legacy_editor)
    if editor_name and editor_name not in editors:
    ...
    merged = {
        ...
        "score": round(max(score, float((existing or {}).get("score", 0.0) or 0.0)), 2),
...
def get_today_photo_entries(selected_date: str | None = None) -> list:
    ...
    return sorted(todays, key=lambda x: (float(x.get("aggregate_candidate_value", 0)), int(x.get("editor_count", 0)), x.get("timestamp", "")), reverse=True)
...
def render_photo_of_day_candidates(context: str = "main") -> None:
    ...
    if context == "sidebar":
        ...
        st.markdown(
            f"<div class='mini-note'>Seviye: {escape(CANDIDATE_LEVEL_LABELS.get(str(pod.get('aggregate_candidate_level') or 'aday'), 'Aday'))} · Gün içi yükleme: {int(pod.get('entries', 0))}</div>",
            unsafe_allow_html=True,
        )
        if pod.get("ready"):
        ...
        for idx, entry in enumerate(entries[:6]):
            ...
            with cols[1]:
                ...
                st.caption(f"Aday seviyesi: {CANDIDATE_LEVEL_LABELS.get(str(entry.get('aggregate_candidate_level') or 'aday'), 'Aday')} · Editör: {int(entry.get('editor_count', 0))}")
        ...
        st.markdown(
            ...
            <div class='pod-meta-grid'>
                <div class='pod-meta-card'>
                    ...
                    <div class='pod-meta-value'>{escape(CANDIDATE_LEVEL_LABELS.get(str(pod.get('aggregate_candidate_level') or 'aday'), 'Aday'))}</div>
                </div>
                <div class='pod-meta-card'>
    ...
    for idx, entry in enumerate(entries[:9]):
        ...
        with col:
            ...
            st.markdown(f"<div class='candidate-score-badge'>{escape(CANDIDATE_LEVEL_LABELS.get(str(entry.get('aggregate_candidate_level') or 'aday'), 'Aday'))}</div>", unsafe_allow_html=True)
            ts = str(entry.get('timestamp', ''))
            if 'T' in ts:
                st.caption(f"Saat: {ts[11:16]}")
...
def build_facebook_caption(photo_entry: dict) -> str:
    ...
    return (
        ...
        f"Aday seviyesi: {CANDIDATE_LEVEL_LABELS.get(str(photo_entry.get('aggregate_candidate_level') or 'aday'), 'Aday')}\n\n"
