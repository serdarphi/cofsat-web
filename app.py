...
def inject_css() -> None:
    st.markdown(
        ...
        .upload-title {font-size: 1.05rem; font-weight: 700; margin-bottom: .12rem;}
        .upload-hint {color: var(--muted); font-size: .9rem; margin-bottom: .35rem;}
        ...
        .stFileUploader label {font-weight: 600 !important;}
...
def scroll_to_results() -> None:
    components.html(
        ...
        <script>
        const move = () => {
          const anchor = window.parent.document.getElementById('analysis-results-anchor');
          if (anchor) {
...
def render_restored_sections(
    result: CritiqueResult,
    selected_mode: str,
    phi_grid_img: Image.Image,
    diagonal_img: Image.Image,
    spiral_img: Image.Image,
    best_scheme: str,
    scheme_reason: str,
) -> None:
    st.markdown("---")
    ...
    with g1:
        st.markdown("### Altın oran ızgarası")
        st.image(phi_grid_img, use_container_width=True)
    with g2:
        st.markdown("### Altın diyagonaller")
        st.image(diagonal_img, use_container_width=True)

    g3, g4 = st.columns([1, 1], gap="large")
    with g3:
        st.markdown("### Altın spiral")
        st.image(spiral_img, use_container_width=True)
    with g4:
        st.markdown(
            f"""
            <div class="summary-card">
                <div class="editor-title">En uygun görünen şema</div>
                <div class="mini-note"><strong>{best_scheme}</strong><br><br>{scheme_reason}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with c1:
        st.markdown(
            ...
            <div class="panel-card">
                <div class="section-title">İlk okuma</div>
                <div class="mini-note">{result.first_reading}</div>
            </div>
            <div class="panel-card">
                <div class="section-title">Yapısal okuma</div>
                <div class="mini-note">{result.structural_reading}</div>
            </div>
            <div class="panel-card">
                <div class="section-title">Editöryel sonuç</div>
