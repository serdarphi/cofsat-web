...
QWEN_VISION_MODEL = _read_secret_or_env("QWEN_VISION_MODEL", "Qwen/Qwen2.5-VL-3B-Instruct")
...
def allow_local_fallback() -> bool:
    # Kritik: OpenAI seçiliyse sessiz yerel/sabit yoruma düşme.
    # API key, kredi, limit veya model hatası varsa gerçek hata ekranda gösterilsin.
    if openai_is_forced():
        return False
    return _env_flag("COFSAT_ALLOW_LOCAL_FALLBACK", False)
...
def _openai_api_status_probe() -> Dict[str, str]:
    """OpenAI bağlantısını küçük ve ucuz bir istekle kontrol eder.
    Amaç: key var/yok tartışması yerine gerçek HTTP hatasını görmek.
    """
    ...
    if not api_key:
        return {
            "ok": "no",
            "status": "missing_key",
            "message": "OPENAI_API_KEY okunamadı.",
            "detail": "Secrets veya Environment Variables içinde OPENAI_API_KEY görünmüyor.",
        }
    ...
    try:
        ...
        if response.status_code == 200:
            return {
                ...
                "detail": f"Model: {OPENAI_VISION_MODEL}",
            }
        return {
            "ok": "no",
...
def render_openai_api_healthcheck(stop_on_error: bool = True) -> None:
    """OpenAI zorunluysa uygulamanın en başında gerçek API durumunu gösterir.
    Böylece sistem sabit/yerel yoruma düşmez; hata görünür olur.
    """
    ...
    if status.get("ok") == "yes":
        # Sidebar'ın en üstündeki ÇOFSAT kimliğini bozmamak için
        # başarı mesajını burada ayrı bir Streamlit kutusu olarak basmıyoruz.
        # Durum bilgisi sidebar içindeki özel kartta gösteriliyor.
        return

    st.error(status.get("message", "OpenAI API kontrolü başarısız."))
    st.caption("Bu durumda ÇOFSAT yerel/sabit yoruma düşmez; gerçek hata aşağıdadır.")
...
# Girişten sonra, fotoğraf yüklenmeden önce API durumunu açıkça kontrol et.
render_openai_api_healthcheck(stop_on_error=True)
...
def _safe_provider_error_message(error_text: str) -> str:
    ...
    if "insufficient_quota" in lowered or "quota" in lowered or "billing" in lowered:
        return "OpenAI API kota/billing hatası verdi. Kredi veya proje limiti aktifleşmemiş olabilir."
    if "429" in lowered or "rate" in lowered or "too many requests" in lowered or "çok fazla istek" in lowered:
        return "OpenAI API 429 verdi: kota, rate limit veya ödeme limiti sorunu olabilir."
    if "401" in lowered or "unauthorized" in lowered or "invalid api key" in lowered:
        return "OpenAI API anahtarı doğrulanamadı. Key var ama geçersiz/iptal edilmiş olabilir."
    if "403" in lowered or "forbidden" in lowered or "project" in lowered or "organization" in lowered:
        return "OpenAI API yetki/proje/organizasyon hatası verdi."
    if "model" in lowered or "not found" in lowered or "does not exist" in lowered:
        return "OpenAI model erişim hatası verdi. OPENAI_VISION_MODEL değerini kontrol edin."
    return "OpenAI Vision çağrısı başarısız oldu. Gerçek hata Sistem Durumu panelinde görünüyor."
...
def main() -> None:
    ...
    with st.sidebar:
        _api_status = st.session_state.get("_cofsat_openai_healthcheck", {})
        _api_ok = isinstance(_api_status, dict) and _api_status.get("ok") == "yes"
        _api_label = "OpenAI API aktif" if _api_ok else "OpenAI API kontrol ediliyor"
        _api_detail = str(_api_status.get("detail", f"Model: {OPENAI_VISION_MODEL}")) if isinstance(_api_status, dict) else f"Model: {OPENAI_VISION_MODEL}"
        st.markdown(
            f"""
            <div class='sidebar-shell' style='margin-bottom:.65rem;'>
                <div class='sidebar-card sidebar-hero'>
                    <div class='sidebar-chip'>📷 ÇOFSAT</div>
                    <div class='sidebar-heading'>Fotoğraf Ön Değerlendirme</div>
                    <div class='sidebar-subtext'>Editöryal okuma, puanlama ve günün fotoğrafı alanı.</div>
                    <div class='sidebar-sep'></div>
                    <div class='sidebar-subtext'><strong>{escape(_api_label)}</strong> · {escape(_api_detail)}</div>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
