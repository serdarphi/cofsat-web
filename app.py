...
def build_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    first = result.first_reading.rstrip('.')
    structural = result.structural_reading.rstrip('.')
    editorial = result.editorial_result.rstrip('.')
    tags_text = ', '.join(result.tags[:3]).lower() if result.tags else 'bu anlatı'

    if editor_name == "Selahattin Kalaycı":
        return (
            f"Ben burada önce şunu görüyorum: {issue}.\n\n"
            f"İyi çalışan taraf da var; {strength.lower()}. Ama fotoğraf asıl kararını biraz daha net vermek zorunda.\n\n"
            f"{structural}.\n\n"
            f"Ben olsam tek hamlede şurayı toparlarım: {move}."
        )
    if editor_name == "Güler Ataşer":
        return (
            f"Bu fotoğrafın ilk duygusu bana yumuşak ama gerçek geliyor. {first}.\n\n"
            f"Özellikle {strength.lower()} Bu yüzden izleyiciyle küçük bir bağ kurabiliyor.\n\n"
            f"Yine de içindeki his tam açılmadan kalmasın diye şuraya bakmak isterim: {issue.lower()}.\n\n"
            f"Küçük ama doğru bir dokunuşla çok güzelleşir: {move}."
        )
    if editor_name == "Sevgin Cingöz":
        return (
            f"Bu kareye seçki içinden bakınca önce şu tarafı öne çıkıyor: {strength}.\n\n"
            f"Ama editoryal ağırlığın daha sağlam oturması için {issue.lower()}.\n\n"
            f"{editorial}.\n\n"
            f"Benim kısa notum şu olur: {move}."
        )
    if editor_name == "Mürşide Çilengir":
        return (
            f"Küçük bir beden, büyük duyguların ağırlığını saklıyor gibi... Bu karede sessizlik bile konuşuyor.\n\n"
            f"{first}. Gözlerinde ya da taşıdığı duruşta anlatılamayan bir hikâye hissi var.\n\n"
            f"Işık ve gölge hikâyeyi sessizce ama etkileyici biçimde taşıyor; ama fotoğrafın daha da derine işlemesi için {issue.lower()}.\n\n"
            f"Bence bu kare güçlü bir duygu taşıyor. Küçük bir dokunuşla daha da unutulmaz olabilir: {move}."
        )
    if editor_name == "Gülcan Ceylan Çağın":
        return (
            f"Bu karede kompozisyon ilk anda kendini hissettiriyor. {structural}.\n\n"
            f"Perspektif, denge ve boşluk kullanımı fotoğrafın omurgasını kurmuş; özellikle {tags_text} tarafı bunu destekliyor.\n\n"
            f"Yine de yapının daha etkili durması için {issue.lower()}.\n\n"
            f"Doğru bir son düzenlemeyle kompozisyon çok daha güçlü görünür: {move}."
        )
