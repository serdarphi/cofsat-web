...
def save_photo_result(image_bytes: bytes, uploaded_name: str, result) -> None:
    ...
    kept = []
    for item in data:
        if item.get("date") == today and item.get("image_hash") == image_hash:
            existing = item
        else:
            kept.append(item)
...
def render_photo_of_day_candidates(context: str = "main") -> None:
    ...
    if context == "sidebar":
        ...
        st.markdown(
            f"<div class='mini-note'>Nihai puan: {get_entry_display_score(pod):.1f}/100 · Gün içi yükleme: {int(pod.get('entries', 0))}</div>",
            unsafe_allow_html=True,
        )
        if pod.get("ready"):
            st.success(f"Türkiye saatine göre {PHOTO_OF_DAY_HOUR:02d}:00 sonrası en yüksek nihai puanlı fotoğraf seçildi.")
        else:
    ...
    status_text = (
        f"Türkiye saatine göre {PHOTO_OF_DAY_HOUR:02d}:00 sonrası en yüksek nihai puanlı fotoğraf otomatik olarak günün fotoğrafı oldu."
        if pod.get("ready")
...
def render_editor_score_summary_cards(editor_scores: Dict[str, float]) -> None:
    st.markdown(
        ...
        .editor-score-mini-chip {
            border-radius: 999px;
            padding: 6px 10px;
            border: 1px solid rgba(198,120,67,.20);
        ...
        .editor-score-mini-value {
            color: #b91c1c;
            font-weight: 800;
            margin-left: 6px;
        }
        .editor-score-mini-empty {
            color: #94a3b8;
            font-weight: 700;
            margin-left: 6px;
        }
        ...
        .compact-score-buttons div[data-testid="stButton"] > button {
            border-radius: 10px;
            min-height: 2.0rem;
            font-weight: 800;
            font-size: .80rem;
    ...
    for editor_name in EDITOR_NAMES:
        ...
        try:
            ...
            current_value = 0
        value_html = (
            f"<span class='editor-score-mini-value'>{current_value}</span>"
            if current_value else
            "<span class='editor-score-mini-empty'>—</span>"
        )
        chips.append(f"<div class='editor-score-mini-chip'>{editor_name}{value_html}</div>")
    st.markdown("<div class='score-row-label'>Kim ne puan verdi?</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='editor-score-mini-row'>{''.join(chips)}</div>", unsafe_allow_html=True)


def render_selected_editor_score_controls(image_hash: str, result: CritiqueResult, selected_editor_name: str) -> None:
    ...
    current_entry = None
    for item in entries:
        if isinstance(item, dict) and item.get("image_hash") == image_hash:
            current_entry = item
            break

    system_score = float(getattr(result, "total_score", 0.0) or 0.0)

    if isinstance(current_entry, dict) and isinstance(current_entry.get("editor_scores"), dict):
        editor_scores = dict(current_entry.get("editor_scores") or {})
    else:
        editor_scores = {}
    ...
    raw_current_value = editor_scores.get(selected_editor_name, 0)
    try:
        current_value = int(float(raw_current_value or 0))
    except Exception:
        current_value = 0

    if current_value:
        current_badge = f"<span class='compact-score-current'>{current_value} / 100</span>"
    else:
        current_badge = "<span class='compact-score-empty'>Henüz puan verilmedi</span>"

    st.markdown("<div class='compact-score-wrap'>", unsafe_allow_html=True)
    st.markdown(
        f"""
        <div class='compact-score-head'>
            <div class='compact-score-title'>{selected_editor_name} · Editör Puanı</div>
            <div>{current_badge}</div>
        </div>
        <div class='compact-score-meta'>
            Sistem: {system_score:.1f}/100 · Editör ort.: {editor_avg:.1f}/100 · Nihai: {final_score:.1f}/100
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown("<div class='compact-score-buttons'>", unsafe_allow_html=True)
    score_values = list(range(10, 101, 10))
    button_cols = st.columns(len(score_values), gap="small")
    for idx, score_value in enumerate(score_values):
        with button_cols[idx]:
            is_active = current_value == score_value
            clicked = st.button(
                str(score_value),
                key=f"selected_editor_score_{image_hash}_{selected_editor_name}_{score_value}",
                use_container_width=True,
                type="primary" if is_active else "secondary",
            )
            if clicked:
                update_photo_editor_score(image_hash, selected_editor_name, score_value)
                st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown(
        "<div class='score-hint-inline'>Puan vermeyi unuttuysanız daha sonra tekrar gelip buradan verebilirsiniz.</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
def render_editor_score_controls(image_hash: str, result: CritiqueResult) -> None:
    if EDITOR_NAMES:
        render_selected_editor_score_controls(image_hash, result, EDITOR_NAMES[0])


def render_editor_comments(image_hash: str, result: CritiqueResult, selected_editor_name: str, ai_report: Optional[Dict] = None) -> None:
    ai_comments = {}
    if isinstance(ai_report, dict):
        maybe_comments = ai_report.get("editor_comments")
        if isinstance(maybe_comments, dict):
            ai_comments = maybe_comments

    if selected_editor_name in EDITOR_NAMES:
        editor_name_to_show = selected_editor_name
    elif EDITOR_NAMES:
        editor_name_to_show = EDITOR_NAMES[0]
    else:
        return

    selected_comment = ai_comments.get(editor_name_to_show)
    if not isinstance(selected_comment, str) or not selected_comment.strip():
        selected_comment = build_editor_comment(editor_name_to_show, result)

    safe_comment = escape(selected_comment.strip())

    st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
    st.markdown(f"<div class='section-title'>{editor_name_to_show}</div>", unsafe_allow_html=True)
    st.markdown(
        f"<div class='mini-note' style='white-space: normal; line-height:1.8; font-size:1.02rem;'>{safe_comment}</div>",
        unsafe_allow_html=True,
    )
    render_selected_editor_score_controls(image_hash, result, editor_name_to_show)
    st.markdown("</div>", unsafe_allow_html=True)
...
def main() -> None:
    ...
    with tab2:
        ...
        render_editor_comments(current_image_hash, result, selected_editor_name, ai_report)
