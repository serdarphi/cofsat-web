...
def logo_to_data_uri(logo_path: Optional[str]) -> str:
    if not logo_path:
        return ""
    try:
        mime = "image/png"
...
def build_emotional_reading(metrics: ImageMetrics, scores_100: Dict[str, float], mode: str, editor_mode: str) -> str:
    brightness = metrics.brightness_mean
    tension = metrics.dynamic_tension_score
    contrast = metrics.contrast_std

    if brightness < 95 and tension > 0.62:
        text = "Fotoğrafın genel hissi daha çok gergin, içe dönük ve yoğun çalışıyor; karanlık tonlar ve kadraj içi gerilim bu duyguyu besliyor."
    elif brightness < 115:
        text = "Fotoğrafta sakin ama hafif ağır bir atmosfer var; tonların bastırılmış yapısı izleyiciyi daha yavaş ve dikkatli bakmaya çağırıyor."
    elif contrast < 38 and scores_100["duygusal_yogunluk"] >= 65:
        text = "Kare sert çarpmaktan çok yumuşak bir yakınlık kuruyor; daha nazik ton geçişleri duyguyu bağırmadan taşıyor."
    elif tension > 0.68:
        text = "Bu karede hareket ve huzursuzluk hissi belirgin; göz merkezde kalmak yerine kadraj içinde dolaşmaya zorlanıyor."
    else:
        text = "Fotoğrafın duygusu dengeli; ne tamamen sessiz ne de aşırı gergin, daha çok kontrollü bir iz bırakıyor."

    if mode == "Portre":
        text += " Portrede bu hissin inandırıcı olabilmesi için yüz ve bakış ilişkisinin temiz kalması çok önemli."
    elif mode == "Sokak":
        text += " Sokak fotoğrafında bu duygu en çok zamanlama ve katman ilişkisiyle güç kazanır."
    return tone_text(text, editor_mode)


CRITIQUE_DIAGNOSIS_MAP = {
    "ilk_etki": {
        "summary": "Bu karede ana problem ilk bakış etkisinin yeterince hızlı kurulamaması.",
        "effect": "İzleyici fotoğrafta kalmadan önce güçlü bir tutunma noktası arıyor; bu da karenin akılda kalıcılığını düşürüyor.",
        "cause": "Giriş etkisini taşıyacak ton, ritim veya odak vurgusu henüz yeterince belirgin değil.",
        "fix": "Ana özneyi ya da ana geometrik ilişkiyi ilk bakışta okunur hale getirecek bir kontrast, sadeleştirme veya kırpma kararı ver."
    },
    "teknik_butunluk": {
        "summary": "Bu karede ana problem teknik bütünlüğün anlatıyı tam taşıyamaması.",
        "effect": "Fotoğrafın niyeti hissedilse bile ton veya netlik kararsızlığı güven duygusunu azaltıyor.",
        "cause": "Pozlama, kontrast veya netlik ilişkisi bazı bölgelerde birbirini desteklemek yerine dağıtıyor.",
        "fix": "Highlight ve gölge dengesini toparla; ardından ana özne çevresinde kontrollü bir netlik ve lokal kontrast desteği ver."
    },
    "kompozisyon": {
        "summary": "Bu karede ana problem kompozisyon iskeletinin tam sıkı kurulmaması.",
        "effect": "Göz kadraj içinde dolaşıyor ama karar verdiğin görsel rotayı net biçimde takip edemiyor.",
        "cause": "Boşluklar, kenar ağırlıkları veya çizgi ilişkileri ana yapıyı tam desteklemiyor.",
        "fix": "Kadraj kenarlarını yeniden değerlendir; ana çizgiyi güçlendiren bir kırpma ve gereksiz yan ağırlıkları azaltan bir sadeleştirme uygula."
    },
    "odak_ve_hiyerarsi": {
        "summary": "Bu karede ana problem gözün net bir ana odak bulmakta zorlanması.",
        "effect": "İzleyici nereye bakacağını biliyor gibi başlasa da fotoğrafta uzun süre kalmak için yeterli görsel sebep bulamıyor.",
        "cause": "Ana özne ile çevresi arasında parlaklık, kontrast veya detay ayrımı yeterince açık kurulmamış.",
        "fix": "Ana öznenin olduğu bölgeyi lokal kontrast ve ton ayrımıyla öne çıkar; çevredeki rekabet eden alanları yarım adım geri çek."
    },
    "anlati_gucu": {
        "summary": "Bu karede ana problem anlatının tam açılmaması.",
        "effect": "Fotoğraf teknik olarak ayakta kalsa bile izleyicide ikinci bir anlam katmanı bırakmakta zorlanıyor.",
        "cause": "Sahne içinde hissi taşıyan jest, ilişki veya bağlam yeterince görünür değil.",
        "fix": "Anlatıyı taşıyan insan, nesne ya da ilişkiyi daha görünür kıl; gereksiz bağlamsal kalabalığı azaltıp hikâyenin merkezini netleştir."
    },
    "gorsel_dil": {
        "summary": "Bu karede ana problem biçimsel tercihlerin ortak bir görsel dile tam dönüşmemesi.",
        "effect": "İzleyici fotoğrafın ne hissettirdiğini sezse de bu hissi taşıyan biçimsel tutarlılığı tam göremiyor.",
        "cause": "Ton, ritim, boşluk ve yapı tercihleri aynı karaktere hizmet edecek kadar birleşmemiş.",
        "fix": "Tek bir karakter seç: daha sert, daha yumuşak, daha grafik ya da daha şiirsel; sonra ton ve kadraj kararlarını o karaktere göre hizala."
    },
    "sadelik": {
        "summary": "Bu karede ana problem gereksiz görsel yükün ana fikrin önüne geçmesi.",
        "effect": "Fotoğrafın etkisi var ama göz, asıl hikâyeyle gereksiz detaylar arasında enerji kaybediyor.",
        "cause": "Kenarlarda veya arka planda gereğinden fazla dikkat çeken küçük ağırlık merkezleri var.",
        "fix": "Kırpma, ton bastırma veya seçici karartmayla dikkat çeken yan alanları azalt; tek bir merkez hissi kur."
    },
    "niyet_tutarliligi": {
        "summary": "Bu karede ana problem niyetin izleyiciye yeterince açık geçmemesi.",
        "effect": "Fotoğraf estetik olarak okunuyor ama neden çekildiği tam berraklaşmayınca etkisi dağılabiliyor.",
        "cause": "Teknik ve kompozisyon kararları aynı anlatısal karara tam bağlanmıyor.",
        "fix": "Çekim niyetini tek cümlede tarif et ve kadrajdaki her öğeyi bu niyete hizmet edip etmediğine göre yeniden ele."
    },
    "isik_yonu": {
        "summary": "Bu karede ana problem ışığın ana anlatıyı yeterince taşımaması.",
        "effect": "Göz sahneyi görüyor ama hangi bölgenin daha önemli olduğunu ışık üzerinden hızlıca ayırt edemiyor.",
        "cause": "Aydınlık-koyu ayrımı, özne ve çevre arasında hiyerarşi kuracak kadar kararlı değil.",
        "fix": "Özneyi taşıyan ana ışık yönünü güçlendir; rekabet eden parlak bölgeleri kıs ve ana alanı yarım stop kadar belirginleştir."
    },
    "derinlik_hissi": {
        "summary": "Bu karede ana problem derinlik duygusunun sınırlı kalması.",
        "effect": "Fotoğraf iki boyutlu hissediliyor ve izleyici kadrajın içine girmek yerine yüzeyde kalıyor.",
        "cause": "Ön plan, orta plan ve arka plan arasında ton ya da netlik ayrımı yeterince kurulmamış.",
        "fix": "Katmanları ton ve kontrastla ayır; ön planı ya referans ya da perde olarak kullanıp orta planı esas anlatı bölgesi haline getir."
    },
    "dikkat_dagitici_unsurlar": {
        "summary": "Bu karede ana problem dikkat dağıtan alanların ana özneyle yarışması.",
        "effect": "İzleyici doğru yere baksa bile yan bölgelerdeki parlaklık veya detay enerjisi odağı zayıflatıyor.",
        "cause": "Özellikle kenar ve arka plan bölgelerinde gereğinden güçlü ikinci odaklar oluşmuş.",
        "fix": "Dikkat kaçaklarını ton olarak bastır, gerekiyorsa kırp ve ana odak dışında kalan parlak yüzeyleri geri çek."
    },
    "zamanlama": {
        "summary": "Bu karede ana problem anın en güçlü noktada yakalanmamış hissi vermesi.",
        "effect": "Sahne çalışıyor ama tam zirve anı hissi gelmediği için fotoğrafın vurucu etkisi azalıyor.",
        "cause": "Jest, bakış, adım ya da sahne içi ilişki en açık birleşim anına tam oturmamış.",
        "fix": "Aynı sahne tipinde yarım saniye önce ve sonrası için daha sabırlı bekleme pratiği kur; mikro zamanlama bu kareyi ciddi biçimde büyütür."
    },
    "negatif_alan": {
        "summary": "Bu karede ana problem boşluk kullanımının tam kararında oturmaması.",
        "effect": "Boşluk nefes alanı yaratmak yerine bazen kararsız ya da amaçsız hissedilebiliyor.",
        "cause": "Negatif alan ana özneyle anlamlı gerilim kurmak yerine yalnızca boş kalmış gibi görünüyor.",
        "fix": "Boşluğu ya daha cesur kullan ya da daralt; boş alanın ana özneye yön, yalnızlık veya denge katmasını hedefle."
    },
    "duygusal_yogunluk": {
        "summary": "Bu karede ana problem duygusal etkinin izleyiciye yeterince direkt geçmemesi.",
        "effect": "Fotoğraf düzgün görünse de uzun süre akılda kalacak duygusal iz biraz zayıflıyor.",
        "cause": "Yüz, jest, ışık ya da sahne ilişkisi duyguyu taşıyacak kadar açık yoğunlaşmamış.",
        "fix": "Duygunun merkezindeki alanı daha temiz, daha görünür ve daha az rekabetli hale getir; özellikle yüz, el veya jest hattını güçlendir."
    },
    "editoryal_deger": {
        "summary": "Bu karede ana problem editoryal ağırlığın tam oluşmaması.",
        "effect": "Fotoğraf tek başına hoş görünse de bir seçki içinde güçlü bir cümle gibi durmakta zorlanıyor.",
        "cause": "Karede fikir var ama seçki mantığında öne çıkacak kadar net bir vurgu veya özgünlük henüz açılmamış.",
        "fix": "Fotoğrafın haber, hikâye veya seri içindeki rolünü düşünerek daha net bir seçim yap; genel güzellikten çok özgül cümleyi güçlendir."
    },
    "tekrar_bakma_istegi": {
        "summary": "Bu karede ana problem ikinci bakışı çağıracak katmanın sınırlı olması.",
        "effect": "Fotoğraf ilk anda çalışsa bile izleyici tekrar dönmek için yeterli keşif alanı bulmayabiliyor.",
        "cause": "İlk okumanın arkasında yeni detay, ritim veya anlam katmanı yeterince güçlü değil.",
        "fix": "Ana etkiyi bozmayacak ikinci bir katman kur; küçük bir jest, arka plan ilişkisi ya da ton ritmi ikinci bakışı davet eder."
    },
}


def build_advanced_critique(scores_100: Dict[str, float], metrics: ImageMetrics, mode: str, editor_mode: str) -> Tuple[str, str, str, str]:
    weakest_key = min(scores_100.items(), key=lambda x: x[1])[0]
    base = CRITIQUE_DIAGNOSIS_MAP.get(weakest_key, CRITIQUE_DIAGNOSIS_MAP["odak_ve_hiyerarsi"])

    effect = base["effect"]
    cause = base["cause"]
    fix = base["fix"]

    if weakest_key in {"odak_ve_hiyerarsi", "dikkat_dagitici_unsurlar"} and metrics.highlight_clip_ratio > 0.025:
        cause += " Parlak bölgelerin kontrolsüz kalması bu rekabeti daha da artırıyor."
        fix += " Özellikle parlak yüzeyleri geri çekmen hızlı sonuç verir."
    if weakest_key == "teknik_butunluk" and metrics.shadow_clip_ratio > 0.08:
        cause += " Gölge tarafında bilgi kaybı hissi de buna eşlik ediyor."
    if weakest_key == "derinlik_hissi" and metrics.edge_density < 0.06:
        cause += " Katmanlar arasındaki mikrokontrast düşük kaldığı için hacim duygusu zayıflıyor."
    if weakest_key == "kompozisyon" and abs(metrics.center_of_mass_x - 0.5) < 0.08 and abs(metrics.center_of_mass_y - 0.5) < 0.08:
        cause += " Ağırlık merkezi fazla ortada kaldığında kadraj daha az kararlı ve daha az dinamik hissedilebiliyor."

    return (
        tone_text(base["summary"], editor_mode),
        tone_text(effect, editor_mode),
        tone_text(cause, editor_mode),
        tone_text(fix, editor_mode),
    )
...
def critique_image(image_bytes: bytes, mode: str, editor_mode: str) -> CritiqueResult:
    ...
    diagnosis_summary, diagnosis_effect, diagnosis_cause, diagnosis_fix = build_advanced_critique(scores_100, metrics, mode, editor_mode)
    emotional_reading = build_emotional_reading(metrics, scores_100, mode, editor_mode)
    ...
    return CritiqueResult(
        ...
        diagnosis_effect=diagnosis_effect,
...
def inject_css() -> None:
    st.markdown(
        ...
        .hero-logo {width:82px; height:auto; display:block; object-fit:contain;}
        ...
        .insight-grid {display:grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: .9rem;}
        .insight-card {background: rgba(255,255,255,.05); border:1px solid rgba(255,255,255,.1); border-radius:18px; padding:1rem; min-height: 140px;}
        .insight-kicker {font-size:.8rem; text-transform:uppercase; letter-spacing:.08em; color: var(--accent); margin-bottom:.35rem;}
        .insight-text {color: var(--text); line-height:1.6; font-size:.96rem;}
        .focus-read {display:grid; grid-template-columns: repeat(3, minmax(0,1fr)); gap:.8rem;}
        .focus-chip {background: rgba(255,255,255,.05); border:1px solid rgba(255,255,255,.1); border-radius:16px; padding:.8rem;}
        .focus-chip strong {display:block; margin-bottom:.28rem;}
        .footer-trademark {color: rgba(255,255,255,.78); text-align:center; font-size:.92rem; margin-top:.7rem; letter-spacing:.04em;}
        ...
        div[data-baseweb="popover"] [role="option"][aria-selected="true"] {
            background: rgba(255,140,66,0.30) !important;
            color: #111111 !important;
        ...
        section[data-testid="stSidebar"] .ghost-badge,
        section[data-testid="stSidebar"] .hero-badge {
            color: #111111 !important;
        ...
        div[data-baseweb="popover"] [aria-selected="true"] {
            background: rgba(255,140,66,.30) !important;
            color: #111111 !important;
        }
...
def build_curatorial_diagnosis(result: CritiqueResult, selected_mode: str) -> List[Tuple[str, str]]:
    scores = result.rubric_scores

    if scores["anlati_gucu"] >= 72:
        narrative = "Kare yalnızca bir anı kaydetmiyor; izleyicinin zihninde küçük bir hikâye alanı da açıyor. Bu yüzden fotoğrafın en güçlü tarafı yalnızca teknik düzeni değil, taşıdığı niyet hissi."
    elif scores["anlati_gucu"] >= 58:
        narrative = "Fotoğrafta bir anlatı çekirdeği var; ancak bu çekirdeğin daha net görünmesi için bazı bölgelerin geri çekilmesi ve ana kararın daha açık hale gelmesi gerekiyor."
    else:
        narrative = "Bu kare şimdilik daha çok görsel bilgi veriyor. Dünya çapında bir iş hissi için, izleyicinin neye tutunacağını ilk birkaç saniyede daha net söylemesi gerekiyor."

    if scores["kompozisyon"] >= 72 and scores["odak_ve_hiyerarsi"] >= 70:
        structure = "Kadrajın iskeleti ile odak hiyerarşisi birbirini destekliyor. Yani göz, fotoğraf içinde rastgele dolaşmıyor; davet edildiği bir rota buluyor."
    elif scores["kompozisyon"] >= 60:
        structure = "Kadrajda çalışmaya başlayan bir iskelet var. Fakat dünya seviyesi bir görünüm için, ana öznenin etrafındaki görsel ağırlık biraz daha disiplinli dağıtılmalı."
    else:
        structure = "Kompozisyon henüz tam sıkılaşmamış görünüyor. Bu da iyi fikir taşıyan karenin etkisini olduğundan daha zayıf gösterebilir."

    if selected_mode == "Portre":
        editorial = "Portre tarafında bir üst seviyeye çıkmak için yüz, göz ve çevre arasındaki ilişki daha rafine kurulmalı; arka plan özneyi taşımaya hizmet etmeli, onunla yarışmamalı."
    elif selected_mode == "Sokak":
        editorial = "Sokak okumasında dünya seviyesine yaklaşan kareler, an ile katmanı aynı anda taşır. Burada zamanlama ve sahne düzeni biraz daha senkron çalıştığında fotoğraf çok daha sert vuracaktır."
    elif selected_mode == "Belgesel":
        editorial = "Belgesel okumada asıl güç, bağlamın görünürlüğü ile görsel dürüstlüğün aynı anda hissedilmesidir. Bu kare o çizgiye yaklaşmış; daha net seçimlerle editoryal ağırlığı artar."
    else:
        editorial = "Soyut okumada asıl değer, biçim kararlarının ortak bir dil üretmesidir. Fotoğraf bu yönde potansiyel taşıyor; daha tutarlı ton ve ağırlık dağılımı ile çok daha rafine görünür."

    return [
        ("Anlatı çekirdeği", narrative),
        ("Kompozisyon kararı", structure),
        ("Editoryal sıçrama", editorial),
    ]


def build_focus_explainer(main_points: List[Tuple[int, int]], distraction_points: List[Tuple[int, int]], image_size: Tuple[int, int]) -> List[Tuple[str, str]]:
    w, h = image_size

    def zone_name(x: int, y: int) -> str:
        hx = "sol" if x < w/3 else ("sağ" if x > 2*w/3 else "orta")
        hy = "üst" if y < h/3 else ("alt" if y > 2*h/3 else "merkez")
        return f"{hy} {hx}"

    primary = zone_name(*main_points[0]) if main_points else "merkez"
    secondary = ", ".join(zone_name(x, y) for x, y in main_points[1:3]) if len(main_points) > 1 else "ikincil vurgu zayıf"
    distraction = ", ".join(zone_name(x, y) for x, y in distraction_points[:2]) if distraction_points else "belirgin dikkat kaçağı yok"

    return [
        ("Ana odak", f"En baskın görsel ağırlık {primary} bölgesinde toplanıyor. İzleyicinin ilk bakışta tutunduğu yer büyük olasılıkla burası."),
        ("Göz akışı", f"Göz, ana odaktan sonra şu bölgelere sıçrama eğilimi gösteriyor: {secondary}. Bu, kadraj içi dolaşımın mevcut rotasını tarif eder."),
        ("Dikkat kaçağı", f"Ana anlatının enerjisini bölebilecek bölgeler: {distraction}. Bu alanları ton veya kadraj açısından biraz geri itmek okuma gücünü artırır."),
    ]
...
def main() -> None:
    ...
    logo_uri = logo_to_data_uri(logo_file)
    logo_html = f'<div class="hero-logo-wrap"><img class="hero-logo" src="{logo_uri}" alt="ÇOFSAT logo"></div>' if logo_uri else ""
            ...
            <div class="hero-head">
                ...
                <div class="hero-copy">
                    ...
                    <p>
                        ...
                        daha profesyonel bir okuma deneyimi sunan; eleştiri kalitesini, görsel rehberleri ve karar netliğini birlikte yükselten arayüz.
                    </p>
    ...
    components.html(
        ...
        <script>
        const go = () => {
          const el = window.parent.document.getElementById('analysis-results');
    ...
    st.markdown("### Gelişim Odaklı Eleştiri Motoru")
    a1, a2 = st.columns(2, gap="large")
    with a1:
        st.markdown("<div class='panel-card'><div class='section-title'>Ana teşhis</div>", unsafe_allow_html=True)
        st.write(result.diagnosis_summary)
        st.markdown("<div class='section-title'>İzleyiciye etkisi</div>", unsafe_allow_html=True)
        st.write(result.diagnosis_effect)
        st.markdown("</div>", unsafe_allow_html=True)
    with a2:
        st.markdown("<div class='panel-card'><div class='section-title'>Teknik neden</div>", unsafe_allow_html=True)
        st.write(result.diagnosis_cause)
        st.markdown("<div class='section-title'>Net çözüm</div>", unsafe_allow_html=True)
        st.write(result.diagnosis_fix)
        st.markdown("</div>", unsafe_allow_html=True)

    st.markdown(
        f"<div class='panel-card'><div class='section-title'>Duygusal okuma</div><div class='mini-note'>{result.emotional_reading}</div></div>",
        unsafe_allow_html=True,
    )

    diagnosis = build_curatorial_diagnosis(result, selected_mode)
    focus_read = build_focus_explainer(main_points, distraction_points, image.size)

    st.markdown("### Profesyonel Okuma Özeti")
    d1, d2 = st.columns([1.15, 0.85], gap="large")
    with d1:
        st.markdown("<div class='panel-card'><div class='section-title'>Küratöryel değerlendirme</div><div class='insight-grid'>", unsafe_allow_html=True)
        for title, body in diagnosis:
            st.markdown(
                f"<div class='insight-card'><div class='insight-kicker'>{title}</div><div class='insight-text'>{body}</div></div>",
                unsafe_allow_html=True,
            )
        st.markdown("</div></div>", unsafe_allow_html=True)
    with d2:
        st.markdown("<div class='panel-card'><div class='section-title'>Odak okuması</div><div class='focus-read'>", unsafe_allow_html=True)
        for title, body in focus_read:
            st.markdown(
                f"<div class='focus-chip'><strong>{title}</strong><span class='mini-note'>{body}</span></div>",
                unsafe_allow_html=True,
            )
        st.markdown("</div></div>", unsafe_allow_html=True)

    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["Görsel Katmanlar", "Editör Okuması", "Oranlar", "Altın Oran", "Teknik"]
    )
    ...
    with tab2:
        ...
        st.markdown("<div class='panel-card'><div class='section-title'>Teşhis → Etki → Çözüm</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='mini-note'><strong>Teşhis:</strong> {result.diagnosis_summary}</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='mini-note'><strong>Etkisi:</strong> {result.diagnosis_effect}</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='mini-note'><strong>Çözüm:</strong> {result.diagnosis_fix}</div></div>", unsafe_allow_html=True)
    ...
    s1, s2, s3 = st.columns(3, gap="large")
    with s1:
        st.markdown("<div class='panel-card'><div class='section-title'>Altın oran ızgarası</div>", unsafe_allow_html=True)
        st.image(phi_grid_img, use_container_width=True)
        st.markdown("<div class='mini-note'>Ana odak, kesişim noktalarına ne kadar yaklaşıyorsa ızgara o kadar işlevsel görünür.</div></div>", unsafe_allow_html=True)
    with s2:
        st.markdown("<div class='panel-card'><div class='section-title'>Altın diyagonaller</div>", unsafe_allow_html=True)
        st.image(diagonal_img, use_container_width=True)
        st.markdown("<div class='mini-note'>Kadraj içi yön, eğim ve hareket hissini değerlendirmek için diyagonal yapı hızlı okuma sağlar.</div></div>", unsafe_allow_html=True)
    with s3:
        st.markdown("<div class='panel-card'><div class='section-title'>Altın spiral</div>", unsafe_allow_html=True)
        st.image(spiral_img, use_container_width=True)
        st.markdown(f"<div class='mini-note'><strong>Öneri:</strong> {best_scheme}. {scheme_reason}</div></div>", unsafe_allow_html=True)

    st.markdown("### Editör Okuması ve Eleştiriler")
    ...
    with e2:
        ...
        st.markdown("<div class='section-title'>Editöryel sonuç</div>", unsafe_allow_html=True)
        st.write(result.editorial_result)
        st.markdown("<div class='section-title'>Çekim notları</div>", unsafe_allow_html=True)
        for n in result.shooting_notes:
            st.markdown(f"- {n}")
        st.markdown("<div class='section-title'>Düzenleme notları</div>", unsafe_allow_html=True)
        for n in result.editing_notes:
            st.markdown(f"- {n}")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        <div class="footer-trademark">Serdar Bayram™</div>
        """,
        unsafe_allow_html=True,
    )
