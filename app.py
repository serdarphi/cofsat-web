...
def _character_opening(level: int, editor_name: str) -> str:
    if editor_name == "Selahattin Kalaycı":
        mapping = {
            1: "Bence fotoğrafın derdi okunuyor ama cümlesi henüz tam sıkı değil.",
            2: "Fotoğraf bir şey söylüyor; yine de kararlarını biraz daha net duymak istiyorum.",
            3: "Fotoğrafın kuvveti var ama kırıldığı yerler de açıkça görünüyor.",
            4: "Fotoğraf iyi niyetle kalmasın; neyi savunduğunu daha sert kurması gerekiyor.",
            5: "Fotoğrafın bahaneye değil, daha net karara ihtiyacı var.",
        }
    elif editor_name == "Güler Ataşer":
        mapping = {
            1: "Burada usulca çalışan bir his var ve onu tamamen kaybetmemek önemli.",
            2: "Fotoğraf izleyiciye yumuşak bir kapı açıyor; o kapıyı biraz daha görünür kılabilir.",
            3: "İlk duygusu samimi, ama bu samimiyetin daha iyi taşınabileceği yerler var.",
            4: "Fotoğrafın duygusu güzel; yine de ona eşlik etmeyen bölgeler hissi dağıtıyor.",
            5: "Buradaki asıl his kıymetli, ama fotoğraf bunu yeterince koruyamıyorsa açık söylemek gerekir.",
        }
    else:
        mapping = {
            1: "Kare editoryal zeminde nazikçe çalışan bir öneri sunuyor.",
            2: "Fotoğrafın yapısı okunuyor; fakat seçki içinde daha belirgin bir pozisyon isteyebilir.",
            3: "Bu karede iyi bir editoryal çekirdek var, ama bütün kararlar aynı güçte değil.",
            4: "Seçkiye girecekse yapısal kararlarını daha ikna edici hale getirmesi gerekir.",
            5: "Editoryal açıdan bakınca, bu fotoğrafın cümlesi henüz tam keskinleşmemiş.",
        }
    return mapping.get(int(level), mapping[3])
...
def build_editor_comment(result: CritiqueResult, editor_name: str, selected_character_level: int) -> str:
    ...
    scene_type = profile.get("scene_type", result.suggested_mode or "Genel")
    ...
    strength = result.key_strength.rstrip(".")
    issue = result.key_issue.rstrip(".")
    move = result.one_move_improvement.rstrip(".")
    structural = soften_for_editor(result.structural_reading).rstrip(".")
    editorial = soften_for_editor(result.editorial_result).rstrip(".")
    first_reading = soften_for_editor(result.first_reading).rstrip(".")

    if editor_name == "Selahattin Kalaycı":
        lines = [
            opening,
            f"Ben bu fotoğrafta önce {primary_region} tarafına tutunuyorum; çünkü ana ağırlık {subject_hint} üzerinden kurulmuş.",
            f"Ama hemen ardından sorun görünmeye başlıyor: {issue}.",
            f"Yapısal tarafta durum şu: {structural}.",
            f"{distraction_region.capitalize()} bölgesi gereğinden fazla konuştuğunda fotoğrafın ana cümlesi zayıflıyor ve {eye_flow}.",
            f"Benim için buradaki tek kritik hamle şu: {move}.",
        ]
        if selected_character_level >= 4:
            lines.append("Kısacası fotoğrafın potansiyeli var, ama potansiyel tek başına yetmiyor; kararını daha sert vermesi gerekiyor.")
        else:
            lines.append(f"İyi çalışan tarafı da var: {strength}.")
        return "\n\n".join(lines)

    if editor_name == "Güler Ataşer":
        lines = [
            opening,
            f"Fotoğraf bende önce {mood} bir hava bırakıyor ve bakışı doğal olarak {primary_region} bölgesine taşıyor.",
            f"Orada hissettiğim şey şu: {strength}.",
            f"Bu yüzden fotoğraf izleyiciyle bağ kurabiliyor; özellikle {subject_hint} çevresinde gerçek bir temas oluşuyor.",
            f"Yine de {distraction_region} tarafı devreye girdiğinde bu temas hafifçe dağılıyor ve {issue.lower()}.",
            f"Işığın {light_character} karakteri güzel bir temel veriyor. Bence fotoğrafı daha da inceltecek dokunuş şu olur: {move}.",
        ]
        if selected_character_level >= 4:
            lines.append("Ben burada sert olmak istemem ama şunu da saklamam: fotoğrafın duygusu var, fakat o duyguyu taşıyamayan bölgeler geri çekilmezse etkisi yarım kalır.")
        else:
            lines.append("Küçük bir sadeleşmeyle bu fotoğraf çok daha içten ve kalıcı bir etki bırakabilir.")
        return "\n\n".join(lines)

    lines = [
        opening,
        f"Bu kareyi {scene_type.lower()} damarında okuyorum; çünkü ana kurgu {subject_hint} etrafında toplanıyor.",
        f"İlk editoryal okuma şöyle başlıyor: {first_reading}.",
        f"Ardından yapısal tablo netleşiyor: {structural}.",
        f"Seçki mantığında baktığımda karar düğümü {primary_region} ile {distraction_region} arasındaki ağırlık dağılımında oluşuyor.",
        f"Bu fotoğrafın editoryal karşılığı benim için şu cümlede toplanıyor: {editorial}.",
        f"Dolayısıyla öncelikli müdahale {move}.",
    ]
    if result.suggested_mode:
        lines.append(f"Ben bu fotoğrafı özellikle {result.suggested_mode.lower()} seçki içinde, doğru eşleşmelerle daha güçlü çalışırken görüyorum.")
    return "\n\n".join(lines)
