...
def render_editor_score_summary_cards(editor_scores: Dict[str, float]) -> None:
    ...
    st.markdown("<div class='mini-note' style='margin-top:.65rem;'>Kim ne puan verdi?</div>", unsafe_allow_html=True)
...
def render_selected_editor_score_controls(image_hash: str, result: CritiqueResult, selected_editor_name: str) -> None:
    ...
    current_value = int(float(editor_scores.get(selected_editor_name, 0) or 0))
    current_badge = (
        f"<span class='compact-score-current'>{current_value} / 100</span>"
        if current_value else
        "<span class='compact-score-empty'>Henüz puan verilmedi</span>"
    )
    ...
    st.markdown(
        "<div class='mini-note' style='margin-top:.45rem;'>Puan vermeyi unuttuysanız daha sonra tekrar gelip buradan verebilirsiniz.</div>",
        unsafe_allow_html=True,
    )
...
def render_editor_score_controls(image_hash: str, result: CritiqueResult) -> None:
    # Eski genel puan paneli yerinde kalsın isteyen durumlar için,
    # ama ana akış artık seçili editörün yorumunun altında çalışıyor.
    render_selected_editor_score_controls(image_hash, result, EDITOR_NAMES[0] if EDITOR_NAMES else "")
...
def render_editor_comments(image_hash: str, result: CritiqueResult, selected_editor_name: str, ai_report: Optional[Dict] = None) -> None:
    ...
    st.markdown("<div class='panel-card'>", unsafe_allow_html=True)
    st.markdown(f"<div class='section-title'>{selected_editor_name}</div>", unsafe_allow_html=True)
    st.markdown(
        f"<div class='mini-note' style='white-space: normal; line-height:1.8; font-size:1.02rem;'>{escape(comment_for(selected_editor_name))}</div>",
        unsafe_allow_html=True,
    )
    render_selected_editor_score_controls(image_hash, result, selected_editor_name)
    st.markdown("</div>", unsafe_allow_html=True)
