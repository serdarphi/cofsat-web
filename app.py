...
def draw_phi_grid(img: Image.Image) -> Image.Image:
    out = Image.blend(img.convert("RGB"), Image.new("RGB", img.size, (22, 28, 36)), 0.10).convert("RGBA")
    overlay = Image.new("RGBA", out.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay)
    w, h = out.size
    ...
    for x in (x1, x2):
        draw.line((x, 0, x, h), fill=(8, 12, 18, 110), width=7)
        draw.line((x, 0, x, h), fill=(112, 203, 255, 250), width=3)
    for y in (y1, y2):
        draw.line((0, y, w, y), fill=(8, 12, 18, 110), width=7)
        draw.line((0, y, w, y), fill=(112, 203, 255, 250), width=3)

    for x in (x1, x2):
        for y in (y1, y2):
            draw.ellipse((x - 10, y - 10, x + 10, y + 10), fill=(8, 12, 18, 130))
            draw.ellipse((x - 6, y - 6, x + 6, y + 6), fill=(112, 203, 255, 255))

    return Image.alpha_composite(out, overlay).convert("RGB")


def draw_golden_diagonals(img: Image.Image) -> Image.Image:
    out = Image.blend(img.convert("RGB"), Image.new("RGB", img.size, (28, 24, 20)), 0.08).convert("RGBA")
    overlay = Image.new("RGBA", out.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay)
    w, h = out.size
    x1, x2, y1, y2 = phi_grid_positions(w, h)
    ...
    for i, line in enumerate(lines):
        draw.line(line, fill=(18, 10, 6, 95), width=7 if i < 2 else 5)
        draw.line(line, fill=(255, 154, 82, 238 if i < 2 else 205), width=3 if i < 2 else 2)

    return Image.alpha_composite(out, overlay).convert("RGB")


def draw_golden_spiral(img: Image.Image) -> Image.Image:
    out = Image.blend(img.convert("RGB"), Image.new("RGB", img.size, (18, 28, 18)), 0.10).convert("RGBA")
    overlay = Image.new("RGBA", out.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay)
    w, h = out.size
    ...
    for i, box in enumerate(boxes[:-1]):
        ...
        if phase == 0:
            ...
            start, end = 0, 90
        draw.arc((x1, y1, x2, y2), start=start, end=end, fill=(10, 18, 10, 115), width=8)
...
def inject_css() -> None:
    st.markdown(
        ...
        .stApp {
            background:
                radial-gradient(circle at 0% 0%, rgba(104,213,255,0.14), transparent 25%),
                radial-gradient(circle at 100% 0%, rgba(248,214,109,0.12), transparent 22%),
        ...
        [data-testid="stSidebar"] {
            background: linear-gradient(180deg, rgba(8,14,28,0.95), rgba(15,24,47,0.92));
        ...
        .hero {
            ...
            background:
                ...
                linear-gradient(135deg, rgba(104,213,255,0.08), rgba(248,214,109,0.06));
        ...
        .rubric-row {margin-bottom: .75rem;}
        .rubric-head {
            display:flex;
        ...
        .rubric-fill {
            height: 100%;
            border-radius:999px;
            background: linear-gradient(90deg, rgba(104,213,255,0.98), rgba(248,214,109,0.98));
            box-shadow: 0 0 20px rgba(104,213,255,0.24);
        }
        .upload-panel {
            text-align:center;
            ...
            background:
                ...
                radial-gradient(circle at top, rgba(104,213,255,0.07), transparent 62%);
        }
        .upload-icon {
            width: 52px;
            height: 52px;
            ...
            font-size: 1.45rem;
            background: linear-gradient(135deg, rgba(104,213,255,0.22), rgba(248,214,109,0.22));
        ...
        section[data-testid="stFileUploaderDropzone"] {
            border: 1.5px dashed rgba(15,23,42,0.18) !important;
        ...
        section[data-testid="stFileUploaderDropzone"]:hover {
            ...
            border-color: rgba(104,213,255,0.55) !important;
            box-shadow:
                0 24px 55px rgba(0,0,0,0.18),
                0 0 0 4px rgba(104,213,255,0.10) !important;
        }
        section[data-testid="stFileUploaderDropzone"] * {
        ...
        .scheme-name {
            ...
            padding:.35rem .6rem;
            border-radius:999px;
            border:1px solid rgba(104,213,255,.25);
...
def main() -> None:
    ...
    st.markdown(
        ...
        <div class="upload-panel">
            ...
            <div class="upload-hint">Dosyanızı sürükleyip bırakın ya da kutunun içinden dosya seçin. Temiz, hızlı ve tek adımlı yükleme akışı için bu alan sade tutuldu.</div>
            <div class="upload-pills">
                <span class="upload-pill">JPG</span>
                <span class="upload-pill">PNG</span>
                <span class="upload-pill">WEBP</span>
                <span class="upload-pill">TIFF</span>
                <span class="upload-pill">Heatmap + Göz Akışı</span>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
