...
def render_sidebar(selected_mode: str, selected_editor_mode: str, selected_editor_name: str) -> None:
    with st.sidebar:
        ...
        st.markdown(f"<div class='mini-note'>Fotoğraf yüklendiğinde aşağıda seçili editörün anlık yorumu da görünür. Vision AI durumu: {'Açık' if openai_vision_available() else 'Kapalı'}</div>", unsafe_allow_html=True)
...
def main() -> None:
            ...
            <div class="hero">
                ...
                <div class="hero-badges">
                    ...
                    <span class="hero-badge">Ton: {selected_editor_mode}</span>
                                        <span class="ghost-badge">Heatmap + Göz Akışı + Altın Oran</span>\n                    <span class="ghost-badge">Vision AI: {"Açık" if openai_vision_available() else "Kapalı"}</span>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with st.spinner("Fotoğraf okunuyor; metrikler, sahne profili ve Vision AI yorumu hazırlanıyor..." if openai_vision_available() else "Fotoğraf okunuyor, metrikler çıkarılıyor ve görsel katmanlar hazırlanıyor..."):
    ...
    with top_right:
        ...
        st.info((ai_report.get('global_summary') if isinstance(ai_report, dict) and ai_report.get('global_summary') else result.editor_summary))
