...
def build_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    def pick(options: List[str], salt: str) -> str:
        return _pick_by_seed(options, f"{editor_name}-{salt}-{subject}-{primary}-{secondary}-{score:.1f}")
    ...
    def concrete_observation() -> str:
        p = place(primary)
        s = place(secondary)
        d = place(distraction)
        sp = subject_phrase()
        options = [
            f"{p.capitalize()} duran {sp}, fotoğrafın ilk ağırlığını tek başına üstleniyor.",
            f"Göz önce {p} yerleşen {sp}e gidiyor; kadrajın kapısı oradan açılıyor.",
            f"Fotoğrafın asıl çağrısı {p} tarafında başlıyor; {sp} bütün bakışı oraya topluyor.",
            f"{p.capitalize()} kurulan vurgu net; kadraja giriş oradan oluyor.",
        ]
        if "at" in subject:
            options.extend([
                f"{p.capitalize()} duran at, fotoğrafın bütün ritmini sırtlayan ana unsur olmuş.",
                f"Atın {p} taraftaki yeri, kadrajın enerjisini tek seferde belirliyor.",
            ])
        if face_count >= 1:
            options.extend([
                f"{p.capitalize()} görünen yüz, fotoğrafı teknik bir görüntü olmaktan çıkarıp insana yaklaştırıyor.",
                f"Yüzün {p} tarafta kurulması, kareye hemen bir insan teması getiriyor.",
            ])
        if human_presence > 0.65 and face_count == 0:
            options.append(f"{p.capitalize()} duran figür ya da beden izi, fotoğrafın hikâyesini görünür kılıyor.")
        if "boşluk" in subject or "biçim" in subject:
            options.extend([
                f"{p.capitalize()} başlayan biçim ve boşluk ilişkisi, fotoğrafın ilk cümlesini kuruyor.",
                f"Kütle ile boşluğun {p} hattındaki karşılaşması, bakışı içeri çekiyor.",
            ])
        return pick(options, "obs")

    def relation_sentence() -> str:
        p = place(primary)
        s = place(secondary)
        options = [
            f"Sonra bakış {s} tarafına açılıyor ve fotoğrafın ikinci nefesi orada başlıyor.",
            f"İkinci okuma {s} hattında beliriyor; kare tek noktaya sıkışmadan genişliyor.",
            f"Kadraj yalnızca {p} ile kalmıyor, {s} tarafında da küçük bir karşılık buluyor.",
        ]
        if complexity == "yoğun":
            options.append(f"Arka taraftaki yoğun yapı, {p} ile {s} arasındaki geçişi canlı tutuyor.")
        elif complexity == "sade":
            options.append(f"Sadelik hissi sayesinde {p} ile {s} arasındaki ilişki daha rahat okunuyor.")
        return pick(options, "rel")
    ...
    def light_sentence() -> str:
        ...
        bottom = float(metrics.get("bottom_brightness", 0))
        hi = float(metrics.get("highlight_clip_ratio", 0))
        if abs(top - bottom) > max(abs(left - right), 8):
            if top > bottom:
                options = [
                    "Işık üstten indiği için sahnenin duygusu daha sessiz ama daha derin duruyor.",
                    "Üstten gelen ışık, kadrajı tepeden bastırmadan toparlıyor.",
                ]
            else:
                options = [
                    "Alttaki koyuluk ve üstteki açıklık birlikte güçlü bir geçiş kuruyor.",
                    "Aşağıdaki koyu alan ile yukarıdaki açıklık, fotoğrafa güçlü bir çıkış hissi veriyor.",
                ]
        elif abs(left - right) > 8:
            if left > right:
                options = [
                    "Soldan gelen aydınlık, bakışı doğal biçimde o tarafa topluyor.",
                    "Işığın solda daha açık kurulması, sahnenin dengesini belirliyor.",
                ]
            else:
                options = [
                    "Sağ taraftaki açıklık, gözün hareketini sessizce yönlendiriyor.",
                    "Işığın sağda açılması, fotoğrafın ritmine küçük ama etkili bir yön veriyor.",
                ]
        elif hi > 0.03:
            options = [
                "Parlak alanlar yer yer sertleşse de ışık fotoğrafın omurgasını kuruyor.",
                "Işık bazen sertleşiyor ama yine de ana duyguyu taşımayı başarıyor.",
            ]
        elif "sert" in light:
            options = [
                "Sert ışık burada işe yarıyor; sahneyi diri tutuyor.",
                "Işığın sertliği görüntüyü dağıtmıyor, aksine biraz daha ayakta tutuyor.",
            ]
        else:
            options = [
                "Işık yumuşak akıyor; sahnenin hissini bastırmadan taşıyor.",
                "Yumuşak ışık, görüntünün duygusunu zorlamadan öne çıkarıyor.",
            ]
        return pick(options, "light")

    def issue_sentence() -> str:
        d = place(distraction)
        options = [
            f"Yine de {d} tarafı biraz kısılırsa asıl vurgu çok daha temiz duyulur.",
            f"Tek kırılma noktası {d}; orası sakinleştiğinde kare hemen toparlanır.",
            f"Şu anda gözü en çok oyalayan yer {d}; o ağırlık hafiflerse fotoğraf daha net konuşur.",
        ]
        if score >= 70:
            options.append(f"Asıl mesele büyük değil; sadece {d} tarafındaki küçük ses fazlalığı biraz geri çekilmeli.")
        return pick(options, "issue")

    def warm_close() -> str:
        options = [
            "İçindeki şey iyi; biraz daha ayıklanınca çok daha uzun süre akılda kalır.",
            "Fotoğrafın kalbi yerinde, sadece çevresindeki gürültü biraz azalmalı.",
            "Zaten çalışan bir tarafı var; onu biraz daha görünür kılmak yeterli olur.",
        ]
        if score >= 70:
            options.append("Şimdiden güçlü duruyor; küçük bir temizlik onu daha da özel kılar.")
        return pick(options, "close")

    if editor_name == "Selahattin Kalaycı":
        sentences = [
            pick([
                concrete_observation(),
                f"{place(primary).capitalize()} duran ana vurgu doğru yerde; fotoğraf ilk anda oradan açılıyor.",
                f"Bu fotoğrafta kararın büyük kısmı {place(primary)} verilmiş; iyi olan taraf da orası.",
            ], "sel-first"),
            pick([
                relation_sentence(),
                f"{place(primary).capitalize()} başlayan akış, {place(secondary)} tarafına uzanıyor; bu iyi.",
            ], "sel-second"),
            pick([
                f"Ama {issue_sentence().lower()}",
                f"Benim takıldığım yer {issue_sentence().lower()}",
            ], "sel-third"),
            pick([
                "Daha net bir seçim yapılırsa fotoğraf hemen güçlenir.",
                "Kadrajın neyi savunduğu biraz daha belirgin olursa kare başka bir yere çıkar.",
            ], "sel-fourth"),
        ]
    ...
    if editor_name == "Güler Ataşer":
        sentences = [
            pick([
                f"İlk anda bana gelen şey, {place(primary)} duran {subject_phrase()} ile kurulan o küçük insan bağı oluyor.",
                f"{place(primary).capitalize()} tarafta insana dokunan yumuşak bir yer var; ben fotoğrafı önce oradan okudum.",
                f"Bu fotoğrafın kalbi bence {place(primary)} tarafında atıyor; göz orada kısa bir süre durmak istiyor.",
            ], "gul-first"),
            pick([
                light_sentence(),
                f"{light_sentence()} Bu da karedeki {mood} havayı daha görünür yapıyor.",
            ], "gul-second"),
            pick([
                f"{place(distraction).capitalize()} tarafı biraz sakinleşse bu duygu daha doğrudan geçer.",
                f"Şu an o bağı zayıflatan yer biraz {place(distraction)}; orası geri çekilirse fotoğraf daha rahat nefes alır.",
            ], "gul-third"),
            pick([
                warm_close(),
                "Bence burada güzel bir şey var; sadece biraz daha açıklık istiyor.",
            ], "gul-fourth"),
        ]
        return " ".join(sentences)
    ...
    if editor_name == "Sevgin Cingöz":
        sentences = [
            pick([
                f"{place(primary).capitalize()} ile {place(secondary)} arasında kurulan ilişki, fotoğrafın omurgasını oluşturuyor.",
                f"Bakışın yolu belli: önce {place(primary)}, sonra {place(secondary)}. Bu netlik kareyi ayakta tutuyor.",
                f"Fotoğraf tek noktaya yaslanmıyor; {place(primary)} ile {place(secondary)} arasında bir denge arıyor.",
            ], "sev-first"),
            pick([
                relation_sentence(),
                f"{complexity.capitalize()} yapı da bu akışı tamamen dağıtmadan destekliyor.",
            ], "sev-second"),
            pick([
                light_sentence(),
                f"{light_sentence()} Bu yüzden kadrajın ritmi kolayca kaybolmuyor.",
            ], "sev-third"),
            pick([
                issue_sentence(),
                "Biraz daha toparlandığında fotoğrafın etkisi çok daha berrak olur.",
            ], "sev-fourth"),
        ]
        return " ".join(sentences)
    ...
    if editor_name == "Mürşide Çilengir":
        sentences = [
            pick([
                f"{place(primary).capitalize()} duran {subject_phrase()}, sanki içine çektiği bir duyguyu saklıyor.",
                f"Ben bu karede önce {place(primary)} tarafındaki sessizliği duydum; fotoğraf kendini oradan açıyor.",
                f"{place(primary).capitalize()} bölümünde kalan his, görüntünün asıl sesini kuruyor.",
            ], "mur-first"),
            pick([
                f"{light_sentence()} Bu yüzden sahnedeki {mood} hava daha da görünür oluyor.",
                f"{light_sentence()} Kareyi güzel yapan şey de biraz bu; duyguyu bastırmadan taşıması.",
            ], "mur-second"),
            pick([
                f"{place(distraction).capitalize()} biraz geri çekilse o iç ses daha rahat duyulur.",
                f"Şu an o sessizliği en çok bölen yer {place(distraction)}; orası hafiflese fotoğraf daha derinden işler.",
            ], "mur-third"),
            pick([
                "Ben bu kareyi bağırmadan konuştuğu için sevdim.",
                warm_close(),
            ], "mur-fourth"),
        ]
        return " ".join(sentences)
    ...
    if editor_name == "Gülcan Ceylan Çağın":
        sentences = [
            pick([
                f"Kompozisyonun taşıyıcı hattı {place(primary)} ile {place(secondary)} arasında kurulmuş.",
                f"Gözü taşıyan yön duygusu {place(primary)}ten başlayıp {place(secondary)}e uzanıyor.",
                f"{place(primary).capitalize()} başlayan akış, {place(secondary)} tarafında tamamlanıyor; kompozisyon orada çalışıyor.",
            ], "gulc-first"),
            pick([
                light_sentence(),
                f"{complexity.capitalize()} yapı da bu yerleşimi destekliyor.",
            ], "gulc-second"),
            pick([
                f"Yalnız {place(distraction)} biraz ağırlık alınca denge tam yerinde kalmıyor.",
                f"Kadrajı biraz en çok zorlayan yer {place(distraction)}; orası sakinleşince kompozisyon daha temiz okunur.",
            ], "gulc-third"),
            pick([
                "Küçük bir toparlama ile bu yerleşim çok daha güçlü bir hale gelebilir.",
                "Kompozisyonun çekirdeği güzel; sadece son dokunuşunu istiyor.",
            ], "gulc-fourth"),
        ]
        return " ".join(sentences)
