...
def build_editor_comment(editor_name: str, result: CritiqueResult) -> str:
    ...
    anchor_sentence = _subject_sentence(subject, primary, face_count)
    light_sentence = _light_sentence(metrics, light, primary)
    flow_sentence = _flow_sentence(primary, secondary, distraction, metrics)

    if score >= 75:
        closing_soft = "Küçük dokunuşlarla uzun süre akılda kalacak bir kareye dönüşür."
        closing_direct = "Bu haliyle bile güçlü; ufak bir temizlik onu çok yukarı taşır."
    elif score >= 60:
        closing_soft = "Biraz toparlanırsa etkisi çok daha temiz ve kalıcı olur."
        closing_direct = "Kararını biraz daha net verirse kare hemen güçlenir."
    else:
        closing_soft = "İçindeki duygu var; biraz daha ayıklanınca daha net duyulur."
        closing_direct = "İyi bir çekirdek var ama daha net bir seçim istiyor."

    if editor_name == "Selahattin Kalaycı":
        first = _pick_by_seed([
            f"{anchor_sentence}",
            f"{_region_to_place(primary).capitalize()} tutunan şey doğru; fotoğraf ilk anda oradan açılıyor.",
            f"Bu karede asıl yük {_region_to_place(primary)} duran {subject} tarafında."
        ], f"{editor_name}-{primary}-{subject}-{score}")
        return " ".join([
            first,
            flow_sentence,
            f"Ama {distraction} tarafı gereğinden fazla ses çıkarınca ana etki biraz gevşiyor.",
            "Ben burada daha net bir tercih görmek isterdim; ya ana şeyi daha baskın bırakmalı, ya da çevreyi biraz susturmalı.",
            closing_direct
        ])

    if editor_name == "Güler Ataşer":
        first = _pick_by_seed([
            f"Bu fotoğrafa bakınca önce {mood} bir hava geliyor; sonra {_region_to_place(primary)} duran {subject} insana yaklaşıyor.",
            f"{_region_to_place(primary).capitalize()} tarafında insana dokunan yumuşak bir taraf var; kareyi ben en çok oradan sevdim.",
            f"İlk anda göze gelen şey {_region_to_place(primary)} kurulan küçük duygu alanı oluyor."
        ], f"{editor_name}-{mood}-{primary}-{subject}")
        return " ".join([
            first,
            light_sentence,
            f"Yalnız {distraction} biraz sakinleşse o duygu daha doğrudan geçecek.",
            f"Şu haliyle bile içinde sıcak bir damar var; sadece fotoğrafın nefesini biraz açmak gerekiyor.",
            closing_soft
        ])

    if editor_name == "Sevgin Cingöz":
        first = _pick_by_seed([
            f"Fotoğrafın yükü {_region_to_place(primary)} ile {_region_to_place(secondary)} arasında kurulmuş; bu iyi bir başlangıç.",
            f"Kare önce {_region_to_place(primary)} tarafında tutunuyor, sonra {_region_to_place(secondary)}e açılıyor; yapı orada kuruluyor.",
            f"Bu karede bakışın yolu belli; göz {_region_to_place(primary)}ten başlayıp {_region_to_place(secondary)}e doğru ilerliyor."
        ], f"{editor_name}-{primary}-{secondary}-{complexity}")
        return " ".join([
            first,
            f"{light_sentence} {complexity.capitalize()} yapı da bunu destekliyor.",
            f"Fakat {distraction} bölgesi biraz hafiflese ana vurgu çok daha temiz görünür.",
            "Karede iyi bir temel var; ben sadece ağırlık dağılımının biraz daha kararlı olmasını isterdim.",
            closing_soft
        ])

    if editor_name == "Mürşide Çilengir":
        first = _pick_by_seed([
            f"{_region_to_place(primary).capitalize()} duran {subject}, sanki içine çektiği bir şeyi sessizce taşıyor.",
            f"Bu karede beni en çok {_region_to_place(primary)} tarafındaki sessiz duygu çekiyor.",
            f"{_region_to_place(primary).capitalize()} bölümünde saklı kalan his, fotoğrafın asıl sesini kuruyor."
        ], f"{editor_name}-{primary}-{mood}-{subject}")
        return " ".join([
            first,
            f"{light_sentence} Bu da {mood} havayı daha görünür yapıyor.",
            f"Ama {distraction} biraz geri çekilse o iç ses daha rahat duyulacak.",
            "Ben bu kareyi, bağırmadan konuştuğu için sevdim; sadece etrafındaki gürültü biraz azalmalı.",
            closing_soft
        ])

    if editor_name == "Gülcan Ceylan Çağın":
        first = _pick_by_seed([
            f"Kompozisyonun ana hattı {_region_to_place(primary)} ile {_region_to_place(secondary)} arasında kurulmuş.",
            f"Bu karede gözü taşıyan şey {_region_to_place(primary)}ten {_region_to_place(secondary)}e uzanan yön duygusu.",
            f"{_region_to_place(primary).capitalize()} başlayan akış, {_region_to_place(secondary)} tarafında tamamlanıyor; kompozisyon orada çalışıyor."
        ], f"{editor_name}-{primary}-{secondary}-{light}")
        return " ".join([
            first,
            light_sentence,
            f"{complexity.capitalize()} yapı fotoğrafa katman vermiş; ancak {distraction} biraz ağırlık alınca denge tam kararında kalmıyor.",
            "Küçük bir kadraj toparlaması ya da ton sakinleştirmesiyle kompozisyon çok daha temiz okunur.",
            closing_direct
        ])
